...
for $L$ a fractional divisor, so this would not pose an issue.
	\item We included Hilbert polynomial in Examples $3.2- 3.5$.
...
The referee also made the following suggestions.
\todo{Aaron: Since there's only one thing here, can we remove this enumerate?
Alternatively, if we do end up including more than one of these, can we give the suggestion
and response together, instead of listing all the suggestions and then all the responses?}
\begin{enumerate}
	\item Change notation from $\frac{e_i - 1}{2 e_i}$
		to $\frac{s_i}{2 s_i + 1}$ where $s_i = \frac{e_i - 1}{2}$ and
		we range over all integers $s_i$ instead of odd integers $e_i$.
\end{enumerate}

We decided not to implement the above suggestions for the following
reasons, respectively.

\begin{enumerate}
	\item Firstly, we maintained the $e_i$ notation for consistency
	with the notation established in Voight--Zureick-Brown. These
	$e_i$ correspond to the stabilizer orders in the log spin
	canonical ring case considered there. Furthermore, we often
	list degrees of generation and relations enumerated by signatures
	of the form $(g; e_1, \ldots, e_n; \delta)$ with bounds given in
	the	form $\min(5, e_1, \ldots, e_n)$ which are more natural to
