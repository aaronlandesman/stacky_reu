...
\sssec*{Idea of Proof:}
We check the theorem in two cases, depending on whether $\Delta > 0$.
If $\Delta > 0$, we first check that the theorem holds for $L_X$
by adding in log points, and then check it holds for $L$ by raising
the stabilizer orders of stacky points in a sequence of steps. When
raising the stacky orders, it is important that we increment the
stabilizer orders of the maximum possible number of stacky points
at each step, so that the corresponding set will remain admissible.
