...
third condition is quite technical, but the conditions follow from
certain slope conditions, given by ~\ref{custom:sat-1-*},
~\ref{custom:sat-1-**}, and ~\ref{custom:sat-1-***}.  
...
= 0$. Then we check that the theorem holds for $\halfcan$ by adding
stacky points and then inductively raising the stabilizer orders of
stacky points in the following sequence of steps. When
