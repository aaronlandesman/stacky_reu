...
One method would be to use \cite{buckleyRZ:ice-cream-orbifold-and-riemann-roch}
...
First, by \cite{buckleyRZ:ice-cream-orbifold-and-riemann-roch},
\todo{in theorem 3.1, it states n is at least 2, what is going on?}
...
In order to show this is an isomorphism, one can write down the Hilbert series using \cite{buckleyRZ:ice-cream-orbifold-and-riemann-roch} and then use pole order considerations at $P_1$ to check the above relation exists. One can then check that the generators and relation determine a ring with the desired Hilbert series. The verification is analogous to Example ~\ref{eg:base-0-377} and is omitted.
\end{example}
...
The proof will be fairly involved. To show part (a), we 
...
at most one new primitive,
\todo{Aaron: What does ``new'' primitive mean? Does it mean the same thing as ``at most one primitive''
...
be elements of $R_Lx$
\todo{Aaron: What is $R_Lx$? Should this just be $R_L$?}
