...
\sssec*{Idea of proof:} The proof will be fairly involved. To show part (a), we first define a set of generators of $R_\halfcan$ over $R_{\halfcan'}$ with one generator corresponding to each best lower approximation. We then use Riemann--Roch to count the dimension of $R_\halfcan$ over $R_{\halfcan'}$ and show that the set of elements we produce forms a basis.

Next, part (b) immediately follows from an application of Lemma ~\ref{lem:relations_from_generators_induction} so we only need verify the hypotheses of Lemma ~\ref{lem:relations_from_generators_induction}. The first two hypotheses follow immediately from the definition of block order. Checking the third condition is quite technical but follows from certain slope conditions derived from the conditions ~\ref{custom:sat-1-*}, ~\ref{custom:sat-1-**}, and ~\ref{custom:sat-1-***}.
