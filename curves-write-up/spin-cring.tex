...
\todo{Use Hilbert polynomial in an example}
...
attention to effective divisors and in Lemma ~\ref{lem:sat-1} we are
able to obtain much tighter bounds.  Moreover, Lemma ~\ref{lem:sat-1}
also extends to curves of genus $g > 0$. 
%Before stating Lemma
%~\ref{lem:sat-1}, we recall some theory from continued fractions.
...
%
% proves 
%\begin{defn}\label{defn:cont-fract}
%Let $[a_1, \ldots, a_n]$ be the (Hirzebruch-Jung) continued fraction
%on $a_1, \ldots, a_n$ (given by $1 + \frac{1}{a_1+\frac{1}{... + \frac{1}{a_n}}}$).
%\end{defn}
%\begin{prop}\label{prop:primitives}
%The primitives 
%\end{prop}

%\noindent
%Figure \ref{fig:s14/5-lattice} gives a pictorial representation of the posi%tive best lower approximations of $\alpha = \frac{14}{5}$.
%\end{rem}
...
%\end{figure}
%\todo{Is this figure (figure 1)  still useful anywhere?}
...
\sssec*{Idea of proof:}\todo{Update subsection}
The proof will be fairly involved. To show part (a), we first apply 
literature on continued fractions to 
...
\beta} \rfloor \}$.
This reduces the problem at hand to finding the primitives of the cone in $\mathbb{Z}\times \mathbb{Z}$ bounded by $y=0$ and $y=\frac{\alpha}{\beta}x$.  

Literature on Cohn's geometric interpretation of hirzabruch jung fractions offers us an explicit formula for these primitives (Proposition 4.3 \todo{cite and add to bibtex}); one observes they all lie in degrees at most $\beta$ (since points in degree $\gamma=\beta+\omega$ can by written as a sum of points in degree $\beta$ with those in degree $\omega$).  Note that if two primitives had the same $y$ coordinate, then they would differ by a multiple of $(1,0)$ and thus could not both be generators; thus all the $c_i$'s (their $y$ coordinates) are distinct, and we can thus order them with $c_i<c_{i+1}$ for all $i$.  Furthermore, note that in any degree $k$ greater than 1, there can be at most one new primitive, since we can construct points with $y$ coordinates between 0 and $\lfloor k\frac{\alpha}{\beta}\rfloor$ as a sum of elements in degree 1 and degree $k-1$, and these new primitives have pole order greater than all primitives of smaller $x$-coordinate.  Therefore, the ordering of primitives with $c_i<c_{i+1}$ for all $i$ also ensures that the $x$-values $k_i$'s are also in weakly increasing order.  Finally, let $y_1, \ldots, y_n$ be elements of $R_Lx$ with pole orders given by $c_1,\ldots, c_n$ and degrees given by the $x$ coordinates of the corresponding primitives.
%We will the use positive best lower approximations to
...
%\noindent
%be the positive best lower approximations of $\frac{\alpha}{\beta}$. 
%5Choose a positive best lower approximation $\frac{c_i}{k_i}$.  Choose $z_1, \ldots, z_r \in R_\halfcan$ so that $\sum_{j=1}^r \deg(z_j) = k_i$ and $\deg(z_j)<k_i$ for all $j\in \{1, \ldots r\}$. Observe that $\frac{-\ord_P^{\halfcan'}(z_j)}{\deg(z_j)}< \frac{\alpha}{\beta}$, for each $j$.  Hence, for each $j$, 
%\[
...
and for all $r<h$ we have $\prod_{j=1}^r {y_j}^{b_j}\in S \cup S_0$. \todo{Check whether all these $S$ need to be changed to $S\cup S_0$ like here.}   
