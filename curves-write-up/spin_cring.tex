...
In the case that $M(\Gamma)$ has some odd weight modular form, it in fact has an odd weight modular form in weight $3$. This holds in the case that the genus $g \geq 2$ because $\deg \lfloor 3L \rfloor > 2g-1,$ so by Riemann--Roch, $\dim_k H^0(\sx,3L) > 0,$ implying there must be some generator in weight $1$ or $3$. The case that $M(\Gamma)$ has a generator in degree 1 or 3 when $g(X) = 0$ or $1$ follows from the fact that it has such a generator in the base cases given in Tables ~\ref{table:g-0-base-cases} and ~\ref{table:g-1-base}. Hence, the bound on the degree of generators and relations in the case that $M(\Gamma)$ has some odd weight modular form, given in Corollary ~\ref{cor:main-mod-forms}, is closely related to the degree of the minimal odd weight modular form.
\end{rem}
...
Suppose $\Gamma \subset SL_2(\BZ)$ is a congruence subgroup. Then, since the action  of $SL_2(\BZ)$ has points with stabilizer order 2 and 3, the action of $\Gamma$ can only have points with stabilizer order 2 and 3. Furthermore, if $\Gamma$ has some odd weight modular form, then it must have no points with stabilizer order 2 (see Remark ~\ref{rem:odd-denom}). Therefore, by Corollary ~\ref{cor:main-mod-forms}, $M(\Gamma)$ is generated in degree at most $5$ with a relation in degree at most $12$. Furthermore, if the coarse space associated $\Gamma$ does not have .
\end{example}
