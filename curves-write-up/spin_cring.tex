...
$M(\Gamma) = \bigoplus_{k = 0}^\infty M_k(\Gamma)$. One of the best ways to desribe the ring $M(\Gamma),$ is to write down a presentation.
...
ordering on $\Bk[y_{d_1', 1}, \ldots, y_{d_m', m}] \otimes \Bk[x_{d_1, 1},
...
Counting dimensions, we see $h^0(\sx, dL')=1$ for all $d\in \mathbb{Z}_{\ge 0}$ so it is immediately clear that $R_{\halfcan'} \cong \Bk[x]$ with $x$ a generator in degree 1.
\end{example}
...
\[
	\dim_\Bk H^0 (\sx', d K_{\sx'}) = 1, 0, 0, 1, 0, 1, 1, 2, 1, 1, 2, 1, 3, 2, 3, \ldots
\]
...
relations, we show that $\dim_\Bk (R')_d = \dim_\Bk
(\Bk[x_{7, 2}, x_{7, 1}, x_{5, 1}, x_{3, 1}] / \initial(I))_d$ for 
all degrees $d \geq 0$. In particular, 

\begin{align*}
	\dim_\Bk \left(\Bk[x_{7, 2}, x_{7, 1}, x_{5, 1}, x_{3, 1}] / I\right)_d &=
	\dim_\Bk \left(\Bk[x_{7, 2}, x_{7, 1}, x_{5, 1}, x_{3, 1}]	/ \initial(I)\right)_d \\
	&= \dim_\Bk \left(\Bk[x_{7, 2}, x_{7, 1}, x_{5, 1}, x_{3, 1}] / \langle
	x_{7, 2}^2, x_{5, 1}^2 \rangle \right)_d
\end{align*}
\noindent
for all $d \geq 0$. But the fact $\dim_\Bk (R')_d = \dim_\Bk \left(\Bk[x_{7, 2},
x_{7, 1}, x_{5, 1}, x_{3, 1}] / \langle x_{7, 2}^2, x_{5, 1}^2
\rangle\right)_d$ can be checked by looking at the $\Bk$-basis at each degree
$d$.
...
\] 
so $S$ contains elements in degree $d \in \BN$ with each pole order in $\{1 \ldots, \lfloor d\frac{\alpha}{\beta}\rfloor\}$.  Thus, by dimension counting, $S$ forms a $\Bk$-basis for $R_\halfcan$ over $R_{\halfcan'}$, and we have proven part (a).
...
\[
	\Bk[u, x_1, \ldots, x_m] \cup V \cup \left(\cup_{i=1}^n y_i \cdot \Bk[u, y_1, \ldots, y_n]\right). 
\]
...
Since the ideal in $\Bk[u, y_1, \ldots, y_n]$ generated by is $S$ is $(y_1, \ldots, y_n) \cdot \Bk[u, y_1, \ldots, y_n]$ and $\bigcup_{i=1}^{n-1} U_i$ generates every monomial in $\cup_{i=1}^n y_i \cdot \Bk[u, y_1, \ldots, y_n]-S$, we have shown that all monomials of $\Bk[u, x_1, \ldots, x_m, y_1, \ldots, y_n]$ are contained in
\begin{align*}
...
i'}^ay_{i, e_i}^b$ with $a \geq 0, b > 0$ form a $\Bk$-basis for $R_{i}$ over $R_{i-1}$. These elements do not lie in $R_{i - 1}$ because the pole order of $y_{i, e_i'}^ay_{i,e_i}^b$ at $Q_i$ is larger than that of any element in the $d^{th}$ component of $R_{i - 1}$. 
...
\[
	J_{\halfcan,d} = \sum_{j = 1}^{d- 1}\Bk[x_1, \ldots, x_n]_j \cdot I_{\halfcan,d-j}.
\]
\end{defn}
...
\Bk[x_1, \ldots, x_n]_{d - 2}$ so that $s_1 g + s_2 h \equiv f \bmod J_{\halfcan, d}.$
\end{lem}
\begin{proof}
...
By Lemma ~\ref{lem:generation-5}, $\deg x_i \leq 5$ for $1 \leq i \leq n$. Therefore, we may write $f = \sum_{i = 1}^{n}a_i x_i$ with $a_i \in \Bk[x_1, \ldots, x_n]_{d-\deg x_i}$.
We next show that for all $1 \leq i \leq n$ there exist $g_i, h_i \in \Bk[x_1, \ldots, x_n]_{d - \deg x_i - 2}$ so that $a_i = s_1g_i + s_2h_i \bmod I_{\halfcan, \deg a_i}.$ 
...
Therefore,
$g \equiv s_2 \rho \bmod I_{\halfcan, d-2}$ and $h \equiv - s_1 \rho \bmod
I_{\halfcan, d - 2}$. Hence,
\begin{align*}
