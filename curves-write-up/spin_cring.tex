...
and initial ideal of relations of log spin canonical rings from a presentation of the canonical ring of the coarse space. Many
...
generated in degree at most $5$ with relations in degree at most $10$. In fact, it is not difficult to reduce generation in weight at most 5 to generation in weight at most 4, as noted in Remark ~\ref{rem:cusp-generation-4}. Note that the exceptional cases in Tables ~\ref{table:g-1-exceptional} and ~\ref{table:g-0-exceptional} do not occur because there is a cusp, so $\delta > 0$.
\end{example}
...
In addition to the bound on the degree of the generators and relations, as detailed in Theorem ~\ref{thm:g-1-main}, the proof of Theorem ~\ref{thm:g-1-main} yields an explicit procedure for computing those minimal generators and relations. That, is one can start with the generators and relations found in the base cases and inductively add in generators and relations as one adds and then increments stacky points. As described in Remark ~\ref{rem:genus-2-explicit-bound}, when $e: = \max(e_1,\ldots, e_r) \geq 7$, there is necessarily a generator in degree $e$ and a relation in degree $2e$.
\end{rem}
...
Similarly to the case of genus 1, as described in Remark ~\ref{rem:genus-1-explicit-bound}, the proof of Theorem ~\ref{thm:g-0-main} gives an explicit construction of the generators and relations for the log spin canonical ring $R_L$. Furthermore, as described in Remark ~\ref{rem:genus-2-explicit-bound}, when $e := \max(e_1, \ldots, e_r)$ is at least $7$, there is a generator in degree $e$ and a relation in degree at least $2e-4$. This can be seen from the inductive application of Lemmas ~\ref{lem:sat-1}, \ref{lem:sat-2}, and \ref{lem:sat-3}.
\end{rem}

\begin{rem}
\label{rem:cusp-generation-4}
Here, we describe how to obtain a slightly better bound for our application to modular forms from example ~\ref{eg:congruence-bounds}. A careful scrutiny of Theorem ~\ref{thm:g-0-main} reveals that, if $\Delta > 0$ and $\sx$ has signature $(0;3, \ldots, 3; \delta)$, then $R_L$ is generated in degree at most 4. This is $\delta > 0$, so $L_X$ is effective. Addtionally, $R_{L_X}$ is generated in degree 1, and inductively applying Lemma ~\ref{lem:sat-1} only adds generators in degree 3 and 4. Therefore, $R_L$ is generated in degree at most 4. Note that a similar analysis of the proof of Theorem ~\ref{thm:g-1-main} yields that when $g = 1$, congruence subgroups are generated in degree at most 4.
\end{rem}
...
\begin{enumerate}
	\item As noted in Remark ~\ref{rem:explicit-generators}, the 
		...
		\halfcan$ when the genus of $\sx$ is $0$ or $1$.  When $\sx$ has genus at least 2, Lemmas ~\ref{lem:sat-1} and $\ref{lem:sat-2}$ allow us to explicitly construct a presentation from a presentation of its coarse space $X$.  However, computing a presentation of a general curve $X$ requires nontrivial computation.  This suggests the following Petri-like question:
		\begin{question}
		\label{ques:spin-canonical-petri}
			Is there a general structure theorem for of a set minimal 
			generators and minimal relations for $R_\halfcan$, a {\rm(}non-stacky {\rm)} curve $X$ and a spin canonical 
			divisor $L \in \di X$?
		\end{question}
