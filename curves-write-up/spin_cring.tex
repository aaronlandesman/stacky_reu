...
of generators and an explicit initial ideal of relations, as described in Remarks ~\ref{rem:genus-1-explicit-bound} and ~\ref{rem:genus-0-explicit-bound}.
...
\label{eqn:noether-377}
	...
	(k - 21) \halfcan' \rfloor) \twoheadrightarrow H^0 (\sx', \lfloor
	k \halfcan' \rfloor)
\end{align}
...
\begin{align*}
	...
	(k - 3) \halfcan' \rfloor) \rightarrow H^0 (\sx', \lfloor
	k \halfcan' \rfloor) \text{ if } k \geq 12 \text{ and } 3 \nmid k \\
	&H^0 (\sx', \lfloor 7 \halfcan' \rfloor) \otimes H^0 (\sx', \lfloor
	(k - 7) \halfcan' \rfloor) \rightarrow H^0 (\sx', \lfloor
	k \halfcan' \rfloor) \text{ if } k \geq 6, k \not\equiv 0, 5
	\bmod 7
\end{align*}
...
Suppose $c,d \in \BN$ so that $k \le \beta$,  $\frac{c}{d} < \frac{\alpha}{
...
Using the terminology from the proof of Theorem ~\ref{thm:g-high-main}, if $e = \max(e_1,\ldots, e_r) \geq 5$, we see that $R_L,$ has a generator in precisely $e$. This follows from the inductive application of Lemmas ~\ref{lem:sat-1} and \ref{lem:sat-2}, which, at the last step, add a minimal generator of $R_L$ over $R_L'$ in degree $e$. Similarly, this inductive application shows that our given presentation has a relation in degree $2e-4$.
\end{rem}
...
In addition to the bound on the degree of the generators and relations, as detailed in Theorem ~\ref{thm:g-1-main}, proof of Theorem ~\ref{thm:g-1-main} yields an explicit procedure for computing those minimal generators and relations. That, is one can start with the generators and relations found in the base cases, and inductively add in generators and relations as one adds and then increments stacky points. Since the proof of Theorem ~\ref{thm:g-1-main} is given by inductively applying Lemmas ~\ref{lem:sat-1} and ~\ref{lem:sat-2}, when $e = \max(5,e_1,\ldots, e_r) \geq 5$ we obtain that $R_L$ has a generator in degree $e$ and a relation in degree at least $2e - 4$. Further, by examining the statements of Lemma ~\ref{lem:sat-1} and \ref{lem:sat-2} in the case that there $1 \leq i < j \leq r$ with $e_i = e_j = e$, then, at least in our construction, there is a relation in degree $2e$.
\end{rem}
...
Just like the case of genus 1, as described in Remark ~\ref{rem:genus-1-explicit-bound}, the proof of Theorem ~\ref{thm:g-0-main} gives an explicit construction of the generators and relations for the log spin canonical ring $R_L$. Furthermore, similarly to the description given in Remarks in ~\ref{rem:genus-2-explicit-bound} and Remark ~\ref{rem:genus-0-explicit-bound}, when $e = \max(e_1, \ldots, e_r) \geq 5,$ there is a generator in degree $e$ and a relation in degree at most $2e$. This can be seen from the inductive application of Lemmas ~\ref{lem:sat-1}, \ref{lem:sat-2}, and \ref{lem:sat-3}. Additionally, as in the case of genus 1, if there are $1 \leq i < j \leq r$ with $e = e_i = e_j$ then there is a relation in degree $2e$. 
\end{rem}
