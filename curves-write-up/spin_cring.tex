...
\todo{Brian: Don't write out author's name every time.}
...
presentation as a quotient of a polynomial ring. \todo{Brian: Also, sentence 3 of paragraph 1 of the Introduction is debatable (depends on what is meant by “understand”…); e.g., how do you propose to find generators for $M_1(\Gamma)$ when not known to be 0?}
...
\todo{brain: The appeal to stacky GAGA on page 1 sweeps a bit under the rug.  I suppose you noticed (as I just did upon glancing at [14] on the arxiv) that the proof of Prop. 6.1.5 in [14] seems to pass in silence over the removal of the properness hypothesis (which, in the scheme case — no stacks — requires using something like the little Picard theorem or some other device to deal with avoidance of essential singularities).  That is moot for you since only the proper case is relevant for your motivation.  More importantly, since the entire discussion is about spaces of sections of coherent sheaves, one should address stacky GAGA at the level of such $H^0$’s, which is not part of Prop. 6.1.5 in [14], to have adequately treated the motivation.  On line -1 of last paragraph on page 1, should say X is proper and geometrically connected over k.  Note that the proof of Lemma 10.2.1 in [14] gives a thoroughly analytic construction of the line bundle (no ad hoc stuff with divisors), so to link it back to the algebraic case one is using an honest GAGA comparison at the level of categories of coherent sheaves too (with refinement for line bundles).}
...
\]
\todo{Check this takes care of brian's comment 0.}
...
\end{cor}
\todo{Add somewhere that if there is an odd $k$ with nonzero weight, it will generally have to occur in weight 3, according to brian 2.}
\begin{rem}
...
\todo{Add example saying all congruence subgroups with a form of odd weight are generated in weight at most 5 with relations in weight at most 12, (which can be reduced to 10 if the signature is not (0;3,3,3,3;0). Further, if $L_X$ is effective, then generation will be in weight at most 3.}
...
of schemes.
\end{rem}
\todo{brain: Although Def. 2.3 makes sense for any $e_i > 1$, is this the “right” notion when char(k) divides some $e_i$?  (The “corresponding” stack would not be tame, for example, so I’d be hesitant to think that various notions building on Def. 2.3 are really appropriate if char(k) divides some $e_i$; of course, the main motivation comes from char. 0….).}
\begin{defn}
\label{defn:div-ex}
...
represent $R(\sx, \Delta, \halfcan)$. We also often use $H^0(\sx, \sco(D))
:= H^0(\sx, D)$ and $h^0(\sx, \sco(D)) := \dim_k H^0(\sx, \sco(D)),$ where $\sco(D) = \{f \in k(X)^\times : \di f + D \geq 0 \}.$
\todo{Check the above answers brian's objection 4. Also, can $X$ be non-integral, then there might be a problem with the above definition.}
...
Note that $R_{L_X} \cong k[x_1, x_2]$ where $\deg x_1 = \deg x_2 = 1.
...
\hookrightarrow R_L$ induced by the inclusions $H^0(\sx, dL_X)
\hookrightarrow H^0(\sx, dL)$ for each $d \geq 0$. By Riemann Roch,
we see there is some element $y_{1, 3} \in (R_L)_3$ with $\ord_{P_1}
(y_{1, 3}) = -1$, not in the image of the inclusion $\iota$. We
claim that there exist $a_1, a_2 \in k$ and a degree $4$ polynomial
$f(x_1, x_2) \in k[x_1, x_2]$ such that
\begin{align*}
	R_L \cong k[x_1, x_2, y_{1, 3}] / (a_1 x_1 y_{1, 3} + a_2 x_2 y_{1, 3}
	+ f(x_1,x_2))
\end{align*}
...
\begin{align*}
	H^0(\sx, 3L) \otimes H^0(\sx, (d - 3)L) \rightarrow H^0(\sx, dL)
\end{align*}

\noindent
are surjective for $d \geq 4$. A relation of the form $a_1 x_1 y_{1,
...
\label{align:surjection}
	k[x_1, x_2, y_{1, 3}] / (a_1 x_1 y_{1, 3} + a_2 x_2 y_{1, 3} + f(x_1, x_2)) 
	\rightarrow R_L.
\end{align}
...
$k[x_1, x_2, y_{1, 3}] / (a_1 x_1 y_{1, 3} + a_2 x_2 y_{1, 3} +
...
In order to show this is an isomorphism, one can use Riemann--Roch and pole order considerations at $P_1$ to check the above relation exists. One can then check that the generators and relation determine a ring $R$ so that for all $j \in \BZ,$ the $j$th graded component $(R)_j$ satisfies $\dim_k (R)_j = \dim_k R_\halfcan.$ Hence, $R \cong R_\halfcan$. The verification is analogous to Example ~\ref{eg:base-0-377} and is omitted.
...
Suppose $\halfcan$ is a divisor of $X$ such that $R_\halfcan$ is generated by $x_1, \ldots x_m$.  Then we have a map $\phi: k[z_1, \ldots z_m] \rightarrow R_\halfcan, z_i\mapsto x_i$. If $P$ is a point in $X$, then $\phi$ defines a graded-$P$-lexicographic order (shortened to {\bf{graded $P$-lex}}) on $k[z_1, \ldots, z_m]$ as follows.
...
$\gamma^{\otimes a} \cong \eta$ \todo{In item 2 of section 8, I see no reason to specifically focus on X(p).  (I am also skeptical that the definition given there is appropriate; for weight k/2 with odd k it seems not the right definition.  I’ve never looked at [2] or [9].)}
 and 
 $\gamma^{\otimes n} \cong \Omega_{X(p)}^1 \otimes \sco_{X(p)}(\Delta)$.
...
We would also like to mention Evan O'Dorney for enlightening
conversations regarding his work in this field.
