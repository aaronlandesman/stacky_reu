...
Counting dimensions, we see $h^0(\sx, dL')=1$ for all $d\in \BZ_{\ge 0}$ so it is immediately clear that $R_{\halfcan'} \cong k[x]$ with $x$ a generator in degree 1.
\end{example}
...
\end{align*}
Indeed, such a relation must exist, because by Riemann--Roch, $h^0(\sx,4L)$
...
\label{eqn:deg1-sat-ind}
	...
	\rfloor) + \left\lfloor d\frac{\alpha} {\beta} \right \rfloor &&\text{ for all } d \in \BN.
\end{align}
Let
\[
...
Notice that if $r\le n$, $\omega_1, \ldots \omega_r\in \BZ_{\ge 0}$, and $\prod_{j=1}^r y_j^{\omega_j}\in S$, then our definition of $S$ implies $y_r\prod_{j=1}^r y_j^{\omega_j}\in S$ if and only if $c_r$ is maximal among $c_1, \ldots c_n$ not greater than than $c_r + \sum_{j=1}^r c_j \omega_j$.
