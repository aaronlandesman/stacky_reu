...
Another closely related topic is the Hassett-Kell program, which aims to describe log canonical models of the form
\begin{align*}
...
\ssec{One Hypersurface on Projective Space}
\label{ssec:proj-one-point}

\begin{rem}
\label{rem:proj-veronese}
...
Note that the method of proof for Theorem ~\ref{thm:proj-one-point}
...
~\ref{thm:proj-one-points}.
\end{rem}

\begin{thm}
\label{thm:proj-effective}
Let $X = \bp^m$ with $\{x_0, \ldots, x_m\}$ a coordinate system for
$\bp^m$. Let $D = \sum_{i = 1}^{n} \alpha_i H_i$, with $\alpha_i =
\frac{p_i}{q_i} \in \bq$ and each $H_i$ a hypersurface of
...
\begin{proof}
First, we apply Remark ~\ref{rem:proj-veronese} to reduce the
problem to letting each $H_i$ be a hyperplane of $\bp^m$.
Then, the proof follows from applying Remark
~\ref{rem:proj-one-point-ind} inductively on a one-hyperplane case
from Theorem ~\ref{them:proj-one-point}.
\end{proof}
