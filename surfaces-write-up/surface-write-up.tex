...
\right \}.
\end{align}
\todo{Aaron: Possibly take this definition out of the proof, and move it into
	the setup, since we want to reference it later.  \\
Peter: I like having the statement of the lemma being nice and consise; also
the statement has nothing to do with this geometric picture, so I find it more
natural to have the geometric side contained in the proof.  I also don't quite
see why it is so bad to reference something from inside a previous proof; a
reader can surely locate it...

Aaron: In general, I like the idea of having proofs self contained,
so that one should only refer to the proof statement, not the
body of the proof.
I'm not very strongly tied to this idea though, and I think it's fine. I think it's fine.
}
...
\begin{equation}\label{eqn:epsilon_i,j}
	...
	\end{split}
\end{equation}
Note that the above expression makes sense when $i < j$, but one
can similarly define $e_{i,j}$ when $j < i$.  
\todo{Peter: I changed the above expression,
but I'm still feel a little dubious that this is easier to read...

Aaron: I agree this is harder to read. Why don't we just write
$e_{i,j} = a e_i + b e_j$ (where of course you replace a and b by explicit
constants). Wouldn't that be much cleaner? Also, I think we should
explicitly state what the small rearrangement is to switch 
$i$ and $j$, but perhaps it won't be relevant if you write $e_{ij} $
as a linear combination of $e_i, e_j$.}
...
\todo{Aaron: Shouldn't we say something above like, the kernel
	is bounded by the maximum of something over all $i$,
	so for the remainder of the proof, we fix an $i$.
	Also, I think it would help if things
	that depend on $i$, such as $E,\sigma,\lambda$
have an $i$ subscript.}
...
Aaron: OK. In that case, why don't you say that? Since I didn't understand those constraints on $r_{i,j}$, I think
the reader would probably also be lost without explicitly writing those constraints.
Also, what about $\lambda$? Is that uniquely determined?}
...
\todo{Aaron: Would it be reasonable to given these $q_{ij}$ a $\mu$
superscript or subscript to indicate dependence on $\mu$?}
and $\lambda \in
