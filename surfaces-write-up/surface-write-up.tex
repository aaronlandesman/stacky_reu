...
Our first aim in this section is to prove Theorem ~\ref{thm:proj-generators-relations} bounding the number of generators and relations of $R_D$. Before doing so, we justify the assumption ~\eqref{eqn:bpm-basis-assumption} through a several convincing examples.
...
In Example ~\ref{eg:radical}, we show that it is still, in general, necessary to add ghost points, even when the irreducible components of a divisor are not all contained in a hyperplane.
\end{rem}
...
Combining the above results, we obtain the following bound on the degrees of generators and relations of $R_D$.
...
In this section, the main aim is to prove Theorem ~\ref{thm:}, bounding the degree of generators and relations of the canonical ring of a $\bq$-divisor on any hirzebruch surface.
...
We first consider the subcone $\Sigma_1 \subset \Sigma$
given by
\begin{align*}
