...
\todo{Aaron:
Does $E$ depend on $\omega$? I.e. have we fixed an $\omega$ for the rest of the proof?
If so, why can we do this?
\\
Your write, it depends on $\omega$, not just $i$.  We can do this, because we will take the maximum over all $\omega$.  What I mean by this, is for any starting relation (with initial term $\omega$ under some appropriate order) we reduce it to a different relation of the form described in the proof.  We then take the maximum of the degrees of the relation that any $\omega$ is reduced to.
}
\todo{Aaron: If $i$ has been fixed for the remainder of the proof, why 
are we using $i$ as dummy subscripts everywhere?\\
I don't follow; as in $e_i$'s?  or $e_{i,j}$'s?  $i$ is fixed in all of these statements, whereas $j$ ranges as necessary ($j$ is not fixed).}
with $E$ of the form
\begin{equation}\label{eqn:hirz-E-translation}
	...
	\sum_{j\in \Tp} r_{j,i} e_{j,i}: r_{j,i} \in \bq, \sum_{j\in \Tp} r_{j,i} \kappa_{i,j}'' = 1 	& \mbox{ if } i\in \Tm.
	\end{cases}
\end{equation}
Specifically, if we write out $\omega = \sum_{j\in \Te} s_j e_j + \sum_{j\in \Tp, k \in \Tp} s_{j,k} e_{j,k}$, then any choice of $r_{j,k}$'s not greater than the respective $s_{j,k}$'s such that $r_{i,j} \kappa_{i,j}' =1$ (when $i\in \Tp$ but a similar statement holds for $i\in \Tm$) will suffice.  Such a choice of $r_{i,j}$'s uniquely determines the necessary $\lambda$.
