...
\\
Peter: I understand where your objection is coming from.  However, I don't see a good way to fix it without creating a very ugly statement of the lemma.
}
...
\begin{equation}\label{eqn:hirz-E-translation}
	...
	\end{cases}.
\end{equation}
Specifically, if we write out $\sigma = \sum_{j\in \Te} s_j e_j + \sum_{j\in \Tp, k \in \Tp} s_{j,k} e_{j,k}$, then any choice of $r_{j,k}$'s not greater than the respective $s_{j,k}$'s such that $r_{i,j} \kappa_{i,j}' =1$ (when $i\in \Tp$ but a similar statement holds for $i\in \Tm$) will suffice.  Such a choice of $r_{i,j}$'s uniquely determines the necessary $\lambda$.
