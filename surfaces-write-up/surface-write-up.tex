...
\maketitle
\todo{Aaron: Remove labels which are not referenced}
\todo{Aaron: Make sure indexing at 0 is correct in projective space
section, particularly toward the end}
%%%%%%%%%%%%%%%%%%%%%%%%%%%% Introduction %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Let $\bp^m$ have projective coordinates $x_0,\ldots, x_m$. Let
$f_0, \ldots, f_n$ be homogeneous polynomials in $x_0, \ldots, x_m$ such that
for $1 \leq i \leq n$ we have $D_i = V(f_i)$ and $\deg(f_i) = a_i,$ and such that $f_0,\ldots, f_m$ are independent linear polynomials in $x_0, \ldots, x_m$. 
Let $D =
\sum_{i=0}^n \alpha_i D_i \in \bq \otimes_\bz \di \bp^m$ where
$\alpha_i = \frac{c_i}{k_i}$.
Then notate 
\[\ell_i = \lcm_{j\ne i}(k_j).\] 
...
\noindent
where $\alpha_i \in \bq$ and $D_i \in \di X$ is an integral
...
\sum_{i=1}^{n}\alpha_i$. For such a divisor $D$ with $\alpha_i =
...
~\eqref{align:pm-span} lie in $H^0(\mathbb{P}^m,dD)$, and all functions of that form lie in $H^0(\mathbb{P}^m, dD)$.
\todo{Aaron: Peter, what do you mean when you say ``all functions of that form lie in...''? How is
this different from saying that functions satisfying that equation lie in $H^0$?}
...
of degree $\deg \lfloor d \deg D \rfloor $ \todo{Peter: Why are we looking at
monomials in this degree?  Don't we want to say monomials in degree $d$ or
something?

Aaron: You're right it was a typo. I fixed it now though. The monomials
in this degree are precisely those monomials which
are a basis for $H^0(dD).$ If we just took monomials in degree d,
those would only form bases in the case $\deg D = 1$. Here, we're talking
about a $\bk$ basis, so the fact that there is a d there doesn't really matter,
we only care about generatord for the vector space $H^0(dD)$ where $dD$ is some
arbitrary divisor.
}
...
nonzero. Define a relation on $\bz^{m+1}$ by $\vec{v} \prec \vec{w}$ if $i \leq j$.  \todo{Peter: Why do we have $i$ defined as the biggest and $j$ defined as the smallest?  Anyway, I don't believe this is a partial ordering.  Specifically, I don't believe it satisfies antisymmetry:
\\
Let $m=1$.  $\vec{v}= (1,0), \vec(w) = (2,0)$.  Then $\vec{w} \prec \vec{w}$
and $\vec{w} \prec{v}$, which by antisymmetry should imply $\vec{v} = \vec{w}$.
I'm not sure if I am missing something simple in the definition ... but this
seems to contradict it being a partial order.  Maybe we don't actually care
that it is a partial order though\ldots.

Aaron: Good catch. You're right though, that it's not important
that it is a partial ordering. I changed it to a relation.
}
\end{defn}
...
\begin{align} \label{eqn:pm-basis-assumption} f_0, \ldots, f_{m}
	\text{ are $m$ independent linear forms.} \end{align}
...
\begin{equation}\label{eqn:epsilon_i}
	e_i \colon = d_i \epsilon_i %(-\alpha_1 d_i, \ldots, - \alpha_{i-1} d_i, \frac{d_i}{\bida_i + \bidb_i}\sum_{k \ne i}
