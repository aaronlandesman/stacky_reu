...
$f_i \in \sco(\bida_i, \bidb_i)$.  Suppose $f_1$ and $f_2$ are independent linear
polynomials in $x_0$ and $x_1$ and $f_3$ and $f_4$ are independent
linear polynomials in $y_0$ and $y_1$.
...
\[
	...
	\max_{j \in S_-} \bigl(\ell_{i, j} (\bida_i \bidb_j - \bida_j \bidb_i)\bigr)
	&\mbox{ if } i \in S_+ \\
	\max_{j \in S_+} \bigl(\ell_{j, i} (\bida_j \bidb_ i - \bida_i \bidb_j) \bigr)
	&\mbox{ if } i \in S_- \end{cases}.
\]
...
\[
	\tau = \sigma
	+ \max \left(\max_{i\in S_=} \bigl(\ell_i \gcd(a_i, b_i) \bigr),
	\; \max_{\substack{i \in S_+ \\ j \in S_-}} \bigl(\ell_{i, j}
