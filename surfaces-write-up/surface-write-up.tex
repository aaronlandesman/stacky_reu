...
\todo{Make sure to include $u^d$ everywhere necessary}
...
rings for projective spaces $\bp^m$ and Hirzebruch surfaces $\hirz_m$ \todo{Peter: Bad to have $m$ as both the dimension of $\bp$ and the coefficient for the hirzebruch surface?}.
When $D$ is a general $\bq$-divisor on $\bp^m$ or
$\hirz_m$, we give bounds on the generators and relations of
$R(X, D)$. In particular, we give a complete description \todo{Peter: The phrase ``complete description'' is ambiguous and might be objectionable to a reader.  Is ``minimal presentation'' better?} of the
...
Another closely related topic is the Hassett-Keel program \cite{hassett:classical-and-minimal-models}, which aims to describe log canonical models of the form \todo{Robin: define $\mathscr{M}_g$? Not sure what this is.  \\
Peter: Isn't it defined in the line below?  (Maybe it was added after your comment)}
\begin{align*}
...
basis for degree one rational functions on $\bp^m$.  \todo{Peter: This assumption is inconsistent with a comment made later (see next todo); later, we allow for there to be only one $D_i$ ($D= D_i$), which contradicts $m>0$.}
...
stated explicitly later, in the case that $D = \alpha_1 D_1$ \todo{Peter: Inconsistent with previous assumption.  To be clear it doesn't matter for any proofs, because that assumption is not necessary until after this section.  I suggest we find some way to move that assumption from earlier until after the effective case.} is supported on a single hypersurface, the relations in Theorem ~\ref{thm:proj-one-point} form a reduced grobner basis with respect to this ordering.
...
$\deg D_i = \bida_i$, $\alpha_i = \frac{c_i}{k_i}\in \bq$, and
$\ell_i = \lcm_{j \neq i} (k_i)$. Define $\Sigma$ as in Equation
...
\todo{Aaron: Do either of you think we should write out this as
a proposition more clearly? I'm fine either way, but it was something
missing which I thought I should point out. \\
Peter: The one line argument is ``Every numerator is a polynomial in $u, v, z, w$; we can generate $u$ and $v$ from $f_1$ and $f_2$, and we can generate $z$ and $w$ from $f_3$ and $f_4$.''  This can be added if you want

Aaron: I agree that argument is obvious. I was more referring to the analog
of proposition ~\ref{prop:pm-span-and-basis}, that is, how to show
every numerator is a polynomial in $u,v,z,w$. This would probably be
an argument that takes a couple paragraphs.
\\
Peter: I see what you mean.  What was the argument for that again.  It was some exact sequence thing you came up with right?
}
...
\todo{Aaron: The next two paragraphs are missing a lot of $u^d$ degree prefixes. Can one of you add them in and go through the proof
to make sure there are no others missing?\\
Peter: I added a bunch.  Should be better (although quite possibly missed a few more).}
Specifically, $\beta_i$,
