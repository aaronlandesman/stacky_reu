...
\end{proof}\todo{Peter: Maybe we can get rid of most of this proof, and replace it with: ``The proof follows by examining the exact sequence $0 \to \ker\chi \to \ker \phi \to \ker \psi \to 0$.''  I don't think it is necessary to prove why this is an exact sequence, as it is taught in a first commutative algebra course.}
...
~\ref{prop:cone-generation} tells us that every element $\sigma \in \Sigma$ can be written
...
$\sigma = \lambda + \sum_{i=0}^{t}\zeta_i e_i$ from proposition Proposition
...
for which both of its bi-degrees are positive \todo{Do we need to specify what we mean by the bi-degrees of a divisor, or will that be apparent to a reader?}. We now justify this
...
$(\beta_i - z_i) \cap \ker(\psi)$, we fix $i \in \{1,\ldots, n\}$ and a relation of 
the form $u^d(z_i - \beta_i)\prod_{j=1}^n z_j^{c_j}$ {\bf for the remainder of the proof}.  Note that there 
...
our work possible.\todo{Peter: Add acknowledgements to a few more peopled - Robin's dad; was there anyone else?}
