...
\begin{example}
\label{eg:radical}
In this example, we justify the necessity of adding ghost points for the bounds obtained in Lemma ~\ref{P-m-generators}, which are also necessary when the divisors are not all contained in a hyperplanes.
...
\todo{Discussion of factoring $\phi$ through $\psi$ and $\chi$}
...
Whenever some $r_j \ge 1$, we can generate the generation at hand by one of lower degree by factoring out the section $\epsilon_j$ corresponding to the lattice point $e_j$ (see Equation \ref{eqn:epilon-def-P-m}).  Therefore, we can bound the degree of generation of $\ker(\psi)$ by
\[
	\delta_i + \sum_{i=1}^n \ell_i a_i.
\]
\end{proof}
