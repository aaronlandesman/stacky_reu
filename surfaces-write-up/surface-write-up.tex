...
\noindent
where $\alpha_i \in \bq$ and $D_i \in \di X$ is an integral codimension 1 closed subscheme of $X$. When it is convenient to do so, we shall sometimes start the indexing at $0$, so that $i$ runs from $0$ to $n$. For such a divisor $D$ with $\alpha_i = \frac{c_i}{k_i}$ let $\ell_i = \lcm_{j\ne i}(k_i)$
and let $\ell_{i,j} = \lcm_{h\ne i,j}(k_h).$ Let $
	R_D := \bigoplus_{d \geq 0} H^0(\bp^m, \lfloor dD \rfloor)$
denote the canonical ring associated to the $\bq$ divisor $D$.
...
Next, we define an ordering on the vectors, which will allow us
to present a system of relations for $R_D$. Although it is not
stated explicitly later, in the case that $D = \alpha_1 D_1$, the relations in Theorem ~\ref{thm:proj-one-point} form a reduced grobner basis with respect to this ordering.
...
The above Theorem ~\ref{thm:proj-one-point} allows us to inductively
compute presentations of effective divisors which are supported on
hyperplanes, but for $m \geq 2$, there are hypersurfaces which
...
The problem that a divisor may be contained in a hyperplane, 
...
in analogy to ~\ref[Theorem 8]{dorney:canonical}
unless we require that $D$ includes ghost points. That is, unless
$D_0,\ldots D_m$ are taken to be hyperplanes spanning $\bp^m$.
In fact, we may construct similar examples using any set of degree
...
projective space. We bound the generators in Lemma ~\ref{lem:proj-
generators}, and we use Lemmas ~\ref{lem:composite-map}, ~\ref{lem:bound-ker-chi}, and ~\ref{lem:proj-relations-psi} to bound the 
...
general and will be used in Section ~\ref{sec:hirz} to bound the 
...
Using Proposition ~\ref{prop:cone generation}, in order to bound
the degree of generation of $R_D$, we only need bound
the degrees of generators of an associated cone. We now carry out
this strategy.
...
Aaron: Ok, can you give a 1-2 sentence explanation then, just saying
which which corresponds to which?
}
