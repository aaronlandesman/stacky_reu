...
The functions $u^d \cdot \prod_{i=1}^n f_i^{c_i}$ satisfying \todo{Peter: Why do we include this $u$?  IN the later sections, I don't use this notation and we should standardize.}
\begin{align}
\label{align:pm-span}
...
hypersurface and give strong \todo{Peter: I'm opposed to the inclusion of the word strong, as it ambiguous and subjective.  I think we should either replace it with tight, and then explicitly say later how it is tight, or just get rid of the adjective.} bounds on generator and relation
degrees for general effective divisors $D$.

\begin{convention}
Let $[x_0, \ldots, x_m]$ be coordinates for $\bp^m$. Let
$\vec{v} = (v_0, \ldots, v_m) \in \bq^{m + 1}$. \todo{Peter: do we want to allow $\vec{v}\in \mathbb{Q}^{m+1}$ rather than $\mathbb{Z}^{m+1}$?  I'm not sure if the arguments actually need the generality (and if so, it is a little worrisome since fractional powers aren't clearly defined algebraically.}  Then write
\[
