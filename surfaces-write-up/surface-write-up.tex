...
Since these varieties are so complicated, it is difficult
to give explicit examples in which we can completely work out the canonical
ring of a modular surface.
Although we did not obtain a bound on the degree of generators and relations
for general rational surfaces, its enough work to get a result for the class of rational varieties we consider.
...
\bk[x_0, \ldots, x_m]$ and let each $D_i = V(f_i)$ with $f_i \in \bk[x_0, \ldots, 
x_m]$. Now suppose that $\{f_0, \ldots, f_n\}$ contains $m + 1$ independent 
linear polynomials in $x_0, \ldots, x_m$ {\rm(}with corresponding 
$\alpha_j$ possibly zero{\rm)}.
...
Then $R_D$ is generated in degrees at most
\[
	\rho = \lcm_{1\le i\le n}(k_i)\sum_{1\le i\le j\le n} \bida_i \bidb_i
\]
...
\end{restatable}
\begin{rem}
Theorem \ref{thm:hirz-generators-relations-full} provides a more precise bound.
\end{rem}
...
linear polynomials in $u, v$ and two independent linear polynomials in
$w, x$ {\rm(}with corresponding $\alpha_j$ possibly zero{\rm)}.
Define $\Te, \Tp$, and $\Tm$ as in Definition ~\ref{defn:t-defn}. Let each $D_i = V(f_i)$ where $f_i \in \sco(a_i, b_i)$.
