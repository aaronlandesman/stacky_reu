...
\begin{flalign*}
	...
	&(i < h_{i, j} < h_{i, j} + 1 < j).
\end{flalign*} \todo{Peter: Can you clarify when these two cases occur: i.e. does the first only occur if $j = h_{i,j}+1$; otherwise, in cases when $i< h_{i,j} < h_{i,j} + 1< j$, do we get relations of both forms?}
