...
\todo{todo: explain what u,v,w,x are here}
...
defined in Equation ~\eqref{eqn:e-i-proj} correspond to the
elements
\begin{equation}
\label{eqn:epsilon-def-proj}
	\epsilon_i := \psi(e_i) = u^{\ell_i a_i}\prod_{j\ne i} (f_j)^{-\alpha_j \ell_i a_i} {f_i}^{\ell_i \sum_{j\ne i} \alpha_j a_j}\in R_D.
\end{equation}
\end{rem}
...
polynomials in $u$ and $v$ and $f_3$ and $f_4$ are independent
linear polynomials in $w$ and $x$.  In order to achieve this condition on
...
surjection $k[w_1, \ldots w_r] \to R_D$.  We can factor $\phi$ through the
semigroup ring 
\[
