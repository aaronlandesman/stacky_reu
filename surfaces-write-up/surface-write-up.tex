...
Let $C_i = V(f_i)$ where $f_i \in \mathscr{P}(a_i, b_i)$; further 
...
\[
	\tau = \sigma
	+ \max \left( \max_{i\in S_=}(\ell_i \gcd(a_i, b_i)), \max_{
	\substack{i \in	S_+ \\ j\in S_-}}(\ell_{i,j} (\bida_i \bidb_j
	- \bida_j \bidb_i))	\right).
\]
\end{thm}
...
We first view the cone inside of $\mathbb{R}^{n+1}$ given by elements of the
...
Thus, the extremal rays of $S$ are given by the set of $e_i$
contained in $H$ and by the points $e_{i, j}$ that are the
intersections of $H$ with edges of the simplex connecting the
distinct $e_i, e_j$ not contained in $\in H$.
...
Let $x_1, \ldots x_r$ be the generators of $R_D$ in degree less than $\sigma$ (as given by Lemma ~\ref{lem:hirz-generators}), and let $\phi$ be the surjection $k[x_1, \ldots x_r] \to R_D$.  We can factor $\phi$ through 
\[
...
\[
	\tau = \sigma
	+ \max \left( \max_{i\in S_=}(\ell_i \gcd(a_i, b_i)), \max_{\substack{
	i \in	S_+ \\ j\in S_-}} (\ell_{i,j} (\bida_i \bidb_j - \bida_j \bidb_i))
	\right).
\]
\end{lem}
...
~\ref{eqn:hirz-relations-psi} \todo{is this the right ref?} as an element of $S$ given
...
\[
	\{e_i \mbox{ if } i \in S\} \cup \{e_{i, j}: j \in S_- \mbox{ if }
	i	\in S_+ \} \cup \{e_{j, i}: j \in S_+ \mbox{ if } i \in S_- \}.
\]
...
Let $F\in R_D$ correspond to $E + \sum_{k\in \Te} (r_k) e_k + \sum_{
\substack{h \in \Tp \\ k \in \Tm}} (r_{h,k}) e_{h,k}$ where $(r)$
 signifies the fractional part of $r$.  Similarly, let $F_\mu \in
R_D$ correspond to $E_\mu + \sum_{\substack{h \in \Tp \\ k\in \Tm}}
(r_{h,k}) e_{h,k}$. Then $F - \sum_{\mu\in M} F_\mu = 0$ gives us a
...
\[
	\deg(F) \le \left\{ \begin{split}
	\ell_i \gcd(\bida_i, \bidb_i)	\mbox{ if } i \in S_= \\
	\max_{j \in S_-}(\ell_{i, j} (\bida_i \bidb_j - \bida_j \bidb_i))
	\mbox{ if } i \in S_+ \\
	\max_{j \in S_+}(\ell_{j, i} (\bida_j \bidb_ i - \bida_i \bidb_j))
	\mbox{ if } i \in S_- \end{split}  \right.
\]
...
\[
	\tau = \sigma
	+ \max \left(\max_{i\in S_=}(\ell_i \gcd(a_i, b_i)), \max_{\substack{
	i \in S_+ \\ j \in S_-}} (\ell_{i, j} (\bida_i \bidb_j - \bida_j
	\bidb_i)) \right).
\]
\end{proof}
...
\[
	\tau = \sigma
	+ \max( \max_{i\in S_=}(\ell_i \gcd(a_i, b_i)), \max_{\substack{
	i \in S_+ \\ j \in S_-}}(\ell_{i, j} (\bida_i \bidb_j - \bida_j
	\bidb_i))).
\]
\end{thm}
