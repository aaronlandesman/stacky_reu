...
\end{align*}
Let $e_1, \ldots, e_t \in \Sigma,$ with $e_i = (\pdeg_i, c_1^i, \ldots, c_n^i)$ be a set of {\bf extremal rays} of $\Sigma$, meaning that the $\Sigma$ is contained in the $R_{\geq 0}$ span of $e_1, \ldots, e_t$.  Then, as a semigroup, $\Sigma$ is generated by elements whose first coordinate is less than $\sum_{i=1}^{n}\pdeg_i$. Furthermore, every element $\sigma \in \Sigma$ can be written in a canonical form $\lambda + \sum_{i=1}^{t} w_i e_i,$ with the first coordinate of $\lambda$ less than $\sum_{i=1}^{t}\pdeg_i$ and $w_1,\ldots, w_t \in \bz$.
...
\end{lem}
\begin{proof}
\todo{define $\bida$ and $\bidb$ Do we have to do this? In the lemma statement I said we were retaining the notation of Lemma ~\ref{lem:composite-map}}
...
\begin{lem}
Let $D = \sum_{i=1}^n \alpha_i C_i$ for curves $C_i$ on $\mathbb{P}^n$.  Let $f_i$ be homogeneous polynomials in $x_0, \ldots, x_m$ such that $C_i = v(f_i)$ for all $i$.  Then $R_D$ is generated in degree at most    and relations in degree at most  . \todo{insert numbers}
\end{lem}
