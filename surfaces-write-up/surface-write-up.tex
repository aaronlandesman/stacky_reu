...
\begin{proof}
We first claim that $\psi$ there are $\beta_1, \ldots, \beta_n$ \todo{Aaron:
Say where these polynomials live in}
...
\noindent
for any degree $d \in \mathbb{N}$ \todo{Aaron: This confuses me because $d$ is
not used above, is $d$ supposed to be $i$? Peter: Fixed?}
 and $c_i \ge -\alpha_i d$ satisfying $\bida_i + \sum_{j = 1}
^n \bida_j c_j = 0$ and $\bidb_i + \sum_{j=1}^n \bidb_j c_j = 0$.

To find such a $\beta$, \todo{Aaron: why is there no $i$ subscript? I changed
this in the $\bp^m$ case, probably copy past, mutatis mutandis}
