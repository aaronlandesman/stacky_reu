...
\todo{Aaron: weren't we going to take this section out and move it into the later sections?}
...
\\
Peter: I still don't entirely agree that explaining the dependency is important.  What I mean by this is that the proof fixes $i, \sigma,$ etc. and proves the desired result.  Proving it for each possible choice of course proves the general result; however, it doesn't seem important to me whether or not the intermediate variables are the same in the proof for $i'$ and $\sigma'$ than for $i$ and $\sigma$.  Either way the proof is valid.  This is why I have been somewhat resistant (sorry if annoyingly so...)to adding them in.

On a side note, in some cases actually writing them in would be cumbersome, e.g. replacing $s_{j,k}$ with $s_{j,k,\sigma}$ (since there is one choice of $s_{j,k}$ for every $\sigma$.  

If you want, can you tell me exactly which variables would be helpful in understanding the proof to know their dependencies?
}
...
%(using the condition on the $r_{i,j}$'s and $r_{j,i}s$) \todo{should one of these be $r_i$'s?}.  \\ Peter: No, but I'm not sure if the parenthetical is helpful anyway.  $r_i$ isn't define or used (hopefully...) anywhere.) 
and has the same degree as $E$.

Let $F = \Psi(E) \in R_D$
\todo{Aaron: Didn't we agree you would write these as $\psi(E)$ or something like that? 
\\
Peter: Okay, I'm still a little unhappy with with the identification of lattice points in $\Sigma$ with monomials (since $\Psi$ maps monomials in a polynomial ring to monomials in $R_D$), but I made the change} and $F_\mu = \psi(E_\mu) \in R_D$ for each $\mu \in M_i$.  
