...
\todo{Incorporate the following section into the $\bp^{m}$ section}
...
is generated in degrees up to $2(\sum_{i=1}^{r}d_r-1)$.
\end{lem}
\begin{proof}
...
\otimes \div \bp^m$ and $D = D' + \alpha H$, with $\alpha =
...
relations generated \todo{over $R_{D'}$?} by the following two types:
\begin{enumerate}
...
over $R_{D'}$ by leveraging the fact that $F_{i}^{\vec{v}}$ has
``maximal pole order'' at some coordinate among rational functions
in degree $i$. Then, the relations $G_{i, j}^{\vec{v}, \vec{w}}$
follow from the demonstration that the $F_{i}^{\vec{v}}$'s generate
all of $R_D$ over $R_{D'}$. We derive the relations
...
generate all of the relations by using them to reduce elements of
$R_D$ to a canonical form.

\begin{proof}
For simplicity, we give a proof for when $D' = 0$.
...
on $\bp^m$, then we look at $R_D$ where $D = D' + C$ for some
degree $\pdeg$ hypersurface $C$. Now use the Veronese mapping
to view $C$ as a hypersurface on $\bp^{\binom{{m + \pdeg}}{\pdeg} - 1}$
and the image of $C$ as the intersection of a Veronese variety
with a hyperplane. Then we can reduce the process of inductively
adding hypersurfaces to when the hypersurfaces are hyperplanes. However, this is accomplished
...
\right \}.
\end{align}
\todo{Aaron: Possibly take this definition out of the proof, and move it into the setup, since we want to reference it later.}
...
\todo{say something about ghost points?}
...
Let $\Sigma_1 \otimes_\bz \bq$ be the $\bq_{\ge 0}$ span of $e_1, \ldots e_n$.
...
$\overline{e_i e_j}$.
The extremal rays are spanned by those $e_i$'s
...
The following are elements of $R_D$ correspond to the extremal rays of
$\Sigma$.
\todo{Aaron: can we rephrase the above to say the images of the $\epsilon_i, \epsilon_{ij}$ lie on the extremal rays?}
For $i \in \Te(D)$, define $\epsilon_i \in R_D$ in degree $d_i = \ell_i \gcd(a_i, b_i)$ by 
\todo{below section needs to be simplified, maybe
by introducing new variables; 
Peter: I replaced some terms with degrees $d_i$ and $d_{i,j}$}
\begin{equation}\label{eqn:epsilon_i}
	\epsilon_i := \left(\prod_{k \ne i} f_k^{-\alpha_k d_i}\right)
	f_i^{ \frac{d_i}{\bida_i + \bidb_i}\sum_{k \ne i}
	\alpha_k (\bida_k + \bidb_k)}.
\end{equation}

\noindent
For $i \in \Tp(D)$ and $j \in \Tm(D)$, define $\epsilon_{i,j}\in R_D$ in degree $d_{i,j} = \ell_{i, j}(\bida_i \bidb_j - \bida_j \bidb_i)$ by
\begin{equation}\label{eqn:epsilon_i,j}
	\epsilon_{i, j} := \left(\prod_{k \ne i, j} f_k^{-\alpha_k d_{i,j}} \right){f_i}^{s_1} {f_j}^{s_2}
\end{equation}
...
\[
	...
	\alpha_k \bidb_k.
\]
\todo{Aaron: Why are $\epsilon_{i,j}$ defined here instead of $e_{i,j}$. We
don't use the $\epsilon$ here, and we do use the $e$ here. \\
Peter: I have a slight leaning towards this notation, because the algebraic
object is use more later (I think, although I'll check to make sure); also, I
think writing out the ordered tuple would actually be harder, because there is
no product notation, and it can be hard to denote the cordinates. 

Aaron: I did
a search for $\epsilon$ and the last time it is mentioned is inside this proof.
In fact, while the $\epsilon$ are not referred to later, the $e_{ij}$ are referred to!
The ordered tuple will look slightly longer, but it is less confusing then
switching from tuple notation to
product notation for no reason, especially when it isn't used again.
It's confusing for the reader who is thinking in terms of lattice points
to suddently switch to thinking in a ring. At the very least, you should say 
it is the preimage of the map from the lattice to the canonical ring, since the
current use of corresponding isn't ever made clear, but
this would probably then take a bunch more space too, and I would also find this significantly more confusing than just saying the lattice points.

Robin: Yeah, I thought that we had agreed upon doing the entire
proof in terms of the lattice points and then occasionally giving
the algebraic translation (and noting that it is simply the
algebraic translation) rather than this ``in-between'' sort of
thing. I agree with Aaron that it's confusing for a reader who has
not gone through the details before.}

A quick calculation deduces that these $\epsilon_i$'s and
$\epsilon_{i,j}$'s lie on the extremal rays represented by the
$e_i$'s and $e_{i,j}$'s, respectively.
\begin{rem}
...
\noindent
whenever there is some degree $d \in \mathbb{N}$
and $c_j \ge -\alpha_j d$ (for all $j$) satisfying $\bida_i + \sum_{j = 1}
^n \bida_j c_j = 0$ and $\bidb_i + \sum_{j=1}^n \bidb_j c_j = 0$.

To find such a $\beta_i$,
...
Let $\omega\in \Omega'$ correspond to the monomial $y_i \prod_{j=1}^n
{y_j}^{c_j}$.  Then we can write $\omega$ as a sum of $r_j e_j$'s for $j\in
...
\todo{Aaron: Is it at all possible to write down what $\lambda$ and the $r_{i,j}$ are?
I suspect we should be able to write them in a couple lines, which would
definitely clarify how we're getting the bound on degree $E$. While I agree
it's fine to hide the computation of the $r_{i,j}$ it would still clarify matters
to say what the $r_{i,j}$ and $\lambda$ are.}
and $r_{i,j} \in \bq$ so that we can write
...
\begin{equation}\label{eqn:hirz-E-translation}
	...
	\sum_{j\in \Tm} r_{i,j} e_{i,j}: r_{i,j}\in \bq, \sum_{j\in \Tm} r_{i,j}\kappa_{i,j}' = 1 & \mbox{ if } i\in \Tp\\
	\sum_{j\in \Tp} r_{j,i} e_{j,i}: r_{j,i} \in \bq, \sum_{j\in \Tm} r_{j,i} \kappa_{i,j}'' = 1 	& \mbox{ if } i\in \Tm\\
	\end{cases}.
\end{equation}

\noindent
From this, we can further deduce that for such an $E$,
\[
...
\]
Let $M$ be the set of monomials terms of $\beta_i$.  Then for $\mu = \prod_{j=1}^n
{f_i}^{c_i}\in M$, we
\todo{Aaron: What is $M$? \\
Peter: I don't quite follow this question... Is your objection that ``the set of monomial terms of $\beta_i$'' is too imprecise?
...
\todo{Aaron: Again, why do we keep defining lattce points in
	terms of elements in the canonical ring? Why not just
write out what the lattice points are in coordinates? \\ 
Peter: Fixed?

Aaron: I like that $E$ is defined in terms of $e_i$.
I'm still confused about $\omega$ and $\mu$ though. see my above comment.
}
can further compute that there exist $q_i, q_{i,j} \in \bq_{\geq 0}$ and $\lambda \in
...
\]
where $\lambda$ is the same as in Equation \ref{eqn:sigma-in-terms-of-lambda/E} and $E_\mu$ is defined in a similar fashion as $E$ in Equation \ref{eqn:hirz-E-translation} by
\begin{equation}\label{eqn:hirz-E_mu-translation}
E_\mu = \left(\sum_{\substack{i \in \{1, 2, 3, 4\} \cap \Te}}q_i e_i\right) +
\left(\sum_{\substack{i \in \{1, 2, 3, 4\} \cap \Tp \\ j\in \Tm}} q_{i,j}
e_{i,j}\right) + \left(\sum_{\substack{i \in \{1, 2, 3, 4\} \cap \Tm \\ j\in
\Tp}} q_{j,i} e_{i,j}\right).
\end{equation}
\todo{Aaron: if $\sigma_\mu$ is an element, how can it be written as
	the sum of an element and a set? Same question for $\sigma$ above.
	Also, I do not know what $E_\mu$ is. You really have to define it,
since there's no way to check the validity of this proof without it.  \\
Peter: $E$ and $E_\mu$ are elements, not sets...

Aaron: My mistake. Thanks for writing it out though. However,
since $E, E_\mu$ are so similar, why not defining them so that they look similar?
I.e., why not define them either both in three cases as $E$
or both as sums as $E_\mu$. It's confusing that they are the same, but not
written in a parallel fashion.
}
...
\todo{add something about ghost points?}
...
\todo{Reference}.  One might then hope to further extend these results to all minimal surfaces:
\begin{question}
