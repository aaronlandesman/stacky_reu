...
Peter: I prefer ``Generated in degrees at most'' (plural).

Aaron: What does plural mean here?
I was thinking of doing something along the lines of
$R$ is generated in degree at most $g$ with relations in degree at most $r$. \\

Peter: The plural is signifying it is not just generated in 1 degree.  It is generated in all the degrees up to $g$ with relations generated in degrees all the degrees up to $r$. \\

Robin: Agree with Peter, because it's more accurate to say that the generators in $R$ lie in degrees $\leq r$.

Aaron: Ah, I didn't see the degrees was pluralized, but now I do, and I agree. Let's adapt this convention.}
...
Peter: Better (on the first comment)? As for the second, that might be a little tricky, because we already have $E_\mu$, so we would have to do something like $E_{i,\mu}$ (which we could of course do...)

Aaron: Does $E_\mu$ depend on $i$? I thought it didn't, but if it does, then we should definitely do $E_{i,\mu}$ since I didn't
realize it depends on $i$. If $E_\mu$ doesn't depend on $i$, then we can just write $E_i$, and $E_\mu$,
and there is no danger of confusion because the subscripts live in different categories.
\\
Peter:  Yes, essentially everything in the remainder of this proof depends on $i$, e.g.: $\omega$, $\lambda$ (depends on $i$ as well as the choice of $r_{i,j}$'s), $E$, all $E_\mu$'s, $F$, $F_\mu$'s; I think that is everything.  I do not believe it would be helpful to add subscripts to all of these.  Instead maybe it would be better just to have one sentence that very clearly says everything in the following proof fixes $i$ and depends on $i$.
...
We now wish to bound the degree of generation the ideal of relations generated by these.  To do this, we first fix $i$ and a relation of the form $(f_i - \beta_i)\prod_{j=1}^n f_j^{c_j}=0$ (note that we ignore the case $i\in \{1, 2, 3, 4\}$ as in those cases all relations listed above are just the trivial relation because $\beta_i = f_i$).  Our goal is to find a relation that the divides this, but whose degree we can bound.  We do so by considering the lattice points corresponding to the monomials appearing in the relation $(f_i - \beta_i)\prod_{j=1}^n f_j^{c_j}$, and finding some $\lambda_\omega\in \Sigma$ that we can factor out of each monomial, to get a relation below a bounded degree.  The following argument makes this precise:
...
component $-\alpha_k d_{j,k} + \kappa_{j,k}''$ for $\kappa_{j,k}', \kappa_{j,k}'' \in \bq$.

We note that for $(f_i - \beta_i)\prod_{j=1}^n f_j^{c_j}=0$ to be a relation, every monomial must actually lie in $R_D$; this implies that $s_i\kappa_i\ge 1$, $\sum_{j\in \Tm} s_{i,j}\kappa_{i,j}' \ge 1$, and $\sum_{j\in \Tp} s_{i,j}\kappa_{j,i}'' \ge 1$ if $i\in \Te$, $i\in \Tp$, and $i\in \Tm$ respectively.

If $i\in \Tp$ choose $r_{i,j}\in \bq_{\ge 0}$ for all $j\in \Tm$ such that $\sum_{j\in \Tm} r_{i,j}\kappa_{i,j}' = 1$; similarly, if $i\in \Tm$ choose $r_{j,i}\in \bq_{\ge 0}$ for all $j\in \Tp$ such that $\sum_{j\in \Tm} r_{j,i}\kappa_{j,i}'' = 1$.
Define $E$ by
\begin{equation}\label{eqn:hirz-E-translation}
	E := \begin{cases}
	\frac{1}{\kappa_i} e_i  &  \mbox{ if }i\in \Te\\
	\sum_{j\in \Tm} r_{i,j} e_{i,j} & \mbox{ if } i\in \Tp\\
	\sum_{j\in \Tp} r_{j,i} e_{j,i} 	& \mbox{ if } i\in \Tm.
	\end{cases}
\end{equation}
Define
\begin{equation}\label{eqn:defn-lambda}
	\lambda = \sigma - E\in \Sigma.
 \end{equation}
Note that
\[
...
Let $M_i$ be the set of monomials terms of $\beta_i$ (recall $\beta_i$ is a polynomial in $f_1, \ldots f_4$).  Let $\mu = \prod_{j=1}^4
{f_j}^{h_j}\in M_i$ and consider the lattice point (in the same degree $d$ as $\sigma$)
\[
...
\]
A quick computation deduces that $E_\mu$ lies in $\Sigma$ (using the condition on the $r_{i,j}$'s and $r_{j,i}'s$) and has the same degree as $E$.

Let $F\in R_D$ be the monomial given by the lattice point $E$ and $F_\mu \in R_D$ be the monomial given by the lattice point $E_\mu$ for each $\mu \in M_i$.  
Then $F - \sum_{\mu \in M} F_\mu = 0$ is a relation %in degree
%\[
%	\deg(F) = \deg(E)
%\]
