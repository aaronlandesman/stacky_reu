...
\begin{equation}\label{eqn:epsilon_i}
	\epsilon_i := \left(\prod_{k \ne i} f_k^{-\alpha_k \ell_i \gcd(\bida_i, \bidb_i)}\right)
...
\begin{equation}\label{eqn:epsilon_i,j}
	...
	j - \bida_j \bidb_i)} \right){f_i}^{s_1} {f_j}^{s_2} \in H^0(\ell_{i, j}
...
A quick calculation deduces that these $\epsilon_i$'s and $\epsilon_{i,j}$'s lie on the extremal rays given by the $e_i$'s and $e_{i,j}$'s.

Thus Proposition ~\ref{prop:cone-generation} implies that $R_D$ is generated in 
...
\begin{proof}
We first claim that $\psi$ there are $\beta_1, \ldots, \beta_n\in \bk[z_1, z_2, z_3, z_4]$
...
\rfloor$ whenever $i > 4$. \todo{Add $\ell_i$ defenition}
...
Lemma ~\ref{lem:hirz-generators-relations} \todo{make sure this is the lemma
and not the theorem with the same name} in the case of $\hirz_k$ from the
...
\todo{Reference}.  One might to provide an analogous result to Lemma
~\ref{lem:hirz-generators-relations} \todo{make sure this is the lemma and not
the theorem with the same name} in the case of all minimal surfaces:
\begin{question}
