...
\\
Peter: I did this for a few of them, but I'm wondering what the purpose of this is.  It appears the same in the PDF either way right?}
...
\end{align*}\todo{Peter: Do we want this indexing from 0 or 1, since we use 0 for $\bp^m$ and $1$ for $\mathbb{F}_m$.}
...
~\eqref{align:pm-span} lie in $H^0(\mathbb{P}^m,dD)$, and all functions of $H^0(\mathbb{P}^m, dD)$ are of that form.
...
\\
Peter:  Oops, meant to write the converse statement, but accidently just restated the original statement-it's fixed now.}
...
relations $G_{i, j}^{\vec{v}, \vec{w}}$ \todo{Peter: hmm... this doesn't say much about how we get the $G_{i,j}$'s.}.
...
in a canonical form $\lambda + \sum_{i=0}^{n} r_i e_i$. Now, let $\lambda_0 \colon =
0,\lambda_1, \ldots, \lambda_m$ be all elements of $\Sigma$ which can be
written in the form $\lambda_i = \sum_{i=0}^{n}r_i e_i$ with $0 \leq r_i < 1.$ \todo{Peter: This sentence needs rephrasing.  1: misuse of $i$ as index.  2: misuse of $m$ as index ($\bp^m$).}
...
+ \deg \lambda_j \leq 2 \cdot \left( \sum_{i=0}^{n}d_i -1 \right).$
