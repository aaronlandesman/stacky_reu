...
\ssec{Effective Divisors of Hypersurfaces on Projective Space}
\label{ssec:proj-one-point}

\begin{rem}
\label{rem:proj-veronese}
For any divisor $D = \sum_{i = 0}^{n} \alpha_i C_i$ on $\bp^m$
with each $C_i$ a hypersurface, we can use the Veronese mappings
to view $D$ as a divisor on $\bp^{m'}$ for some $m' \in \bz_{\geq 0}$
with $D = \sum_{i = 0}^{n} \alpha_i H_i$ where $H_i$ are hypersurfaces
in $\bp^{m'}$.
\end{rem}
...
from Theorem ~\ref{thm:proj-one-point}.
\end{proof}
