...
for some hyperplane $\alpha H$, then we can use the proof above to show
...
= 0$.  Let $\sigma\in \Sigma'$ correspond to the monomial $y_i \prod_{j=1}^n {y_j}^{c_j}$.  Then we can write $\sigma$ as a sum of $r_j e_j$'s for $j\in \Te$ and $r_{j,k}$ for $r\in \Tp, k\in \Tm$.  Let $V$ be the set of $e_i$ $e_{i,j}$ and $e_{j,i}$ as $j$ ranges that exist (i.e. $e_i$ if the hyperplane from lemma \ref{lem:hirz-generators} intersects $e_i$, and the $e_{i,j}$'s or $e_{j,i}$'s if the hyperplane intersects the corresponding edge of the simplex).  A careful computation deduces that if $e_i$ exists
\[
	l
\]
\[
...
\]
 ...
 $= V_1 \cup V_2 \cup V_3$ where
\begin{align*}
	&	V_1 = \{ \{e_i\} \mbox{ if }
	&	V_2= \{e_{i, j}: j \in \Tm \mbox{ if } i	\in \Tp \}
	&	V_3=  \{e_{j, i}: j \in \Tp \mbox{ if } i \in \Tm \}.
\end{align*}
...
a $\mathbb{R}_{\ge 0}$-combination of elements in
\begin{align*}
