...
\\
Peter: I did this for a few of them, but I'm wondering what the
purpose of this is.  It appears the same in the PDF either way
right?

Aaron: Well, the point is to actually remove the references
numbers from equations which aren't referenced, since this
removes clutter. That is, put a star after the environment
name (change begin{equation} to begin{equation*}).
I think it makes sense not to label things
if you don't use those labels, and 
David recommended doing this in his comments.
}
...
\end{align*}\todo{Peter: Do we want this indexing from 0 or 1,
	since we use 0 for $\bp^m$ and $1$ for $\mathbb{F}_m$.

Aaron: Read the next two sentences. I think 1 is fine.}
...
\\
Peter:  Oops, meant to write the converse statement, but accidently
just restated the original statement-it's fixed now.

Aaron: Well, it should really be all such functions are linear
combinations of things in that form. However,
that's what I proved in the next couple setences. I.e.,
the remainder of the proof is showing that this set is indeed
a basis. I removed your added clause.
}
...
\begin{equation}
	...
	\sigma = \lambda + \sum_{i = 0}^{t} w_i e_i
\end{equation}

\noindent
with $w_1, \ldots, w_t \in \bz_{\geq 0}$ \todo{Peter: Replace $w$ with a different letter, since we use it later to mean something else.},
$0 \leq s_i < 1$, and $\lambda = \sum_{i = 0}^{r} s_i e_i$ 
so that the
...
extremal rays $e_0,\ldots, e_n$ in degrees $d_0, \ldots, d_n$ then $\ker \chi$
is generated in degrees at most $2(\sum_{i=0}^{n}d_i-1)$.
\end{lem}
\begin{proof}
Since $e_0, \ldots, e_n$ are extremal rays, by Proposition
~\ref{prop:cone-generation}, every element $\sigma \in \Sigma$ can be written
in a canonical form $\lambda + \sum_{i=0}^{n} r_i e_i$. Now, let
$\lambda_0 \colon =
0,\lambda_1, \ldots, \lambda_t$ be all elements of $\Sigma$ which
can be
written in the form $\lambda_i = \sum_{i=0}^{n}r_i e_i$ with $0
\leq r_i < 1.$ Then, for any $1 \leq j \leq k \leq t,$ we can write $\lambda_j + \lambda_k$ in
the above canonical form, yielding a relation in degree at most $\deg \lambda_j
+ \deg \lambda_k\leq 2 \cdot \left( \sum_{i=0}^{n}d_i -1 \right).$
...
$\sigma = \lambda + \sum_{i=0}^{n}w_i e_i$ as in Equation
