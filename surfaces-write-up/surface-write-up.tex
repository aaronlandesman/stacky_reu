...
\maketitle
\todo{Standardize ``generation in degrees at most.''  \\
Peter: Fixed, although sometimes we say ``generated in degrees less than'' which is meant to be distinct (less than rather than less then or equal to).}
...
of particular $\bq$-divisors on curves.  \todo{Peter: I think this is too much specificity about our result.  I think everything after ``stacky curves'' should be eliminated, as it seems a little self-promoting.}
...
\todo{Peter:  Hmm... this is very vague.  It almost feels like a further question type statement, then a statement of why what we're doing matters.  I guess the real issue, as we have sort of discussed, is that there are not any clear motivating reasons why one would actually use what we have done... Oh well}
...
Let $D = \sum_{i = 0}^{n} \alpha_i C_i \in \bz \otimes_\bz \di \bp^m$, with $\alpha_i =
...
Let $D = \sum_{i=0}^n \alpha_i D_i \in \bq \otimes_\bz \di \bp^m$. Let
...
Let $D = \sum_{i=1}^n \alpha_i D_i \in \bq \otimes_\bq \di \hirz_k.$ Let $D_i =
...
\noindent
Together, Theorems \ref{thm:proj-generators-relations} and \ref{thm:hirz-generators-relations} provide bounds on generators and relations for the canonical ring of $\mathbb{Q}$-divisors of all minimal rational surfaces.  Section \ref{sec:conc} concludes the paper and discusses further questions.

\todo{Aaron: Incorporate the following section into the $\bp^{m}$ section
\\
Peter: I'm not sure what this refers to...  Can you be more specific \\

Aaron: A couple of weeks ago, we agreed that we would remove the general lemmas section, and just
merge it with the $\bp^m$ section. So, I was saying that we should do this.}
...
The functions $u^d \cdot \prod_{i=1}^n f_i^{c_i}$ satisfying \todo{Peter: Why
do we include this $u$?  IN the later sections, I don't use this notation and
we should standardize.

Aaron: Isn't it necessary to distinguish the functions? For instance, we could consider $f_0$ either as a function in degree 1 or 2 for $D = \frac{1}{2}x_0$, and we need
$u^d$ to distinguish which degree it lives in.
I think we will need to go through and include the $d$ throughout.
Do you think there's a way to avoid including the $d$?
}
\begin{align}
\label{align:pm-span}
...
\end{align}\todo{Peter: Should be for $i> m+1$ right?  Looking below I see an $f_0$, where as above $f_i$ ranges 1 to $n$.  We should choose whether or not 0 is included in the indexing and fix the section to match that convention.

Aaron: I tried to fix it at $0$ everywhere. If probably missed a few places, so keep your eyes peeled.
}
...
hypersurface and give tight bounds on generator and relation
degrees for general effective divisors $D$.
\todo{Aaron: make a remark as to why these bounds are tight}
\begin{convention}
Let $[x_0, \ldots, x_m]$ be coordinates for $\bp^m$. Let
$\vec{v} = (v_0, \ldots, v_m) \in \bq^{m + 1}$. \todo{Peter: do we want to allow $\vec{v}\in \mathbb{Q}^{m+1}$ rather than $\mathbb{Z}^{m+1}$?  I'm not sure if the arguments actually need the generality (and if so, it is a little worrisome since fractional powers aren't clearly defined algebraically.

Aaron: No, these vectors should just lie in $\bz$. They're just used
to given an ordering on monomials.
}  Then write
\[
...
\begin{flalign*}
	...
	&(i < h_{i, j} < h_{i, j} + 1 < j).
\end{flalign*}
\todo{Peter: Can you clarify when these two cases occur: i.e. does the first 
only occur if $j = h_{i,j}+1$; otherwise, in cases when $i< h_{i,j} < h_{i,j} + 1< 
j$, do we get relations of both forms? \\
Robin: It purely depends on whether the product $F_i^{\vec{v}} F_j^{\vec{w}} 
$ happens to lie on the ray given by $\frac{c_{h_{i, j}}}{d_{h_{i, j}}}$ which is 
not clearly determined by $i$ or $j$ from what I can see... Unless you see 
something else. Only one relation exists for each $(i, j)$ and it is exactly one 
of the two forms. I figured that the ``either'' made that clear, no?}
...
\todo{Aaron: Has anyone actually gone through the proof
	for general effective divisors carefully, to make
sure the above claim is correct?}
...
\begin{rem}\todo{Peter: maybe move this out of a remark and just say it with the previous example?  As it stands, the previous example is a little unclear as to what its purpose is...}
...
\\
Peter: Maybe we should put this after the ensuing proofs.  That way, rather than saying the Lattice Approach, we could say that this example illustrates how the prveious method requires us to add ghost points (and is false otherwise).}
...
Let $D = \sum_{i=0}^n \alpha_i D_i$ where $\alpha_i = \frac{c_i}{k_i}$.
Then notate
\[\ell_i = \lcm_{j\ne i}(k_i).\]
\end{defn}

\begin{lem} \label{lem:proj-generators}
$R_D$ is generated in degrees at most $\sum_{i=0}^n \ell_i a_i.$
...
\mathbb{Z}, \; c_i \geq -d \alpha_i, \mbox{ and }\sum_{i=0}^{n} \bida_i c_i =
...
\label{eqn:relations-psi-proj}
	(z_i - \beta_i)\prod_{j=0}^n {z_j}^{c_{j}}
\end{equation}
...
~\eqref{eqn:Sigma-def}. Let $\Sigma_i \subseteq \mathbb{Z}^{n + 2}$
...
%proof, and should really be $e_{ij}$, as I was explaining in my previous todo.
...
\rfloor$
\todo{Aaron: Am I misremembering, or did we agree on the phone that you would put $i$ subscripts on all the things that depend on $i$?
If not, weren't you at least going to say somewhere which things depend on $i$?
\\
Peter: I still don't entirely agree that explaining the dependency is
important.  What I mean by this is that the proof fixes $i, \sigma,$ etc. and
proves the desired result.  Proving it for each possible choice of course
proves the general result; however, it doesn't seem important to me whether or
not the intermediate variables are the same in the proof for $i'$ and $\sigma'$
than for $i$ and $\sigma$.  Either way the proof is valid.  This is why I have
been somewhat resistant (sorry if annoyingly so...)to adding them in.

On a side note, in some cases actually writing them in would be cumbersome,
e.g. replacing $s_{j,k}$ with $s_{j,k,\sigma}$ (since there is one choice of
$s_{j,k}$ for every $\sigma$.  

If you want, can you tell me exactly which variables would be helpful in
understanding the proof to know their dependencies?

Aaron: Ok, I'll stop bugging you about it. I just thought we
should add it because I didn't understand that it depended
on $i$ when I read it. Maybe we can see what David
has to say about it when he reads it?
Also, I don't see why calling $s_{j,k}$ by $s_{j,k,\sigma}$ is cumbersome,
...
\todo{Aaron: Is one of these terms corresponding to $E$ and the other to $\lambda$? If so, can you say which?\\
Peter: No, one corresponds to $r_j$ and one to the fractional part of $s_j - r_j$.}
...
Let $F = \Psi(E) \in R_D$
\todo{Aaron: Didn't we agree you would write these as $\psi(E)$ or something like that? 
\\
Peter: Okay, I'm still a little unhappy with with the identification of lattice
points in $\Sigma$ with monomials (since $\Psi$ maps monomials in a polynomial
ring to monomials in $R_D$), but I made the change
Aaron: Thanks. If you're unhappy, you can always just define the map $\iota:\Sigma \rightarrow \bk[\Sigma]$ and say $F_\mu = \iota \circ \psi (E_\mu)$}
 and $F_\mu = \psi(E_\mu)
\in R_D$ for each $\mu \in M_i$.  
Then $F - \sum_{\mu \in M_i} F_\mu = 0$ is a relation %in degree
%\[
%	\deg(F) = \deg(E)
%\]
