...
Let $D = \sum_{i=1}^n \alpha_i D_i$ where $\alpha_i = \frac{c_i}{k_i}$. \todo{Peter: better notation for fraction? Aaron: This seems fine to me. Are you saying you want a better notation, or asking if this is a better notation?}  Then notate
\[\ell_i = \lcm_{j\ne i}(k_i).\]
\end{defn}
...
Then a careful computation deduces that we can write

\todo{Aaron: What is $\sigma$ here? Is it $\rho$? \\
Peter: Should have been $\omega$...}
%if $i\in \Te$, $i\in \Tp$, and $i\in \Tm$, then
\begin{equation}\label{eqn:sigma-in-terms-of-lambda/E}
	\omega = \lambda + E
\end{equation}
for some $\lambda \in \Sigma$ and $E$ of the form
...
Peter: Fixed?}
can further compute that the lattice point, $\omega_\mu = (c_1 ,\ldots c_n)$, corresponding to $\mu
$ is of the form
\[
	\omega_\mu = \lambda + E_\mu
\]
where $\lambda \in \Sigma$ is the same as in Equation \ref{eqn:sigma-in-terms-of-lambda/E} and $E_\mu$ is defined in a similar fashion as $E$ in Equation \ref{eqn:hirz-E-translation}:
\begin{equation}\label{eqn:hirz-E_mu-translation}
E_\mu = \left(\sum_{\substack{i \in \{1, 2, 3, 4\} \cap \Te}}q_i e_i\right) + \left(\sum_{\substack{i \in \{1, 2, 3, 4\} \cap \Tp \\ j\in \Tm}} q_{i,j} e_{i,j}\right) + \left(\sum_{\substack{i \in \{1, 2, 3, 4\} \cap \Tm \\ j\in \Tp}} q_{j,i} e_{i,j}\right)
\end{equation}
for some $q_i$'s, $q_{i,j}$'s and $q_{j,i}$'s (for appropriate $i$'s and $j$'s) in $\bq_{\ge 0}$.  
