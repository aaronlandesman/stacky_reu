...
~\eqref{align:pm-span} lie in $H^0(\mathbb{P}^m,dD)$, and all functions of that form lie in $H^0(\mathbb{P}^m, dD)$. To complete the proof, it
suffices to check functions satisfying conditions ~\eqref{align:pm-basis} form
a basis of $H^0(\bp^m,dD)$. Note that there $\binom{m+ \lfloor dD \rfloor }{m}$ \todo{Peter: $\lfloor dD\rfloor$ is a divisor, not a number.  Do we mean $\deg(\lfloor dD \rfloor)$?}
...
basis of degree 1 rational functions, and so all monomials in $f_0,\ldots, f_m$ of degree $\lfloor d \deg D \rfloor $ \todo{Peter: Why are we looking at monomials in this degree?  Don't we want to say monomials in degree $d$ or something?}
...
\deg \vec v = c_i \right\}.	
\end{align*}\todo{Peter: Should we say ``with $c_i$ as in section 2.1?''}
...
nonzero. Define a partial ordering on $\bz^{m+1}$ by $\vec{v} \prec \vec{w}$ if $i \leq j$.  \todo{Peter: Why do we have $i$ defined as the biggest and $j$ defined as the smallest?  Anyway, I don't believe this is a partial ordering.  Specifically, I don't believe it satisfies antisymmetry:
\\
Let $m=1$.  $\vec{v}= (1,0), \vec(w) = (2,0)$.  Then $\vec{w} \prec \vec{w}$ and $\vec{w} \prec{v}$, which by antisymmetry should imply $\vec{v} = \vec{w}$.  I'm not sure if I am missing something simple in the definition ... but this seems to contradict it being a partial order.  Maybe we don't actually care that it is a partial order though\ldots.}
\end{defn}
