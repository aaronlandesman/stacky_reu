...
\[
	2 \sum_{i=0}^n \ell_i a_i, \frac{\max_{0\le i \le n}
	(	\bida_i)}{\deg(D)} + \sum_{i=0}^n \ell_i a_i.
\]
\end{restatable}
...
$(\lambda, \mu) \cdot (u:v; z:w) \mapsto (\lambda u: \lambda v; \mu z: \lambda^{-m} \mu w),$ as described in \cite[p.~ 6]{counting-cubic}. \todo{add counting-cubic (as found in our Dropbox/stacky-REU/references folder to bibliography} Hence, one can think of
...
In order to achieve this condition on
...
Specifically, $\beta_i$,
is the polynomial obtained by expressing $f_i$ as a polynomial in $f_1, f_2, f_3, f_4$.
%$\beta_i(f_1,f_2,f_3,f_4)$ with 
%$\beta_i(f_1,f_2,f_3,f_4) = f_i$
%as elements of $\bk[\Sigma],$ which is possible as $f_1,f_2,f_3,f_4$ %generate $\Sigma,$ and hence generate $\bk[\Sigma]$. 
Furthermore, these
...
Consider a relation $u^d(f_i - \beta_i)\prod_{j=1}^n f_j^{c_j}=0$ and let it correspond to the lattice point $\sigma: = (d, c_1, \ldots, c_{i-1}, c_{i}+1, c_{i+1}, \ldots, c_n) \in \Sigma$.
...
Since $u^d (f_i - \beta_i)\prod_{j=1}^n f_j^{c_j}$ is a relation, each monomial of it must be an element of $R_D$. This implies that $s_i\kappa_i\ge 1$, $\sum_{j\in \Tm} s_{i,j}\kappa_{i,j}' \ge 1$, and $\sum_{j\in \Tp} s_{i,j}\kappa_{j,i}'' \ge 1$ if $i\in \Te$, $i\in \Tp$, and $i\in \Tm$ respectively.
...
%	\deg(F) = \deg(E)
%\]
which divides $u^d(z_i - \beta_i) \prod_{j = 1}^n z_j^{c_j}$. 
...
eisenbud--harris here?}. By Theorems
