...
{{m + i} \choose {i}}$ generators $\frac{u^{d_i} x^{\vec{v}}}{x_k^{c_i}}$
where $\vec{v} \in \bz_{\geq 0}^{m + 1}$ such that $\sum_{j = 0}^{m}
v_j = c_i$ and $z$ \todo{how many?} relations of the form
...
\begin{proof}
We know $u$ is a minimal generator in degree $1$. If $x^{\vec{v}}$
