...
then $\bk[f_0, \ldots, f_m]$ has transcendence degree $m+1$ over $\bk$.
...
Peter: I like having the statement of the lemma being nice and consise; also the statement has nothing to do with this geometric picture, so I find it more natural to have the geometric side contained in the proof.  I also don't quite see why it is so bad to reference something from inside a previous proof; a reader can surely locate it...
}
...
\todo{say something about ghost points? Peter: Haven't we already done that at the begining of the section?}
...
polynomials in $x_0$ and $x_1$ and $f_3$ and $f_4$ are independent
linear polynomials in $y_0$ and $y_1$.  In order to achieve this condition on $D_1, \ldots D_4$, it may be necessary to add in ``Ghost Divisors'' (i.e. divisors with of the desired form with a coefficient $\alpha_i = 0$).
\begin{defn}
Define 
\begin{equation}
...
From this we can deduce the representatives of $\Sigma$ lying on its extremal rays given by the following:
For $i \in \Te(D)$, define $e_i \in R_D$ in degree $d_i = \ell_i \gcd(a_i, b_i)$ by 
\begin{equation}\label{eqn:epsilon_i}
	e_i := (-\alpha_1 d_i, \ldots, - \alpha_{i-1} d_i, \frac{d_i}{\bida_i + \bidb_i}\sum_{k \ne i}
	\alpha_k (\bida_k + \bidb_k), - \alpha_{i+1} d_i, \ldots, - \alpha_n d_i)
\end{equation}

\noindent
For $i \in \Tp(D)$ and $j \in \Tm(D)$, define $e_{i,j}\in R_D$ in degree $d_{i,j} = \ell_{i, j}(\bida_i \bidb_j - \bida_j \bidb_i)$ by
\begin{equation}\label{eqn:epsilon_i,j}
	\begin{split}& e_{i,j} := (-\alpha_1 d_{i,j}, \ldots -\alpha_{i-1} d_{i,j}, \bidb_j \sum_{k \ne i,j} \alpha_k \bida_k - \bida_j \sum_{k \ne i, j}
	...
	\end{split}
\end{equation}
Note that the above expression is for when $i < j$, but a small rearangement gives when $j < i$.  \todo{Peter: I changed the above expression, but I'm still feel a little dubious that this is easier to read...}
...
A quick calculation deduces that these $e_i$'s and
$e_i{i,j}$'s lie on the extremal rays of $\Sigma$.
%\begin{rem}
...
\\
Peter: There is not a unique choice of the $r_{i,j}$'s.  When we write out $\sigma$ in terms of the $e_i$'s and $e_{i,j}'s$ with coefficients $C_i$ and $C_{i,j}$, any choice of $r_{i,j}$'s that are at most equal to the coefficient $C_{i,j}$ and such that $\sum_{j\in T_-} r_{i,j}\kappa'_{i,j}=1$ will work.}
...
\\
Peter: Well... We had a conversation a while ago that saying something like $j \in \{i\} \cap T_=$ might look a little funny (saying $j$ ranging over the set of i intersect $T_=$ meaning $j$ either ranges over the emptyset or the singleton set of $i$).  I We could make this change in the first one if it is preferable.  On the other hand, transforming the second into the form of the first would be terribly ugly (like 12 cases, some of which are sumed together... or something like that...).
}
