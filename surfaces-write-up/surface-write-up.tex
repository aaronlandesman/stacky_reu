...
\noindent
where $\vec{y}$ and $\vec{w}$ are the unique
...
$\vec{x}_i := (d_i, c_i) \in \bz^2$. Let $(\beta, \gamma) = \ell_1
(d_i, c_i) + \ell_2 (d_{i + 1}, c_{i + 1})$ for $\ell_1, \ell_2 \in
...
\begin{align*}
	...
	{x_k^{c_i}}\right)^{\ell_1} \left(\frac{u^{d_{i + 1}}}
	{x_k^{c_{i + 1}}}\right)^{\ell_2} x^{\vec{v}}.
\end{align*}
...
\begin{align*}
	\frac{u^{\beta} x^{\vec{v}}} {x_k^{\gamma}}	= \prod_{j = 1}
	^{\ell_1} \frac{u^{d_i} x^{\vec{w}_{(j)}}} {x_k^{c_i}}
	\prod_{k = 1}^{\ell_2} \frac{u^{d_{i + 1}} x^{\vec{z}_{(k)}}}
	{x_k^{c_{i + 1}}}
\end{align*}
into products of generators $F_i^{\vec{w}_{(j)}}$
and $F_{d_{i + 1}}^{\vec{z}_{(k)}}$ which are in the form
...
This example easily generalizes to the following situation: Suppose $D = \sum_{i=1}^{n} \frac{d}{s}$
\end{example}
