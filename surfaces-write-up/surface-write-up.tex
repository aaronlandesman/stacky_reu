...
with each $C_i$ a hypersurface and $\alpha_i \in \br$, we can use
the Veronese mappings to view $D$ as a divisor on $\bp^{m'}$ for some $m' \in \bz_{\geq 0}$
with $D = \sum_{i = 0}^{n} \alpha_i H_i$ where $H_i$ are 
hypersurfaces
in $\bp^{m'}$.
\end{rem}
