...
O'Dorney \cite{dorney:canonical} who gives similar descriptions
of canonical rings for general $\bq$-divisors on $\bp^1$. This
study also has connections to the papers by Voight--Zureick-Brown 
\cite{vzb:stacky} and Landesman--Ruhm--Zhang \cite{lrz:spin-cring} on log 
spin canonical rings of particular $\bq$ divisors on stacky curves which 
provide bounds on rings of modular forms of Fuchsian groups.

The work in this paper may help calculate a presentation of certain rings of Hilbert 
...
$\bq$-divisors. \todo{Add references}

\todo{Aaron: This paragraph was already stated above. Can we remove it?}
This work extends the study of canonical rings of
...
\todo{more! Aaron: I think this is plenty of introduction}

\ssec{Main Results}

\todo{put main results! use restate, maybe?}
\todo{Aaron: Some of these theorem statements have been copied from older versions of the statements
and not updated to reflect the difference. I agree we should use Theorem-restate, but we should make
sure to copy the ones stated below, and not copy these ones}
\begin{thm}
...
\begin{thm}
\label{thm:proj-generators-relations-intro}
...
\begin{thm}
\label{thm:hirz-generators-relations-intro}
...
\todo{Incorporate the following section into the pm section}
...
Let $D = \sum_{i=1}^n \alpha_i D_i$ where $\alpha_i = \frac{c_i}{k_i}$. \todo{Peter: better notation for fraction?}  Then notate
\[\ell_i = \lcm_{j\ne i}(k_i).\]
\end{defn}
...
We can talk about sections of $\hirz_m$ in terms of polynomials in $u,v, w,x$; furthermore we can talk about bi-degrees of a monomial $u^a v^{b} w^c x^d$ given by $(a + b + mc, c + d)$.  Sections on open subsets of $\hirz_m$ look like polynomials with each monomial having the same bidegrees, and rational sections have both bidegrees 0. \todo{Peter @ Aaron: can you add your exact sequence argument for why this is true?}

Why can write $\bq$-divisors of $\hirz_m$ as $D = \sum_{i = 1}^n \alpha_i D_i \in \mathbb{Q} \otimes \di (\hirz_m)$, where each $D_i = V(f_i)$ for some polynomial $f_i$, with each monomial having the same bidegrees.  Since sections on $H^0(dD)$ must be rational sections, they must have both bidegrees non-negative, meaning that both bidegrees of $D$ must be non-negative for $R_D$ to be non-trivial.

For the remainder of this section we will assume $D_1, D_2, D_3,$ and $D_4$ are distinct divisors with bi-degrees $(1,0)$
, $(1,0)$, $(0,1)$, and $(0,1)$ respectively. Let $D_i = V(f_i)$ where
$f_i \in \sco(\bida_i, \bidb_i)$.  
Then, $f_1$ and $f_2$ are independent linear
polynomials in $x_0$ and $x_1$ and $f_3$ and $f_4$ are independent
linear polynomials in $y_0$ and $y_1$. \todo{Talk a little about Hirzebruch
surfaces before this paragraph; especially, describe what it's sections look
like. \\
Peter: Fixed?
}
...
and $\sum_{i=1}^n c_i \bidb_i = 0$. We can view $g$ inside the
lattice 
\begin{align*}
...
Let $S$ be the $\mathbb{R}_{\ge 0}$ span of $e_1, \ldots e_n$.
We can intersect $S$ with the hyperplane $H$ given by
$\sum_{i=1}^n \bida_i X_i = 0$  to get a subspace $S' \subseteq S$.  Then, as long as each extremal rays of $S'$ contains an element of the lattice $\Sigma$, those representatives must in fact the extremal rays of $\Sigma'$.

The extremal rays of $S'$ can be represented by points lying only on the edges of 
$\overline{e_i e_j}$
...
\todo{more detail above;  Peter: resolved?}
The following are elements of $R_D$ correspond to lattice point on each of the extremal rays listed above:
...
Peter: I have a slight leaning towards this notation, because the algebraic object is use more later (I think, although I'll check to make sure); also, I think writing out the ordered tuple would actually be harder, because there is no product notation, and it can be hard to denote the cordinates.}
A quick calculation deduces that these $\epsilon_i$'s and $\epsilon_{i,j}$'s lie on the extremal rays represented by the $e_i$'s and $e_{i,j}$'s, respectively.
\begin{rem}
...
\end{rem}\todo{Peter: I don't like having a remark inside of a proof.  Also, it feels like this could reasonably be replaced with a better constructed/organized proof, so that $e_i$'s are just defined this way initially; let me know if you see a good way to do so...  As for Aaron's comment above, we could do something like that, except we already defined $e_i$'s to be something different, which might be equally/more confusing.}

Thus,
Proposition ~\ref{prop:cone-generation} implies that 
$R_D$ is generated in 
...
\]
\todo{Aaron: Do you use $\sigma$ later? We are usually reserving $\sigma$ for a
lattice point. Perhaps something like $\delta$ would be more appropriate if you
want to refer to it later. Also, you should number if if you want to refer to
it later. In general, it's bad style to define things in the proofs. This could
have been defined prior to the theorem statement as well, which would simplify
the statement. We should also then clarify that $\sigma$ depends on $D$.  \\
Peter: $\sigma$ replaced by $\rho$. It's also defined now in the lemma statement rather than its proof.  
}
...
component $-\alpha_j d_j + \kappa_j$ for some $\kappa_j \in \br$ and let $e_{j,k}$ (when it exists) lie in
degrees $d_{j,k}$ with $j^{th}$ component $-\alpha_j d_{j,k} + \kappa_{j,k}'$ and $k^{th}$
component $-\alpha_j d_{j,k} + \kappa_{j,k}''$ for $\kappa_{j,k}', \kappa_{j,k}'' \in \br$
...
\]
Let $M$ be the set of monomials terms of $\beta_i$.  Then for $\mu\in M$, we
can further compute that the lattice point, $\sigma_\mu$, corresponding to $\mu \prod_{j=1}^n
{f_i}^{c_i}$ is of the form
\[
	\sigma_\mu = \lambda + E_\mu
\]
where $\lambda \in \Sigma$ is the same as in Equation \ref{eqn:sigma-in-terms-of-lambda/E} and $E_\mu$ is defined in a similar fashion as $E$ in Equation \ref{eqn:hirz-E-translation} (in particular, they lie in the same degree).
...
\end{thm}
\begin{proof}
The generation degree bound is as stated in Lemma \ref{lem:hirz-generators}.  By Proposition \ref{lem:composite-map}, the degree of generation of $\ker(\phi)$ is at most the maximum of the generation degrees of $\ker(\chi)$ and $\ker(\psi)$, giving us the desired relations bound.
\end{proof}

\todo{Aaron: Shouldn't $\tau$ not include $\sigma$?  Peter: Fixed}
