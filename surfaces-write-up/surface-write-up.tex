...
\[
	\sigma = \sum_{i\in \Te(D)} \gcd(\bida_i, \bidb_i)\ell_i + \sum_{i
	\in \Tp(D) \atop
...
\[
	\tau = \sigma
	+ \max \left( \max_{i\in S_=}(\ell_i \gcd(a_i, b_i)), \max_{i \in
	S_+ \atop j\in S_-}(\ell_{i,j} (\bida_i \bidb_j - \bida_j \bidb_i))
	\right).
\]
\end{thm}
