...
\begin{enumerate}
	\item \todo{Ask David how the new version sounds} We explained
		...
		an explicit example.
	\item ``For example, why give in Section 1.1 the main results in a form
...
since the referee wants this; the cleaner alternative (since the main results
should be stated in a clear-to-understand format without excessive notation)
would be to rephrase the theorems for the main results section without using
explicity notation}
	...
	\item \todo{Make Thm. 1.4 readable by including necessary notations
		before it}
	\item \todo{Section 2 starts with definitions that should have come earlier. (Was Section
1.1 added in a later stage ?)}
	\item The reference to later items given on page 5, line 7 of the previous version
	has been moved to Remark (rem:proj-grobner).
...
well-known.
\item The references for the O'Dorney and Landesman--Ruhm--Zhang papers have been updated. The citation format of the Bruinier and Van Der Geer references have been updated to reflect their status as lecture notes.
