...
\begin{enumerate}
	\item We changed the name ``canonical ring'' to ``generalized canonical ring,'' as the referee suggested,
		since the canonical ring is really
		$R_K$ (i.e., it is the generalized canonical ring $R_D$ as we have defined it in the case $D = K$).
	\item We explained
	...
	\item Theorem 1.4 (now \autoref{thm:hirz-generators-relations}) has been rewritten into a simpler form by reducing some
		notation unnecessary to understanding the general idea. We then state it more precisely as \autoref{thm:hirz-generators-relations-full}. The statement of \autoref{thm:hirz-generators-relations-full} has also been simplified, since we noticed the previously defined quantity $\tau$ was unnecessary.
