...
\title{Generalized canonical rings of $\bq$-divisors on minimal rational surfaces}
...
\begin{abstract}
	...
	on the degree of generators and relations of the generalized
	canonical ring of 
	any effective $\bq$-divisor on projective space.
\end{abstract}
...
\textbf{generalized canonical ring} is $R(X, D) := \bigoplus_{d \geq 0} u^d
H^0(X, \lfloor dD \rfloor)$, where $u$ is a dummy variable to keep
track of degree.
We study the generators and relations of generalized canonical
...
generalized canonical ring when $X = \bp^m$ and $D$ is any effective
$\bq$-divisor.  

In part, this work is motivated by a recent paper by
O'Dorney \cite{dorney:canonical}, which gives similar descriptions
of generalized canonical rings for general $\bq$-divisors on $\bp^1$. This
...
this would allow us to translate rings of Hilbert modular forms
...
Thereby, if our results could be extended to all rational surfaces,
they would give a bound on generators and relations for such
...
immensely complicated: ``Already for $g = 2$, the boundary of this fundamental domain is complicated; Gottschling found that it possesses $28$ boundary pieces'' \cite[p.\ 7]{geer:siegel-modular}.
...
Use $m \in \bz$ to index the dimension of a given projective
...
denote the generalized canonical ring associated to the $\bq$-divisor $D$. 
So, $R_D$ is notation for $R(X,D)$ when $X$ is understood from context.
If $R$
...
Since generator and relation degrees are preserved under base change to the algebraic closure, assuming that $\bk$ is algebraically closed is no restriction
...
generalized canonical rings on $\bp^m$. The first applies
...
Then the generalized canonical ring $R_D$
...
\begin{rem}
\label{rem:proj-tight-bounds-eff}
...
Note that the bounds given in Theorem 
...
\sco_{\bp^1}(m))$ as a projective bundle over $\bp^1$.
...
and relations for the generalized canonical ring of any $\bq$-divisor on any
...
generalized canonical ring is concentrated in degree 0, and if
$\deg D = 0$, then the generalized canonical ring has a single
...
generators of the generalized canonical ring $R_D$ when $D$ is an effective divisor in Theorem ~\ref{thm:proj-generators-relations}
...
generalized canonical rings on projective space, we first
...
\begin{enumerate}[resume]
	\item $c_i = -\lfloor d\alpha_i \rfloor \text{ for } i > m$
\end{enumerate}
...
that those functions satisfying conditions (1-3) are
...
fractional divisors $D \in \bq \otimes_\bz \di \bp^m$. We give an explicit presentation of the generalized canonical ring when $D$ is an effective
divisor.
...
$\alpha$. Then, the generalized canonical ring
\[
	R_D := \bigoplus_{d \geq 0} u^d H^0(\bp^m, \lfloor dD \rfloor)
\]
...
Hirzebruch-Jung continued fraction of $\alpha$. Then, since the
$F_{i}^{\vec{v}}$ generate
...
generalized canonical ring of a general $\bq$-divisor supported on two
...
are not unions of hyperplanes. We now address this general case, giving an inductive presentation of generalized canonical rings of effective divisors, and a tight bound on the degrees of their generators and
relations.
...
the generalized canonical ring $R_H$ of the hyperplane $H \subset \bp^{\binom{m+\pdeg}{\pdeg}-1}$
...
$6$.
However, all lower degrees only have
...
the generator and relation degrees of the generalized canonical ring of any
...
generators and relations of the generalized canonical ring of a $\bq$-divisor
...
then the generalized canonical ring is concentrated in degree 0. If one of the
...
of arbitrary generalized canonical rings on any minimal rational surface. A
...
generalized canonical ring of a divisor on a blow-up of a given surface
in terms of the generalized canonical
rings of some associated divisors that given surface.

Another direction to generalize the work in this paper
would be to try to express generalized canonical rings of $\bq$-divisors on
$X \times Y$ in terms of those on $X$ and $Y$. In this paper,
we bounded the degrees of presentations on generalized canonical rings on $\bp^1 \times \bp^1 \cong \hirz_0$.
Perhaps similar techniques can be used to bound
degrees of presentations on generalized canonical rings on $(\bp^1)^k$ or
