...
\author{Robin Zhang}
\address[Robin Zhang]{Department of Mathematics, Columbia University}
\email{rzhang@math.columbia.edu}
...
Then $R_D$ is generated in degrees at most 
$\omega' := \ell(m+1) + \sum_{i = 0}^n \ell_i a_i$
with relations generated in degrees at most
\[
	\max \left(2 \omega', \frac{\max_{0 \le i \le n}
	(	\bida_i)}{\deg(D)} + \omega' \right).
...
Throughout, for ease of notation,
we work over a fixed algebraically closed field $\bk$.
Our results hold equally well over arbitrary fields, as we now explain.
...
We note that if $L/\bk$ is an inseparable extension,
and $X$ is a scheme over $\bk$ than the base change of the canonical divisor
$(K_X)_L$ may be different then the canonical divisor of the base change
$K_{X_L}$.
Therefore, the canonical ring may not be preserved under base change
along inseparable extensions.
...
We next define some notation used throughout the paper.
...
$R_D := \bigoplus_{d \geq 0} H^0(X, \lfloor dD \rfloor)$
denote the section ring associated to the $\bq$-divisor $D$. 
We often alternatively write
$R_D := \bigoplus_{d \geq 0} u^d H^0(X, \lfloor dD \rfloor)$, where $u$ is a dummy
...
\begin{example}
	\label{example:effective-example}
