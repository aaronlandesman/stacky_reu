...
In order to show this is an isomorphism, one can use Riemann--Roch and pole order considerations at $P_1$ to check the above relation exists. One can then check that the generators and relation determine a ring $R$ so that for all $j \in \BZ,$ the $j$th graded component $(R)_j$ satisfies $\dim_k (R)_j = \dim_k R_\halfcan.$ Hence, $R \cong R_\halfcan$. \todo{Ask David why he suggests $\proj R \cong X$? It seems like this is not enough to know it is the half canonical ring if there aren't enough generators.} The verification is analogous to Example ~\ref{eg:base-0-377} and is omitted.
...
{| c || c | c | c |}	
	\hline
	$\halfcan'$ & Generator Degrees & Degrees of Minimal Relations & $e$ \\ [2pt]
	\hline
	\hline
...
\todo{make tables slightly taller}
