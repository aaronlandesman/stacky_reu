...
directions. 
If $X$ is a curve, \todo{why do you start with a curve and then move to a stacky curve? shouldn't it be the opposite order?} then we say $\sx$ is a {\bf stacky curve} with {\bf coarse space} $X$ and {\bf stacky points} (also called {\bf{fractional points}}) $P_1,\ldots, P_r$ with {\bf stabilizer orders} $e_1,\ldots, e_r \in \BZ_{\geq 2}$ if 
\[
