...
\todo{Peter ``the study of'' is used twice in the first sentence.  Can we find a way to replace one of them?}

\todo{Peter: I don't like the use of ``In particular, this work..."  Can we say something like ``This paper ..."}

\todo{Peter: I don't understand the purpose of the parenthetical ``(or effective).''  What is it trying to say?}

\todo{Peter: I don't like the use of the word ``strongly'' in the third sentence; these sort of adjectives are ambiguous and subjective, and don't add any real content to the statement}

\todo{Peter: Get rid of comment about fractional weight modular forms.  This does not appear anywhere except in further questions, and does not deserve a spot in the abstract.}
...
Let $(X, \Delta, L)$ be a log spin curve of signature $(g;-;\delta)$ with $g \geq 2$. Then $X$ has minimal generators in degree at most 5 and minimal relations in degree at most 10.
\end{cor}
\begin{proof}
If $\Delta =0$, the result is immediate from Reid \todo{cite Reid}. Otherwise, if $\Delta > 0$, the bound on the degrees of minimal generators follows from Lemma ~\ref{lem:generation-5}, while the bound on the degrees of minimal relations follows from Proposition ~\ref{prop:relation-10}.
\end{proof}
...
We are ready to prove our main theorem ~\ref{thm:main} in the case $g \geq 2$. The idea of the proof is to use Lemma ~\ref{lem:generation-5} and Proposition ~\ref{prop:relation-10} to complete the base case when $L = L_X$ and then apply Lemmas ~\ref{lem:sat-1}, ~\ref{lem:sat-2}, and ~\ref{lem:raise-stacky-order} to complete the induction step.
...
%
	\item In Subsection \ref{ssec:bounds-high-genus}, we reference the work of Reid \cite{reid:infinitesimal}.
We use his construction of a uniform bound on the generation of the half-canonical ring and its ideal of relations at 5 and 10 in the non-log, non-stacky case when genus is at least 2.  We extend this bound of 5 and 10 to the log case, and then apply our inductive lemmas to add stacky points and get bounds of $\max(5,e)$ and $\max(10, 2e)$.  Reid in fact proves something slightly stronger: that in most cases his bound is actually 3 and 6 with well-characterized exceptions.
\begin{question}\label{ques:reduce-5,10-to-4,8}
Can we reduce the bounds in Theorem \ref{thm:main} on generation from $\max(5,e)$ and $\max(10, 2e)$ to $\max(4,e)$ and $\max(8, 2e)$. 
\end{question}
\begin{rem}\label{rem:reduce-3,6}
It is unlikely that this can be reduced further, because Lemma \ref{lem:sat-2}, which allows us to add a stacky point in genus $g>0$ saturated at 2, requires a generator in degree 4 and potentially a relation in degree 8, even when inductively adding on stacky points of stabilizer order 3.
\end{rem}
%
...
environment for mathematical learning and collaboration. We thank
Jorge Sentieiro Neves help pointing out Reid's bound on the maximal
degrees of minimal generators and relations of spin canonical rings
in high genera. Finally, we gratefully acknowledge that our
research was financially supported by NSF Grant Award Number 1250467
via the Emory University Number Theory REU. We deeply appreciate
all of the support that has made our work possible
