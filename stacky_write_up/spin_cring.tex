...
The main result of this subsection is that for a log spin curve with no stacky points $(X,\Delta,L)$, the spin canonical ring $R_L$ is generated in degree at most $5$, with relations in degree at most $10$. The case that $\Delta = 0$ was completed in Reid \todo{cite Ried article}. The generation bound is shown in Lemma ~\ref{lem:generation-5} and the relations bound is shown in Lemma ~\ref{prop:relation-10}. We summarize the result in Corollary ~\ref{cor:g-2-presentation-bound}

The proofs of these results are similar to those in Neves ~\cite[Proposition III.4 and Proposition III.12]{neves:halfcan}, but slightly generalized as we assume $\Delta > 0$ instead of $\Delta = 0$ and do not assume there is a basepoint free pencil contained in $H^0(X,L).$ 
...
\end{align*}
is surjective if $n \geq 5.$ In particular, $R_L$ is generated in degree at most $5.$
\end{lem}
...
The next step is to bound the degrees of the relations of $R_L$. This is done in Proposition ~\ref{prop:relation-10}. The idea of the proof of Proposition ~\ref{prop:relation-10} is to use the basepoint-free pencil trick to show that if a relation lies in a sufficiently high degree, it lies in the ideal generated by the relations in lower degrees. We now fix notation for this ideal generated by lower degrees relations.
