...
\todo{incorporate some chunk on O'Dorney somewhere in intro, below is previously written stuff}
In \cite{dorney:canonical}, O'Dorney uses best lower approximations to provide a minimal presentation for the canonical ring of a (not-necessarily effective) \todo{Can we remove this parenthetical? I don't see why the reader would assume effective.} divisor $D=\frac{\alpha_1}{\beta_1}P_1+\frac{\alpha_2}{\beta_2}P_2$ of $\BP_k^1$ \todo{This comes up a lot, but instead of saying $D$ is a divisor of $X$, I'd prefer to say $D \in \di X$ to be more precise. Of course, this is minor, and it's fine to keep it this way.} consisting of at most 2 points in its support, \todo{It might be clearer to say $\# \Supp(D) \leq 2.$} to show it is generated in degree $e=\max{\beta_1,\beta_2}$ with relations generated in degrees at most $2e$. \todo{Can you break the previous sentence into two sentences?} He \todo{Probably better not to say he, maybe give a theorem number where he does this?} further extends the result to more points, but only bounds the degree of minimal generation by a value on the order the least common multiple of the denominators of fractional points. \todo{This sentence seems very imprecise to me, and I wouldn't understand it if I hadn't read Evan's paper. Can you state this more precisely, or possibly just remove it.}

In Lemma ~\ref{lem:sat-1} we provide an inductive method to strengthen
O'Dorney's result \cite[Theorem 8]{dorney:canonical} on $\BP^1_k$ to divisors $D = \sum_{i=1}^n \frac{\alpha_i
...
degrees at most $2e$. \todo{Are you sure you're not assuming $\frac{\alpha}{\beta}$ is effective? If not, doesn't what you say contradict Evan's example showing that his bounds of about lcm are asymptotically sharp?} Furthermore, given additional constraints on $H^0(
X, \lfloor dD \rfloor)$ for various values of $d$, we can apply the
same Riemann-Roch \todo{same as what? Maybe just say we can generalize his results to arbitrary genus?} arguments in arbitrary genus. 
To do this, we extend an inductive result \todo{Is ``inductive result'' a phrase? Can we just remove the word ``inductive'' every time it appears?} by Voight and
Zureick-Brown \cite[Theorem 8.3.1]{vzb:stacky} on stacky canonical rings to arbitrary $\BQ$-
divisors. \todo{Again, I don't think it can be arbitrary, and what are the $\BQ$ divisors on?}
...
$\Delta = \sum_{i=1}^{n}P_i \in \di X,$ with $P_i$ distinct points of $X$. \todo{Do we want to assume they are distinct?} Let
...
\label{sec:induction}
First we present several lemmata which play a crucial role in the
inductive step of the proof of the main theorem, Theorem
~\ref{thm:main}. In Subsection ~\ref{ssec:add-points} we prove
Lemma ~\ref {lem:sat-1}, ~\ref{lem:sat-2}, and ~\ref{lem:sat-3}
which describe the generators and relations of $R_{D + \frac{\alpha
}{\beta}P},$ given those in $R_D,$ where $X$ is a curve, $D \in \di
X \otimes \BQ $ and  $\frac{\alpha}{\beta} \in \BQ.$ Next, in
Subsection ~\ref{ssec:raise-orders}, we prove Lemma
~\ref{lem:raise-stacky-order}, which the generators and relations 
of the log spin canonical ring curve with signature $(g; e_1', 
\ldots, e_\tau'; 0)$ to that of the log spin canonical ring of a 
curve with a certain subset of the $e_ i'$ incremented by $2$. 
While Lemma ~\ref{lem:sat-1} may be applied in a fairly general 
context, Lemma ~\ref{lem:raise-stacky-order} is more specific to 
half-canonical divisors, although even Lemma
~\ref{lem:raise-stacky-order} has certain generalizations, as noted
in Remark ~\ref{rem:raise-fractional-order}
...
First, we introduce some notation from O'Dorney.  \cite{dorney:canonical}
\begin{defn}\label{den:lower-approximation}
If $\frac{\alpha}{\beta}\in \BQ$ (for $\alpha, \beta \in \mathbb{Z}, \beta> 0$ written in reduced form), then a \begin{bf}{best lower approximation}\end{bf} to $\frac{\alpha}{\beta}$ is a a fraction $\frac{c}{d}$ (written in reduced form with $d>0$) such that there is no $\frac{c'}{d'}$ with $0<d'<d$ and $\frac{c}{d}\le \frac{c'}{d'}$.  Moreover, the non-negative best lower approximations of $\frac{\alpha}{\beta}$ form a sequence
\[
	0=\frac{c_0}{d_0} < \frac{c_1}{d_1} < \ldots < \frac{c_r}{d_r} = \frac{\alpha}{\beta}.
\]
\todo{This Moreover statement shouldn't really be part of the definition.}
\end{defn}
\todo{add a picture of lattice}

Previously discussed O'Dorney's result and our improvement when restricting to effective. Moved to Intro.
\todo{new transition}

First, however, we give a criteria to determine if a set of monomials generates the initial ideal of relations, and define notation for a type of monomial ordering. \todo{Perhaps say which parts of the proofs of which future lemmas it will be used in?}

\todo{Somehow, I feel like most of this page can be cut, it seems to mostly vaguely describe what we are doing, and takes up a lot of space.}
...
$R_{\halfcan'}$ with ideals of relations $I'$ and $I$ respectively \todo{Usually respectively is applied when two things are said at the same time, this doesn't seem to fit the usual gramattical pattern}.  
...
\begin{enumerate}
\item[(\ref{lem:relations_from_generators_induction}.1)] \todo{It would probably be clearer to just call these conditions 1,2,3 instead of 3.2.1...}  $T\prec S\prec k[x_1, \ldots, x_n]$ (meaning all monomials in $T$ precede all monomials in $S$ which proceed all monomials in $k[x_1, \ldots, x_n]$)
...
\end{align*}
\todo{Aaron: I don't understand why this holds}
...
\end{align*}
\todo{Aaron: Why can't the initial term lie in S?}
\todo{It might be helpful to say where you use conditions 1 2 and 3.}
\end{proof}

As suggested by the previous lemma, \todo{Reference the lemma, instead of using previous, since things might get moved around} we will need an ordering on monomials, and the following definition will prove useful in the ensuing lemma. \todo{Again, reference this}
\begin{defn}
We equip the graded ring $k[x_1, \ldots x_m]$, with weight $a_i$ and pole order $-\ord_P(x_i)$ attached to $x_i$, \todo{attached to $x_i$ doesn't really sound like the correct terminology to me} the graded $P$ reverse lexicographic order {\bf{g-P-revlex}} \todo{This name seems a bit funny looking to me} if for $f=\prod_{i=1}^m {x_i}^{q_i}\ne g=\prod_{i=1}^m {x_i}^{r_i}$, $f\prec g$ iff \todo{It's better not to use abbreviations like iff in papers. Also, usually in definitions, one uses if in place of iff.} one of the following holds
\begin{enumerate}
\item $\deg(f) < \deg(g)$
\item $\deg(f) = \deg(g)$ and $-\ord(f) > -\ord(g)$ \todo{Order at which point, in this line and the next}
\item $\deg(f) = \deg(g)$, $-\ord(f) = -\ord(g)$, and for $i$ maximal such that $q_i-r_i$ is non-zero, that expression is negative. \todo{I do not understand what this means, maximal under what ordering? Also, what expression are you talking about?}
\end{enumerate}
\end{defn}
We leave it as an exercise to verify that this is an order.  \todo{check for ourselves that this actually is, Aaron: I suggest not to use the phrase ``leave as an exercise'' in a paper.}
We now offer \todo{Maybe state instead of offer?} a lemma that allows us to inductively add $\mathbb{Q}$-points \todo{$\BQ$ points usually refers to maps to $\spec \BQ$. Maybe we can just say a rational multiple of a poitn?} to a divisor and describe the new generators and relations.
...
satisfying $h^0(X, \lfloor{\halfcan'}\rfloor)\ge 1$. Suppose $P$ is not a base-point of $d\halfcan'$ for all $d\in \mathbb{N}^+$ meaning that $R_{\halfcan'}$ has generators (in the necessary degrees \todo{What does this mean?})
...
\]
and equip $k[y_1, \ldots, y_n]$ with g-P-revlex and $k[u, x_1, \ldots, x_m, y_1, \ldots, y_n]$ with a block order \todo{it should be called block order, instead of ``a block order''}.
...
$V=\{x_i y_j: 1\le i\le m, 1\le j\le n\}$
 and $U_i$ is the set of monomials of the form $\prod_{j=1}^{i} y_i^{a_i}$ \todo{should this be $y_j^{a_j}$ instead of $y_i^{a_i}$?} with $a_j\in \mathbb{Z}^{\ge 0}, a_i\ne 0$ \todo{this conflation of $j$ and $i$ is confusing me, is $j$ ranging over all indices from $1$ to $i$?} such that
$\sum_{j=1}^i a_j c_j \ge c_{j+1}$
and there does not exist $((b_1, \ldots b_i)\ne (a_1, \ldots a_i)$ such that with all $b_j\le a_j$ and 
$\sum_{j=1}^i b_jc_j\ge c_{j+1})\}$. \todo{what is this close bracket closing?}  \\

\item[(c)] In particular, $R_\halfcan$ is generated over $R_\halfcan'$ in degrees at most $\beta$ with relations $I$ generated over $I'$ by relations in degree is generated $\max(2\beta, \beta + \sigma)$ {\rm(}where $\sigma=\max(\deg(x_i))$ \todo{include what indexing set this $i$ is over ($ i = 1,\ldots, m$).}{\rm)}.
...
\noindent
\todo{why is there no floor in the above equation, as in ~\ref{eqn:deg1_sat_ind_dim}} so in $H^0 (X, \lfloor d\halfcan \rfloor)$ we can find elements with pole
...
construct the $y_i$'s as desired. \todo{cite where} \todo{What do we desire about the $y_i$?}
...
c_i $ at $P$.  Moreover, a set $\lfloor d_i \frac{\alpha}{\beta}\rfloor$ \todo{how is this a set?} elements in $(R_{D})_{d_i}$ with respective pole orders $1, \ldots, \lfloor
...
For any $j, i \in \mathbb{N}$, with $i < j$, we have $\frac{c_i}{d_i} > \frac{c_j}{d_j}$. So, for any $z_1, \ldots, z_n$ with $\deg(z_j)<d_i$ \todo{are the $i$ and $j$ mismatched?} for all $z_j$, \todo{It might be clearer to say for all $j \in \{1,\ldots, ?\},$ since the subscript is really want is changing, or instead for all $j$ with $1 \leq j \leq ?$ } $\prod_{j=1}^n z_j$ must satisfy \todo{It seems like each $z_j$ must satisfy this, instead of the product satisfying this}
\todo{In the next equations, are we viewing these as rational functions on the structure sheaf or on $L'$, what point is the order of $z_j$ taken with respect to?}
\[
	\frac{-\ord_P(z_i)}{\deg(z_i)} < \frac{c_i}{d_i},
\]

\noindent
so we have the pole order of $\prod_{j=1}^n z_j$ is \todo{what is the following sum taken over?}
\[
	\sum -\ord(z_j) < c_i .
\]
...
\]
and for $i \in \mathbb{N}$, if $c_j$ is the maximal element among of $c_1, \ldots c_n$ at most $i$ (note that this always exists since $1=c_1\le i$), define
\[
	S_i = y_j S_{i-j}.
\]
Then define 
...
\iffalse{
\begin{cor}
Let $R_\halfcan$ be the fractional weight canonical ring associated to a fractional $\frac{2\alpha}{\beta}$-weight cusp forms with $\frac{\alpha}{\beta}$ in reduced form and $\beta\ge 3$, with signature $(g; e_1, \ldots, e_n; \delta)$ where $g=0$.  Then $R_\halfcan$ is generated in degrees at most $e = \max(e_i)$ with relations generated in degrees at most $2e$.
\end{cor}
\begin{proof}
Let $K$ be the log canonical divisor of the curve $(\sx, \Delta)$ associated to the $\frac{2\alpha}{\beta}$ weight cusp forms.  We see that $\frac{\beta}{\alpha}\halfcan_\sx\sim K_\sx$, so the floors of each 

Then $\beta \halfcan_\sx \sim \alpha K_{\sx}$ so $\beta\lfloor so $\beta | \alpha \deg(K_\sx)$; since $\gcd(\alpha, \beta)=1$, $\beta| \deg(K_\sx)\ge -2$.  Thus, since $\beta\ge 3$, $\deg(K_\sx)\ge 0$.  Thus $\deg(
\end{proof}
}\fi

We now restrict our attention to log canonical rings of stacky curves.  The previous lemma deals with most relevant induction cases when inducting from a half-canonical ring saturated at one.  We complement it with the following two lemmas that allow us to induct under certain conditions when the half-canonical ring is saturated at two and three.
...
_P(y_i) = 1$ by Riemann-Roch. \todo{Understand why/whether we need to cite Lemma 5.4.7.} 
...
Lemma 5.4.7.}  We see by pole order considerations that
...
\todo{I see it says this above, but I think in the table, it should be made clear exactly what the hypotheses on $k,a,b,c,e_i$ are.}
