...
\[
	...
	= 1}^r \left\langle \frac{1}{e_i}P_i \right\rangle \right) \subseteq \BQ \otimes \di X
\]
\end{defn}
...
For a homogeneous polynomial $f \in k[x]_{\vec{a}}$, we define the \textbf{initial term} $\initial(f)$ to be the monomial in the support of $f$ that is largest with respect to $\prec$, and we define $\initial(0)=0$.  Let $I \subseteq k[x]_{\vec{a}}$ be a homogeneous ideal.  We define the \textbf{initial ideal} $\initial(I)$ to be the ideal generated by $\{\initial(f) : f \in I\}$.  A \textbf{Gr\"obner basis} (also called a \textbf{standard basis}) for $I$ is a set $\{f_1,\dots,f_n\} \subseteq I$ such that 
