...
\todo{Make the sentences in the abstract shorter}
\todo{Make sure we call all divisors spin canonical divisors}
...
\todo{I changed the following to incorporate the no odd weight case. Is it ok?}

The main theorem of this paper, Theorem ~\ref{thm:main}, combined with a result of Voight and
Zureick-Brown ~\cite[Lemma 10.2.1]{vzb:stacky}, together with Voight and Zureick-Brown ~\cite[Theorem 9.3.1]{vzb:stacky} and ~\cite[9.3.1]{vzb:stacky} provide the following application
to modular forms. Note that the result from Voight and
Zureick-Brown ~\cite[Lemma 10.2.1]{vzb:stacky} follows almost immediately from the classical work of Kodaira--Spencer
...
\ldots, e_r; \delta)$. 
If $M_k(\Gamma) = 0$ for all odd $k$, then $M(\Gamma)$ is generated in degree at most $3 \cdot\max(3,e_1,\ldots, e_r)$ with relations in degree at most $6 \cdot \max(3,e_1,\ldots, e_r)$.
If there is some odd $k$ for which $M_k(\Gamma) \neq 0,$ then, the ring of modular forms
$M(\Gamma)$ is
generated as a $\BC$-algebra by elements of degree at most $e = \max
 (5, e_1, \ldots , e_r)$ with relations in degree at most $2e,$ so long
...
A pair of a stacky curve and a log divisor $(\sx, \Delta)$ is called a \textbf{log stacky curves}
...
triples $(\sx, \Delta, \halfcan)$ where $\sx$ is a stacky curve, $\Delta$ is a log divisor, and $\halfcan \in \di \sx$ satisfies
$2\halfcan \sim K_X + \Delta + \sum_{i=1}^{r}\frac{e_i-1}{e_i}P_i$. Such a divisor $\halfcan$
is called a \textbf{log spin canonical divisor} on $(\sx, \Delta)$. 
We use $\halfcan_X := \lfloor L \rfloor $ to refer to the spin canonical divisor
(also known as the half-canonical divisor, semi-canonical divisor, 
or theta characteristic) associated to the
coarse space $X$ of $\sx$, i.e. $\halfcan_X$ is a divisor such
that $2\halfcan_X \sim K_X + \Delta$. Furthermore, we use the following
analogous notation:
...
\todo{possibly compactify or refer to relations, Aaron: I think we should almost remove these definitions, and that these shouldn't be in environments. I suspect people will be familiar with them, or be able to look them up.}
...
\begin{defn}
\label{defn:grlex}
The {\bf graded lexicographic} order, or {\bf grlex} $\prec_{\text{grlex}}$ on a polynomial ring $k[x_{d_1, 1}, \ldots,
x_{d_n, n}]$ is defined as follows.
...
\label{eqn:grlex-tie}
	...
	f_i - f_i' > 0 \text{ for the smallest } i \text{ such that } f_i \neq f_i'.
\end{equation}
\end{defn}
...
rings such that $R_\halfcan$ is generated by elements $x_{d_i, i}$ \todo{They're usually $x_{i,e_i}$ with the degree in the second position, which is a bit strange. Somehow, I think we might want to switch the ordering here, though it still might be confusing.} and $R$
...
Following Voight and Zureick-Brown \cite[Section 8.1]{vzb:stacky}.
the \textbf{block term order} is defined as follows. Let $k[y_{c_1, 1}, \ldots, y_{c_m, m}]$ and $k[x_{d_1, 1}, \ldots,
...
Let $\prec$ be an ordering on $k[x_{d_1, 1}$, $\ldots$, $x_{d_n, n}]$ and let $f \in k[x_{d_1, 1}, \ldots, x_{d_n, n}]$ be a homogeneous
...
$x_{d_n, n}]$. Then the {\bf initial ideal} $\initial(I)$ of $I$ is
...
$V=\{x_i y_j: 1\le i\le m, 1\le j\le n\}$
 and $U_i$ is the set of monomials of the form $\prod_{j=1}^{i} y_j^{a_j}$ with $a_j \in \BN_{\ge 0}$ such that
\begin{enumerate}
...
If $i\in \{1, \ldots, n\}$, then $y_i f$ is either in $S$ or there is some $h \in \BN$ such that $i\le h\le \max(i,l)$ satisfying $\prod_{j=1}^h y_j^{b_j}\not\in S$, 
...
\begin{lem}
\label{lem:sat-2}
Let $(\sx, \Delta, \halfcan)$ and $(\sx', \Delta, \halfcan')$ \todo{The notation i was using for these throughout the paper was $\halfcan$ and $\halfcan'$ without the $\sx$ subscripts. We should make one of them consistent with the other.} be log spin curves with the same coarse space $X = X'$ having signatures $(g; e_1, \ldots, e_r; \delta)$ and $(g, e_1, \ldots, e_{r- 1}, \delta)$, where $e_r=3$.  Suppose $g > 0,$ and, if $g = 1,$ then $\deg 3\halfcan' \geq 2.$ Then, $\sat(Eff(L'))\le 2$. \todo{Why is it automatic that the saturation is 2? Maybe you can explain this in the proof in a sentence.}
...
\begin{enumerate}
	...
	\item[(b)] Equip $k[y_3, y_4]$ with $grevlex$ so that $y_3 \prec 
		...
		order with $y_3, y_4$ preceding $x_2, \ldots, x_m$. \todo{In here and throughout the section, we should just enforce the convention that the $y_i$ come before the $x_j$, and then we won't have to say which precede which.} Then,
		\begin{align*}
...
%\begin{rem}
%Note that $S_{\sigma,J}(i,d-1)$ as defined in condition \ref{Ad-iii} of Definition ~\ref{defn:admissible} can equivalently be defined as $S_{\sigma,J}(i,d-1)= \{j \in J : j \neq i \text{ and }(e_j' + 2d-2)|e_i' + 2d\}.$ Written this way, $\# S_{\sigma,J}(i,d-1)$ is more apparently similar to $\mu(i,d)$ as defined in \cite[Defintion 8.5.1]{vzb:stacky}.
%\end{rem}
...
%\end{lem}
%\begin{proof}
%To show \ref{Ad-i} and \ref{Ad-ii} look at some $\sx'$ with $r- 1$ stacky points $'s=\{Q_1, \ldots, Q_{j - 1}, Q_{j + 1}, \ldots Q_r\}$ including a chosen $Q_i$ (and excluding some $Q_j\ne Q_i$). Then select $y_{e_r'}$ from the choice that makes the set $J'=\{i\}$ an admissible set in $\sx'$; this naturally can be seen inside of $R_{\sx}$ and satisfies \ref{Ad-i}. Furthermore, since $s$ is an admissible set of $L'$, in particular, we must have \ref{Ad-ii} satisfied so $\frac{-\ord_{Q_l}
%^{\halfcan_X'}(y_i)}{\deg (y_i)} = 0$ for\todo{I'm confused how \ref{Ad-ii} follows in genus 0?}
%
...
%\item When $g= 0$, since $L'$ has a nontrivial admissible subset within $\{e_1, \ldots, e_{r- 1}\}$ satisfying \ref{Ad-i}, we know $\deg(\lfloor 3\lfloor)\ge 0$, so it is base-point free.
%\end{itemize}
%\end{proof}
...
\end{align}
\todo{Peter: I understand why this should be true, although its not clear to me that there it easily follows.  Maybe we should explain it a little as to not confuse the reader. Aaron: Can we ask David, this was clear to me in his paper, and he also didn't include it.}

\noindent
Now fix $i \in J$. We check Inequality ~\eqref{eqn:order-degree} holds with $f = y_{i, e_i}z$, implying $y_{i,e_i}z \in R'$ in the three following cases: 
...
By part (a), condition \ref{Ad-ii}, and Lemma
...
{\bf Part (d):}
To check (d), we show \ref{Ad-i},\ref{Ad-ii}, and \ref{Ad-iii} are satisfied. We know \ref{Ad-i} holds by part (b), taking the $y_{
i, e_i}$ as the generators in degree $e_i$. Next, \ref{Ad-ii} is
strictly monotonic in the $e_i$ and hence also holds for $(\sx, J)$.
Finally, if \ref{Ad-iii} holds for $e$ then it holds for $e + 2$ by
definition. This is where we use that \ref{Ad-iii} holds for $d > 0$
...
%Let $X$ be a curve of genus $g \geq 2$ and let $L$ be a divisor with $2 L \sim K +\Delta$. Then, $R_\halfcan$ is generated in degree at most $5$.
%\end{lem}
%
...
%Then, since $X \not \cong \BP^1_k,$ and $\deg L = 1$, we must have $H^0(X,L) = 1$. Hence, we know $L$ is effective, so up to linear equivalence, we may assume $L = P$ where $P$ is hyperelliptic fixed. In this case, it is shown in \cite{neves:halfcan}[Theorem III.19] that $R_\halfcan$ is generated in degree $\leq 3$.
%
...
The base cases of $L' = \frac{3}{7}P_1$ and $L' = \frac{1}{3}P_1 +\frac{1}{3}P_2$ can be similarly computed. Note that in addition to $L' = P - Q + \frac{2}{5}P_1,$ and $L' = \frac{3}{7}P_1$ being base cases, they are also exceptional cases, as listed in Table ~\ref{table:g-1-exceptional}.\todo{Peter: I don't understand this last sentence?  How are they base cases?}
...
First, if $P$ is not a basepoint of $L',$ then the hypotheses of Lemma ~\ref{lem:sat-1} are satisfied. In particular, we may take $u \in h^0(\sx,L)$ with $u \neq 0$ and $\ord_P(u) = \lfloor L|_P \rfloor $\todo{Peter: Is $\lfloor L|_p\rfloor$ standard notation?  It looks weird}, as $P$ is not a basepoint of $L'$. Furthermore, Equation ~\ref{eqn:deg1-sat-ind} is satisfied by Riemann Roch, since $\deg L' \geq 1.$ Therefore, by Lemma ~\ref{lem:sat-1}, the theorem holds for $L= L'+P$.  \todo{Peter: I don't find this paragraph (after the first sentence) very useful.  1: I'm not sure if we even need to go through the check that it satisfies the conditions for lemma 4.5.  2: I don't find the check clear or illuminating as written.  Either we should reword it or remove it (feel free to disagree, of course)}

Otherwise, $P$ is a basepoint of $L'$, so the hypotheses of Lemma ~\ref{lem:sat-2} are satisfied since $\deg 3(L'+P) \geq 2$ as $\deg L' + P \geq 1$\todo{Peter: I don't follow why we have a $+P$ here.  lemma 4.8 requires that $L'$ satisfy $\deg(3L')\ge 2$,not $\deg(3L)\ge 2$.  This isn't a problem though since $\Delta> 0$} Therefore, by Lemma ~\ref{lem:sat-2}, the theorem holds for $L = L'+P$.  The previous argument shows the theorem holds for $L_X$.

To complete the case that $\Delta > 0$, we now need show the theorem holds for a \emph{stacky} log spin divisor $L$.  It suffices to show that if the theorem holds for a divisor $L'$ with $\deg \lfloor L' \rfloor > 0$, then it holds for $L' + \subhalf {e_i}P_i$ with $e_i$ odd.  As above, if $P$ is not a basepoint of $L'$ then the theorem holds for $L' + \subhalf {e_i}P_i$ by Lemma ~\ref{lem:sat-1}; on the other hand, if $P$ is a basepoint of $L'$ then the theorem holds for $L' + \subhalf {e_i}P_i$ by Lemma ~\ref{lem:sat-2}.
...
Second, we deal with the case that $\sx$ has at least two stacky points. If $(\sx',\Delta,L')$ is a spin canonical curve so that $L' = P - Q + \frac{1}{3}P_1+ \frac{1}{3}P_2$, then as found in Example ~\ref{eg:base-1-33}, the signature $\sigma = (1;3, 3; \delta)$ satisfies the hypotheses of Lemma ~\ref{lem:raise-stacky-order}\todo{Peter: Is it better for us to say the signature satisfies the hypothesis rather than the triple?}. Therefore, applying Lemma ~\ref{lem:sat-2} $r-2$ times, we see that the theorem holds for $(\sx',\Delta',L')$ with $L' = P - Q + \sum_{i = 1}^{r}\frac{1}{3}P_i.$ Finally, by Corollary ~\ref{cor:raise-stacky-order}, this theorem holds for $L' = P - Q + \sum_{i = 1}^{r}\subhalf {e_i}P_i$, as desired. \todo{Peter: Why is it immediate that $n$ threes satisfies the condition for admissibility?  We got rid of that proof from earlier.  In fact, didn't we find it to be false?}

\vskip.1in
\noindent
{\sf Case 2b: $L_X = 0$}

This case is analogous to 2a. \todo{Peter: Maybe you should mention something about how the exceptional cases differ, and how this affects the argument?}
\end{proof}
...
As noted in Remark ~\ref{rem:delta-not-1}, $\delta$ is even, so we can reduce to two cases: $\delta \geq 2$ and $\delta = 0$.
In the former case, $\halfcan$ is linearly equivalent to an effective divisor, so the
result of Theorem \ref{thm:main} follows immediately by repeatedly applying Lemma ~\ref{lem:sat-1} to add the necessary fractional points.
On the other hand, the proof when $\delta =
0$ is a more involved, and we dedicate the remainder of this section to proving it through the following steps: 
characterizing saturations points (Subsection ~\ref{ssec:g_0_saturation}), describing
base cases (Subsection ~\ref{ssec:g_0_base}), presenting exceptional
cases (Subsection ~\ref{ssec:g-0-exceptional}), and applying inductive Lemmas ~\ref{lem:sat-3} and ~\ref{lem:raise-stacky-order} to prove\todo{synonym for prove, so its not repeated twice in this sentence?} Theorem \ref{thm:main} when $\delta = 0$ (Subsection ~\ref{ssec:g-0-main}.\todo{Peter: I made some changes to this last paragraph (gramatical and stylistic-not mathematical).  See if you like them.}
...
Let $(\sx', \Delta, \halfcan')$ be a log spin curve with signature $(0; e_1, \ldots, e_r; 0)$. Then, $R' := R_{\halfcan'}$ is generated by elements of degree at most $e = \max(5 , e_1, \ldots, e_r
...
the remaining cases, we one follow a similar method to find a
presentation satisfying the desired conditions.

The following table demonstrates \todo{Peter: Demonstrates maybe isn't the right word.  Really the table states it, but we want to find a better word than that.} that $R(\sx', 0, L')$ is
...
\end{longtable}
\todo{Peter: Can we make a better description for the table above?}
...
\end{longtable}
\todo{Peter: In case c, I changed $J=\{3\}$ to $J=\{2, 3\}$.}
\todo{Peter: I don't particularly like split tables.  Might we consider not using [h], so its not split}
...
in the case $g = 0$.  \todo{This paragraph is kind of repetitive, and awkward.}
...
\end{longtable}
\todo{Peter: Technically, shouldn't the first and last $e$ be 5 instead of 3, since $e=\max(5, e_i)$?}
...
Heuristically, \todo{Peter: Is heuristically the right word here?  I interpret heuristically to mean ``providing a technique or method to solve a problem,'' whereas I think you mean something more along the lines of intuitively.}
...
case which is done, with $L$ replaced by $K$, \todo{Peter: I don't think its helpful to say with $L$ replaced by $K$.  It might slightly confuse the reader into thinking that replacing half canonical divisor with canonical divisor changes the problem.} by Voight and
Zureick-Brown ~\cite[Section 4.2]{vzb:stacky}.

First, consider the case when $\delta \geq 2$. \todo{Peter: it feels a tiny bit redundant here, as we kind of said this in the introduction to the section.}
 In any such cases,
$\halfcan_X$ is an effective divisor so we can inductively apply
the Lemma ~\ref{lem:sat-1} from the classical non-stacky case
to deduce that $R(\sx, \Delta, \halfcan)$ is generated up to degree $e$
with relations generated up to degree $2e$.

By Remark ~\ref{rem:delta-not-1}, it only remains to deal with the case $\delta = 0$, so $\halfcan_X$ is not effective.
Let $\sigma = (0; e_1, \ldots, e_r; 0)$ not be one of the exceptional cases in Table
~\ref{table:g-0-exceptional}.  We get the following three cases, depending on the value of $r$:
...
If $r < 3$, then $\deg 
\lfloor d \halfcan \rfloor < 0$ for all $d \geq 0$ so we get the
trivial ring $R(\sx, \Delta, \halfcan) = k$.
...
\begin{enumerate}
	\item One direction for further research is to 
		...
		divisors would translate to rings of fractional weight modular 
		...
		\end{question}
		Note that that the above question is answered affirmatively in the 
		case that $\sx$ has genus $0$ and $D$ is effective, as follows by 
		inductively applying Lemma ~\ref{lem:sat-1}: If $\sx$ has signature 
		$(g;e_1, \ldots, e_r; \delta)$ then $D$ is generated in degree at 
		most $e = \max(e_1, \ldots, e_r)$ with relations in degree at most
		$2e$. Additionally, it may be possible to modify the proof of Lemma
		~\ref{lem:raise-stacky-order} to extend to the setting of fractional 
		weight modular forms. Given suitable generalizations of these 
		lemmas, one may be able to approach the problem of finding 
		generators and relations for fractional weight modular forms using 
		techniques similar to those in this paper.

	\item In Voight and Zureick Brown \cite[Definition 2.2.7]{vzb:stacky},
		the notion of a generic initial ideal is defined, which 
		encapsulates the idea of whether the relations for $R_\halfcan$ are 
		generically chosen, in a precise sense. The proof of Theorem
		~\ref{thm:main} decidedly does not yield a generic initial ideal, 
		as generators and relations are constructed in a highly nongeneric 
		manner. In particular, Lemma ~\ref{lem:raise-stacky-order} 
	...
	\item As noted in Remark ~\ref{rem:explicit-generators}, the 
		proof of Theorem ~\ref{thm:main} given in this paper gives an 
		explicit procedure for computing the generators and relations of $R_
		L$ when the genus of $\sx$ is $0$ or $1$.
		\begin{question}
		\label{ques:algorithm}
			Can one give an algorithm for explicitly computing the generators 
		...
		algorithm for determining the generators of $L$. This raises the 
		...
		\label{ques:spin-canonical-petri}
			...
			generators and minimal relations for $R_\halfcan$? If so, can one give the 
			generic ideal of $R_\halfcan$ or the generic initial ideal of $R_\halfcan$, as 
			defined in Voight and Zureick Brown \cite[Definition 2.2.7]
			{vzb:stacky}?
		\end{question}
		In particular, if Question ~\ref{ques:spin-canonical-petri} were 
		answered affirmatively one would be able to write down explicit 
		generators and relations for $R_\halfcan$ on a stacky curve, essentially 
		by inductively applying Lemma ~\ref{lem:raise-stacky-order}.
\todo{Aaron, I tried to edit the following question, but it still seems a bit imprecise, and the question in particular is a bit of a long sentence.}
	\item In Subsection \ref{ssec:bounds-high-genus}, we reference 
		...
		lemmas to add stacky points and obtain bounds of $e = \max(5,e_1, \ldots, e_r)$ and $2e$.  Reid \cite[Theorem 3.4]{reid:infinitesimal} in fact proves something slightly stronger: that in 
		most cases his bound is actually 3 and 6 with well-characterized 
		exceptions.
		\begin{question}\label{ques:reduce-5,10-to-4,8}
			In the case that the genus $g \geq 2,$ can we reduce the bounds in Theorem \ref{thm:main} on 
			the degrees of generation and relations from $e = \max(5,e_1,\ldots, e_r)$ and $\max(10, 2e_1, \ldots, 2e_r)$ to $\max(4,e_1, \ldots, e_r)$ and $
			\max(8, 2e_1, \ldots, 2e_r),$ apart from a finite list of exceptions?
		\end{question}
		\begin{rem}\label{rem:reduce-3,6}
			It is unlikely that this can be reduced further for the case 
			of stacky curves, because Lemma \ref{lem:sat-2}, which allows 
			us to add a stacky point in genus $g > 0$ saturated at 2, 
			requires a generator in degree 4 and potentially a relation in 
			degree 8, even when inductively adding on stacky points of 
			stabilizer order 3. However, perhaps in the non-stacky case, one 
			...
			\end{rem}
	\item While Theorem ~\ref{thm:main} gives a minimal set of 
		generators for the log spin canonical ring $R_\halfcan,$ the set of 
		relations is not minimal. In many of the $g = 0$ and $g=1$ cases, 
...
bound given by Reid's work on the maximal degrees of minimal
generators and relations of spin canonical rings in high genera.
Finally, we gratefully acknowledge that our research was
financially supported by NSF Grant Award Number 1250467 via the
Emory University Number Theory REU. We deeply appreciate all of the
