...
\cite[Chapters 7-9]{vzb:stacky} give similar bounds, and we extend their work to all Fuchsian groups $\Gamma$. In order to answer this question, we translate the seemingly analytic question of understanding the ring of modular forms into the algebraic
...
sections. In the case that $g \geq 2$, $\Omega_X$ is ample, so $X \cong \proj R$. Petri's
...
\in \di \sx$ a {\bf log half-canonical divisor} on $(\sx,\Delta)$ if
$2\halfcan \sim K_\sx + \Delta$. If $X$ has genus $g$ and $\delta := \deg \Delta,$ then we
say $\sx$ has \textbf{signature} $\sigma = (g; e_1, \ldots, e_r; \delta)$. A
...
initial ideal of relations for $\halfcan'$ when the half-canonical
...
In \cite{dorney:canonical}, O'Dorney uses best lower approximations to provide a minimal presentation for the canonical ring of a (not-necessarily effective) divisor $D=\frac{\alpha_1}{\beta_1}P_1+\frac{\alpha_2}{\beta_2}P_2$ of $\mathbb{P}^1$ consisting of at most 2 points in its support, showing that it is generated in degree $e=\max{\beta_1,\beta_2}$ with relations generated in degrees at most $2e$.  He further extends the result to more points, but only bounds the degree of minimal generation by a value on the order the least common multiple of the denominators of fractional points.  

In Lemma \ref{lem:sat-1} we provide an inductive method to strengthen
O'Dorney's result \todo{cite theorem 8} on $\BP^1_k$ to divisors $D = \sum_{i=1}^n \frac{\alpha_i
}{\beta_i} P_i$ for points $P_1, \ldots, P_n$ on a curve $X$ such that $H^
0(X, \lfloor D \rfloor) > 0$; our result implies that $R_D$ is generated in
degrees at most $e = \max(\beta_i)$ with relations generated in
degrees at most $2e$.  Furthermore, given additional constraints on $H^0(
X, \lfloor dD \rfloor)$ for various values of $d$, we can apply the
same Riemann-Roch arguments in arbitrary genus. 
To do this, we extend an inductive result by Voight and
Zureick-Brown on stacky canonical rings to arbitrary $\BQ$-
divisors. \todo{cite theorem 8.3.1}

First, however, we give a criteria to determine if a set of monomials generates the initial ideal of relations, and define notation for a type of monomial ordering.

\begin{lem}\label{lem:relations_from_generators_induction} 
Suppose $\halfcan$ and $\halfcan'$ are $\BQ$ divisors on a curve $X$ with $\halfcan=\halfcan'+\frac{\alpha}{\beta}P$, that $R_{\halfcan'}$ is generated by $x_1, \ldots, x_m$ and $R_{\halfcan}$ is minimally generated by $y_1, \ldots, y_n$ over $R_{\halfcan'}$ with ideals of relations $I'$ and $I$ respectively.  Suppose there are sets of monomials $S\subseteq R_\halfcan-R_{\halfcan'}$ and $T\subseteq R_\halfcan-(S\cup R_\halfcan')$, and an ordering $\prec$ such 
\begin{enumerate}
...
\begin{align*}
	& S \\
	\cup \; &\langle T\rangle \\
	\cup \; & \initial_\prec{I'} k[x_1, \ldots, x_m, y_1, \ldots, y_n]
\end{align*}
\end{enumerate}
Then 
\begin{align*}
...
\end{lem}
\begin{proof}
Suppose monomial $f\in T$ has a pole order $a$ at $P$.  Then since $S$ forms a $k$-basis of $R_{\halfcan}$ over $R_{\halfcan'}$, we can write $f + (\sum_{g\in S'} C_g g)+r$ for some finite subset $S'\subseteq S_{\deg(f)}$ and $r\in R_{\halfcan'}$.  This demonstrates that 
\begin{align*}
...
As suggested by the previous lemma, we will need an ordering on monomials, and the following definition will prove useful in the ensuing lemma.
\begin{defn}
We equip the graded ring $k[x_1, \ldots x_m]$, with weight $a_i$ and pole order $-\ord_P(x_i)$ attached to $x_i$, the graded $P$ reverse lexicographic order {\bf{g-P-revlex}} if for $f=\prod_{i=1}^m {x_i}^{q_i}\ne g=\prod_{i=1}^m {x_i}^{r_i}$, $f\prec g$ iff one of the following holds
\begin{enumerate}
\item $\deg(f) < \deg(g)$
\item $\deg(f) = \deg(g)$ and $-\ord(f) > -\ord(g)$
\item $\deg(f) = \deg(g)$, $-\ord(f) = -\ord(g)$, and for $i$ maximal such that $q_i-r_i$ is non-zero, that expression is negative.  
\end{enumerate}
\end{defn}
We leave it as an exercise to verify that this is an order.  \todo{check for ourselves that this actually is}
We now offer a lemma that allows us to inductively add $\mathbb{Q}$-points to a divisor and describe the new generators and relations.

\begin{lem}
\label{lem:sat-1}
Let $X$ be a genus $g$ curve with a $\BQ$-divisor $\halfcan'$
...
$u, x_1, \ldots, x_m$ with $\ord_P(x_i)=0$ for all $x_i$, $\ord_P(u) = 0$, \todo{Aaron: Instead of having $\deg u = 0$, don't we only need that $\ord_P(u) = \lfloor \halfcan'|_P \rfloor$?} and $\deg(u)= 1$ when viewed as sections of $\so_{\halfcan'}$.  Suppose $u, x_1, \ldots, x_m$ generate $R_{\halfcan'}$ in degree at most $\sigma$ with relations generated
...
degree $d_i$ and has a pole at $P$ of degree $c_i$, when viewed as sections of $\so_{\halfcan'}$
...
\item[(c)] In particular, $R_\halfcan$ is generated over $R_\halfcan'$ in degrees at most $\beta$ with relations $I$ generated over $I'$ by relations in degree is generated $\max(2\beta, \beta + \sigma)$ (where $\sigma=\max(\deg(x_i))$).
...
\noindent
be the positive best lower approximation of $\frac{
...
\[
	\frac{-\ord_P(z_i)}{\deg(z_i)} < \frac{c_i}{d _i},
\]
...
\]
then we observe that $S$ and $T$ satisfy the hypothesis of Lemma \ref{lem:relations_from_generators_induction}, so we immediately prove part (b) of this lemma.
...
\todo{Something about fractional weight modular forms?}
...
~\ref{lem:raise-stacky-order}. Under fairly general
conditions, this theorem will allow us to show that if Theorem ~\ref
{lem:raise-stacky-order} holds for a curve with signature
...
curves, with the same stacky points $Q_1, \ldots, Q_r$. Suppose
...
For the purposes of our induction, we will often be adding in a single 
...
\delta)$. We will need to deal with signatures $\sigma$ satisfying one of the following three cases:
\begin{enumerate}
	\item $g = 0, \delta = 0, $ and $r \geq 5$
	\item $g = 1, \delta = 0,$ and $ r \geq 2$
	\item $g \geq 2, \delta$ arbitrary, and $r \geq 1$.
\end{enumerate}
\end{rem}
...
space $X'$ and signature $\sigma = (g; e_1', \ldots, e_r'; \delta)$ and either 
\begin{enumerate}
	\item  $(\sx',\halfcan', J)$ is admissible with generators $x_j, y_{i, e_i'} \in R' = R_{L'}$ with $i \in J$ as in Definition ~\ref{defn:admissible}
	\item or $\sigma$ is one of the signatures described in Remark ~\ref{rem:three-cases}, with $y_{1, 3} = y_{2,3} = \ldots, y_{r,3}$ be a rational section of $\sco(\halfcan_X)$ so that $\ord_{P_i}^{\halfcan_X'}(y_{1,3}) = 1$ at $P_i$ for all $i = 1,\ldots, r$.
\end{enumerate}
Let
...
\subhalf{
	...
	\item[(b)] If $(\sx',\halfcan',J)$ is admissible, then $y_{i, e_i'}^a y_{i, e_i}^b$ with $a \geq 
0,
		b > 0$ span $R$ over $R'$ and the elements $y_{i, e_i}$ minimally
		generate $R$ over $R'$.
	\item[(c)] If $(\sx',\halfcan',J)$ is admissible, equip $k[y] = k[y_{i, e_i}]_{i \in J}$ and $k[x]$ with
...
\begin{proof}
We will only prove this in the case (1), that $(\sx', \halfcan', J)$ is admissible, as the case (2) that $\sigma$ is as in Remark ~\ref{rem:three-cases} is completely analogous.

Let $(\sx, \Delta, L)$ and $(\sx', \Delta, L')$ be the two log spin curves in 
...
Next, define $R_0 = R'$ and for $i \in \{1,\ldots, r\},$ inductively define 
$$R_i = \begin{cases}
	R_{i - 1} &\text{ if }i \notin J\\
...
Finally, we check (d), by checking $(Ad-i),(Ad-ii)$ and $(Ad-iii)$ are satisfied. We know (Ad-i) holds by part (b), taking the $y_{
...
Let $(*_i)$ denote the condition that $(\sx_i,\Delta, L_i)$ satisfies the conditions $(\sx',\Delta, L)$ satisfies the conditions of Lemma ~\ref{lem:raise-stacky-order} and Theorem ~\ref{thm:main} and $(\sx_i, \halfcan_i, J_i)$ is admissible.

Since $(*_0)$ holds by assumption, by induction on $i$, it suffices to show that if $(*_i)$ holds then so does $(*_{i+1}).$ 
Indeed, by applying Lemma ~\ref{lem:admissible_subset}, it follows $(\sx_i, \halfcan_i, J_{i+1})$ is admissible, as $J_{i+1} \subset J_i$. Then, applying Lemma ~\ref{lem:raise-stacky-order} $\frac{e_{i+1}-e_i}{2}$ times, we see $(*_{i+1})$ holds.
\end{proof}
...
Note that $\dim_k \ker f = \dim_k H^0((n-2)L) = (n-3)(g - 1)$ using Riemann--Roch and the assumption if $n \geq 5$ or ($n = 5$ and $\Delta > 0$), as in the case $\Delta > 0$ we have $\deg \Delta \geq 2$.
...
Then, since $X \not \cong \BP^1_k,$ and $\deg L = 1$, we must have $H^0(X,L) = 1$. Hence, we know $L$ is effective, so we must have $L = P$ where $P$ is hyperelliptic fixed . In this case, it is shown in \cite{neves:halfcan}[Theorem III.19] that $R_L$ is generated in degree $\leq 3$.
...
\deg f- \deg a_i -2 \geq \begin{cases}
	...
	4 &\text{ if }\Delta > 0.
\end{cases}$$
In particular, we can write $\phi(a_i) = \phi(s_1) \alpha + \phi(s_1) \cdot \beta$. Choosing $g_i,h_i$ so that $\phi(g_i) = \alpha,\phi(h_i) = \beta$, we have $a_i = s_1 g_i + s_2 h_i \bmod I_{L,\deg a_i}$, as claimed.
...
In this case, we claim the theorem holds for $(\sx',\Delta, L')$. Indeed, the bound on the generators holds by part (a) of Lemmas
~\ref{lem:sat-2} and ~\ref{lem:sat-1},
and the bound on the relations holds by part (b) of Lemmas
~\ref{lem:sat-2} and ~\ref{lem:sat-1}.
Therefore, by Corollary ~\ref{cor:raise-stacky-order}, this theorem 
...
We next check $(\sx', \halfcan',\{1,2\})$ is admissible. We clearly have two generators $x_3, y_3$ in degree 3 with a pole of order $\frac{3- 1}{2}$ by construction. Hence, (Ad-i) holds. We next check (Ad-ii) for the point $P_1$, as the case of $P_2$ is symmetric. Here, by construction
$$
...
The proof of this can be gotten by first using Riemann--Roch and counting the pole order at $P_1$ considerations to check the above relation exists, and that the generators and relation determine a ring whose $k$th graded piece has the same dimension of $R_L$. It is analogous to Example ~\ref{ex:base-377}, and is omitted.
...
Finally, note that this particular case is also exceptional, because there is a relation in degree $12 > 2 \cdot 5$, but once $e' = 5$ is incremented to $e = e'+2 = 7,$ using Lemma ~\ref{lem:raise-stacky-order}, this relation in degree 12 caries over, and so the case $L' = P - Q + \frac{3}{7}P_1$ is not exceptional because $12 < 2 \cdot 7$.
\end{example}
...
Let $\sx$ be a tame, separably rooted stacky curve of genus 1. Let $P,Q$ are distinct hyperelliptic fixed points, and $R$ be a point on $\sx$.
...
\todo{fix this to accommodate the possibility that $P \in supp(L')$, this has already been done suppose ~\ref{lem:sat-1} accommodates it. }
...
First, if $P$ is not a basepoint of $L',$ then the hypotheses of Lemma ~\ref{lem:sat-1}. In particular, we may take $u \in h^0(\sx,L)$ with $u \neq 0$ and $\ord_P(u) = \lfloor L|_P \rfloor $ with $\ord_P(u) = 0$, as $P$ is not a basepoint of $L'$. Furthermore, Equation ~\ref{eqn:deg1-sat-ind} is satisfied by Riemann Roch, since $\deg L' \geq 1.$ Therefore, by Lemma ~\ref{lem:sat-1}, the theorem holds for $L= L'+P$.
...
First, we deal with the case $\sx$ has at least 1 stacky point. By Example ~\ref{eg:exception-1-5}, if $L' = P - Q + \frac{2}{5}P_1,$ then $R_{\halfcan'}$ is generated in degrees $2,3,$ and $5$ with a single relation in degree 12, $(\sx, \halfcan', \{1\})$ is admissible, and satisfies the hypotheses of Lemma ~\ref{lem:raise-stacky-order}. While $L'$ itself is an exceptional case, as it has a generator in degree $12 > 2 \cdot 5$, after applying Lemma ~\ref{lem:raise-stacky-order}, we see that $P - Q + \frac{3}{7}P_2$ does satisfy the constraints of this theorem, because only relations in degree $\leq 14 = 2 \cdot 7$ are added, while the previous relation lies in degree $12$, which is less than $2 \cdot 7 = 14$. Therefore, the Theorem holds for $L' = P - Q + \frac{3}{7}P_2$. Then, applying Lemma ~\ref{lem:raise-stacky-order} $\frac{e-7}{2}$ times shows that the Theorem holds for $L = P - Q + \frac{e- 1}{2e}P_1$.

Second, we deal with the case that $\sx$ has at least two stacky points. If $(\sx',\Delta,L')$ is a spin canonical curve so that $L' = P - Q + \frac{1}{3}P_1+ \frac{1}{3}P_2$, then, by Example ~\ref{eg:base- 1-33}, it follows that the signature $\sigma = (1;3, 3; \delta)$ satisfies the hypotheses of Lemma ~\ref{lem:raise-stacky-order}. Therefore, applying Lemma ~\ref{lem:sat-2} $r-2$ times, we see that the theorem holds for $(\sx',\Delta',L')$ with $L' = P - Q + \sum_{i = 1}^{r}\frac{1}{3}P_i.$ Assume for convenience  $e_1 \leq e_2 \leq \cdots \leq e_r$. Then, by Corollary ~\ref{cor:raise-stacky-order}, this theorem holds for $L' = P - Q + \sum_{i = 1}^{r}\subhalf {e_i}P_i$, as desired.
...
There are two main cases: when $\delta \geq 2$ and when $\delta = 0$
 (note that $\delta$ must be even because $\halfcan_X \in \di X$ with $2\halfcan_X \sim K_X +\Delta$, where $\deg K_X$ is even). \todo{make a remark about this somewhere in the beginning and reference it} In the first case, $\deg \lfloor \halfcan \rfloor
...
\begin{center}
\textbf{Table (I)}: Genus 0 saturation \todo{I see it says this above, but I think in the table, it should be made clear exactly what the hypotheses on $k,a,b,c,e_i$ are.}
\end{center}
...
\geq 0$. Since $\sigma_2$ has saturation $s = 5$, \todo{Why is there a 's here instead of just s?} as noted in
Subsection ~\ref{ssec:g_0_saturation}, then this map is surjective
when $d \geq 21 + s = 25$ (i.e. $R'$ is generated up to degree 24). \todo{Aaron: Isn't $s + 21 = 26$? Shouldn't it be generated up to degree 25, potentially?}
...
and $7 < d \leq 24$. \todo{Should this be 25?} Note that we can again use Max No ether's theorem
...
\begin{align*}
	a_1 x_{5, 1}^2 + a_2 x_{3, 1} x_{7, 1} + a_3 x_{3, 1} x_{7, 2} &= 0 & \text{ in degree $10$ and } \\
...
$x_{7,1} \prec x_{7,2} \prec x_{5,1} \prec x_{3,1}$, the initial ideal of $I$ is \todo{This will only be the case if $a_1,b_1 \neq 0$. We can definitely make this hold if we set up the variables in the right way, but I'm not sure if it's always true.}
\[
...
pole order at $P_3$. \todo{There is actually a small problem here, which needs to be fixed earlier. I think in order to satisfy admissibility, you will actually have to choose $x_{7,i}$ from the beginning to satisfy these constraints, as this will not generically be true. Just make sure to put these constraints on the poles when you define $x_{7,i}$} Use the presentation given
...
\begin{center}
\textbf{Table (II)}: Genus 0 non-effective base cases generator/relations
\end{center}
...
\begin{center}
\textbf{Table (III)}: Genus 0 non-effective base cases (Ad-iii)
\end{center}
...
so that $\sigma \neq \sigma_i$ where $\sigma_i$ is one of the exceptional cases in Table
...
~\ref{lem:sat-3} (recall from Subsection ~\ref
...
conditions of Lemma ~\ref{lem:raise-stacky-order}.
...
satisfy the hypotheses of Lemma ~\ref{lem:raise-stacky-order} as well. \todo{Aaron: I think we replace the above paragraph with an application of Corollary ~\ref{cor:raise-stacky-order}.}
...
up to degree $2e$. \todo{What does it mean to induct on the statements of two lemmas? Clarify this please.}
 \end{proof}
