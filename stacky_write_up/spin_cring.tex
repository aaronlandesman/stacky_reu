...
\begin{defn}
We equip the polynomial ring $k[x_1, \ldots, x_n]$ with the
(weighted graded) reverse lexicographic order \textbf{grevlex}
$\prec$: if 
\[
...
\noindent
and $x^{\vec{n}}$ are monomials in $k[x]_{\vec{a}}$, then $x^{\vec{m
}} \succ x^{\vec{n}}$ if and only if either
\begin{equation}
...
We also introduce a term ordering that is well-suited for inductive arguments: the \emph{block} term order.  
In our inductive arguments, we will often have the following setup: an inclusion $R \supset R'$ of canonical rings such that $R'$ is generated by elements $x_{i,d}$ and $R$ is generated over $R$ by elements $y_j$.  It is natural, therefore, to consider term orders which treat these sets of variables separately.  More formally, we make the following definition.

Let $k[x]_{\vec{a}}$ and $k[y]_{\vec{b}}$ be weighted polynomial rings with term orders $\prec_x$ and $\prec_y$, respectively, and consider $k[y,x]_{\vec{b},\vec{a}} = k[y]_{\vec{b}} \otimes_k k[x]_{\vec{a}}$ the common weighted polynomial ring in these two sets of variables $y,x$.

\begin{defn}
The \textbf{(graded) block} (or \textbf{elimination}) term ordering on $k[y,x]_{\vec{b},\vec{a}}$ is defined as follows: we declare
  \[ y^{\vec{m}}x^{\vec{n}}  \succ y^{\vec{m}'}x^{\vec{n}'}\]  
  if and only if
  \begin{enumerate}
  \item[(i)] $\deg y^{\vec{m}} x^{\vec{n}}  > \deg y^{\vec{m}'} x^{\vec{n}'} $, or
  \item[(ii)] $\deg y^{\vec{m}}x^{\vec{n}} = \deg y^{\vec{m}'} x^{\vec{n}'} $ and 
    \begin{enumerate}
    \item $y^{\vec{m}}\succ_y y^{\vec{m}'}$ or
    \item $y^{\vec{m}} =  y^{\vec{m}'}$ and $x^{\vec{n}} \succ_x x^{\vec{n}'}$.
    \end{enumerate}
  \end{enumerate}
\end{defn}

The block ordering is indeed a term order: the displayed inequalities directly give that any two monomials are comparable, and the inequalities are visibly stable under multiplication by a monomial.  One can similarly define an \textbf{iterated (graded) block} term ordering for any finite number of weighted polynomial rings.

The block ordering is the most suitable ordering for the structure of $R$ as an $R'$-algebra
...
Choose a positive best lower approximation $\frac{c_i}{d_i}$.  Choose $z_1, \ldots, z_r \in R_\halfcan$ so that $\sum_{j=1}^r \deg(z_j) = d_i$ and $\deg(z_j)<d_i$ for all $j\in \{1, \ldots r\}$. Then for each $j$, \todo{Aaron: I don't see why either of the next 2 claims hold, but maybe I'm being silly.}
\[
...
we cannot assume that $\{P_i\} \cap \Supp(L_X) = \emptyset$, as $H^0(X, L_X)$ may have no global sections.  We circumvent this issue by working with orders of zeros and poles relative to the divisor we are inducting from, instead of relative to the structure sheaf.
