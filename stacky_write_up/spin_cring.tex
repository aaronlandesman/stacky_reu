...
\begin{enumerate}
	...
	\item $L = P - Q + \sum_{i=1}^{n}\frac{1}{3}R_i$
\end{enumerate}
...
\begin{example}
Let $L = P - Q + \frac{1}{3}R_1 + \frac{1}{3}R_2$. Here, we claim $R_L \cong k[u,x,y_3,y_4]/(xy_3- \alpha uy_4,y_4^2 - \beta x^2 u - \gamma y^2u)$ where $\alpha,\beta,\gamma \in k$, and $\deg u = 2, \deg x = \deg y_3 = 3, \deg y_4 = 4,$ where $u,x$ are as in Example ~\ref{eg:g_1_deg_third} and $y_3, y_4$ are as in Proposition ~\ref{prop:sat_two_induction}. \todo{Explain why this forms a system of generators and relations, and also state more clearly what $y_i$ are.}
\end{example}
...
\todo{make sure to switch poles and zeros where appropriate}
\begin{lem}
\label{lem:semicanonical_generation}\todo{include that this was as shown in Neves, with degree 6 omitted}
...
Second, we deal with the case that $H^0(X,L) \neq 0$. 
Then, since $X \not \cong \BP^1,$ and $\deg L = 1$, we must have $H^0(X,L) = 1$. 
In this case, $L$ is linearly equivalent to an effective divisor, so we may assume $L = P_1$. 
Then, taking the function $y \in H^0(X,5P_1),$ to be that with $\di_\infty y = 5P_1,$ 
corresponding to the hyperelliptic involution, and $u \in H^0(X,P_1)$ 
corresponding to the rational function 1, we see that $y \cdot v \in H^0
(X,6P_1)$ determines an element with a pole of odd order at $P_1$, which is 
therefore independent from the image of $\sym^3 H^0(X,K)$. Hence, we again 
obtain there are no new generators in degree 6.
\end{proof}
...
\begin{align*}
	f \equiv \sum_{i}^{}a_i x_i \equiv \sum_{i}^{}(s_1g_i + s_2h_i)x_i \equiv s_1 \left( \sum_{i}^{}g_i x_i \right) + s_2 \left( \sum_{i}^{}h_i x_i \right) \equiv s_1 g + s_2 h \bmod J_{L,d}.
\end{align*}
\end{proof}
