...
\todo{combine below two paragraphs with the corresponding
near-duplicate part from the background}
...
\]
\todo{Peter: Would it be better to say $Div(\sx)=(Div(X))[\frac{1}{e_i}P_i]_{1\le i\le r}$ or something like that? Aaron: I agree what is written isn't actually correct, maybe we should say the free direct sum over points not in the $P_i$, together with the fractional points. It's not a localization because we don't want to invert $P_i$.}
...
O'Dorney gives a larger bound in terms of least common multiples of
the denominators when the support of $D$ is allowed to
...
tight for arbitrary $\BQ$-divisors on $\BP^1$, but restricting
...
\cite[Chapter 5]{vzb:stacky}. \todo{again, not quite viewing as a scheme}
...
If $R$ is a graded ring, then we let $(R)_d$ refer to the $d^{th}$
...
\todo{Aaron: Doesn't \cite{vzb:stacky} put the $y_i$ before the $x_i$? Shouldn't we match this convention (here and anywhere we use this in the paper later?}
...
%in \cite{vzb:stacky}, Voight and Zureick-Brown may assume that $\{P_i\} \cap \Supp(K_X) = \emptyset.$ 
	we cannot assume that $\{P_i\} \cap \Supp(L_X') = \emptyset$, as $H^0(X, L_X')$ may have no global sections. We circumvent this issue by working with the orders of zeros and poles relative to $L_X'$, rather than relative to the $\sco_X$.\todo{Peter: This last sentence is kind of awkward.  I changed ``instead of relative to the structure sheaf'' to ``rather than relative to the structure sheaf,'' but it can still be made to sound cleaner... I hope... Aaron: I changed it. Is it better now?}
...
0,
		...
		generate $R$ over $R'$.
	\item[(c)] Endow $k[y] := k[y_{i, e_i}]_{i \in J}$ and $k[x]:=k[x_1,\ldots, x_m]$ with graded monomial orders and give $k[y, x] = k[y] \otimes k[x]$ block order. Let $R = k[y, x]/I.$ Then,
		\begin{align*}
...
\todo{Add conclusion to section. Aaron: Should we have conclusions?}
...
\begin{align*}
	...
	&= \dim_k (k[x_{3, 1}, x_{5, 1}, x_{7, 1}, x_{7, 2}] / \langle
	x_{3, 1} x_{7, 2}, x_{7, 2}^2 \rangle
\end{align*}
