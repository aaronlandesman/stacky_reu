...
O'Dorney's result \todo{Aaron: Which result? If anything, wouldn't this be a strengthening of his bounds from Theorem 8, instead of an extension? I guess it is an extension in that it generalizes to higher genus? Cite the theorem number you are talking about} on $\BP^1_k$ to divisors $D = \sum_i \frac{\alpha_i
}{\beta_i} P_i$ consisting of arbitrarily many points satisfying $H^
0(X, \lfloor D \rfloor) > 0$, showing that they \todo{What are generated?} are generated in
...
same Riemann-Roch arguments from $\BP^1_k$ in arbitrary genus,
...
$u, x_1, \ldots, x_m$ with $\ord_P(x_i)=0$ for all $x_i$, $\ord_P(u) = 0$, \todo{Aaron: Instead of having $\deg u = 0$, don't we only need that $\ord_P(u) = \lfloor \halfcan'|_P \rfloor$?} and $\deg(u)= 1$ when viewed as sections of $\so_{\halfcan'}$.  Suppose $u, x_1, \ldots, x_m$ generate $R_{\halfcan'}$ in degree at most $\bar{d}$ with relations generated
...
In particular, if $\frac{\alpha}{\beta}\le 1$, then $I$ is minimally \todo{Aaron: Should the word minimally be removed here?} generated by elements with leading terms of the form $y_i x_j$ and $y_i y_j$.
...
Before presenting a proof of this, we first prove a lemma that will allow parts $(b)$ and $(c)$ to immediately follow from an appropriate construction of generators in part $(a)$. \todo{Would it make sense to put this lemma before the previous theorem, instead of after the statement?}

\begin{lem}\label{lem:relations_from_generators_induction} 
Suppose $\halfcan$ and $\halfcan'$ are $\BQ$ divisors on a curve $X$ with $D=D'+\frac{\alpha}{\beta}P$.  Suppose $R_{\halfcan'}$ is generated by $x_1, \ldots, x_m$ and $R_{\halfcan}$ is minimally generated by $y_1, \ldots, y_n$ over $R_{\halfcan'}$; specifically, suppose there is a $k$-basis $S$ for $R_\halfcan$ over $R_{\halfcan'}$ such that $S$ consists only of monomials in $x_1, \ldots, x_m, y_1, \ldots, y_n$ and each $m\in S$ is not contained in $R_{\halfcan'}$.  
Let $I$ and $I'$ be the ideals of relations of $R_{\halfcan'}$ and $R_{\halfcan}$, for the given generating sets.  
Let $T\subseteq R_\halfcan - (S \cup R_{\halfcan'})$, such that any monomial of $(k[x_1, \ldots, x_m, y_1, \ldots, y_n])_{\ge 2}$ (the degrees at least 2 with respect to the regrading of each $x_1, \ldots, x_m, y_1, \ldots, y_n$) is either in $S$, $\langle T\rangle$, or $I' k[x_1, \ldots, x_m, y_1, \ldots, y_n]$.  Moreover, suppose there is some ordering $\prec$ such that if $T\prec S\prec k[x_1, \ldots, x_n]$ (that is, all elements of $T$ precede elements of $S$ which proceed monomials only in the $x_i$s). , then 
\begin{align*}
...
\begin{align*}
	\initial(I) & \supseteq \initial(I') k[x_1, \ldots, x_m, y_1, \ldots, y_n]\\
		     & + \langle T \rangle.
\end{align*}
Since every monomial of $k[x_1, \ldots, x_m, y_1, \ldots, y_n]$ either lies in $\langle T\rangle + \initial_\prec(I') k[x_1, \ldots, x_m, y_1, \ldots, y_n]$ or $S$, 
\begin{align*}
	\initial(I) & = \initial(I') k[x_1, \ldots, x_m, y_1, \ldots, y_n]\\
		     & + \langle T \rangle.
\end{align*}
...
\begin{proof}[Proof of Lemma \ref{lem:sat-1}]
By
...
For any $j < i$, $\frac{c_i}{d_i} > \frac{c_j}{d_j}$ \todo{Where do the $z_i$ live? How many $z_i$ are there, what indexing sets are the product and sums taken over?}
...
Then, since $X \not \cong \BP^1_k,$ and $\deg L = 1$, we must have $H^0(X,L) = 1$. Hence, we know $L$ is effective, so we must have $L = P$ where $P$ is hyperelliptic fi xed. In this case, it is shown in \cite{neves:halfcan}[Theorem III.19] that $R_L$ is generated in degree $\leq 3$.
...
In the case of a smooth genus 1 curve, $X$, we know $K_X = 0$, and therefore the only possibilities for half canonical divisors are $\halfcan' = 0$ or $\halfcan' = P - Q$ where $P,Q$ are distinct points, fi xed under the hyperelliptic involution. We aim to inductively construct presentations by increasing the values of the $e_i$ using Lemma ~\ref{lem:raise-stacky-order} and add additional points using Lemma ~\ref{lem:sat-1} and Lemma ~\ref{lem:sat-2}.
...
\todo{fi x this to accomodate the possibility that $P \in supp(L')$, this has already been done suppose ~\ref{lem:sat-1} accommodates it. }
...
To complete the case that $\Delta > 0$, we only need show the theorem holds for $L$, assuming it holds for $L_X$. By induction, it suffices to show that if the theorem holds for a divisor $L'$ with $\deg \lfloor L' \rfloor > 0$, then it holds for $L' + \subhalf {e_i}P_i,$ with $e_i$ odd. Similarly to the above, if $P$ is not a basepoint of $L'$ then the theorem holds for $L' + \subhalf {e_i}P_i$ by emma ~\ref{lem:sat-1}, while if $P$ is a basepoint of $L'$ then the theorem holds for $L' + \subhalf {e_i}P_i$ by Lemma ~\ref{lem:sat-2}.
...
There are two main cases: when $\delta \geq 2$ and when $\delta = 0$
 (note that $\delta$ must be even because $\halfcan_X \in \di X$ with $2\halfcan_X \sim K_X +\Delta$, where $\deg K_X$ is even). \todo{make a remark about this somewhere in the begining and reference it} In the first case, $\deg \lfloor \halfcan \rfloor
...
and $7 < d \leq 24$. \todo{Should this be 25?} Note that we can again use Max Noether's theorem
