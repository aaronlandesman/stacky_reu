...
\todo{Make sure we want to have linear equivalence $\sim$ of divisors instead of $=$ in most places}
\todo{Write all block orders as tensors so we don't need to say the $y_i$ precede the $x_i$.}
\todo{It's a bit strange how many tables are split across pages. Can we we make sure they each lie on a single page. Robin: do this at the end, not yet}
%%%%%%%%%%%%%%%%%%%%%%%%%%%% Introduction %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Introduction}
Let $\Gamma$ be a {\bf Fuchsian} group, i.e. a discrete subgroup of
$PSL_2(\BR)$ acting on the upper half plane $\BH$ by fractional
linear transformations. \todo{Peter: In the abstract we say finite area.  Don't we want that here as well?}  
We consider the graded {\bf ring of modular forms}
$M(\Gamma) = \bigoplus_{k = 0}^\infty M_k(\Gamma)$ \todo{Peter: Is $k$ bad to use here, since we will implicitly later use $k$ as the base field? Robin: Perhaps, although modular forms are traditionally referred to as having weight $k$. Not sure on this one.}. One of the best
...
cusps, with associated divisor of cusps $\Delta$.\todo{Peter: It is weird to have our first discussion of divisors be about the divisors of cusps.  Shouldn't canonical or half canonical divisors be introduced first? Robin: But this is where the log divisor comes from!}
...
\todo{Robin: I changed this back because the new version seemed a bit
long and the period is perfectly acceptable here. the new version
seemed oddly worded to me}
...
\cong \proj R$ \todo{Peter: I'm confused.  Why is this only true when $g\geq 2$? Robin: see statement of Petri's theorem (which actually usually assumes $g \geq 4$)...}. When $g \geq 2$, Petri's theorem gives generators
...
\cite[Corollary 10.4.6]{vzb:stacky}. \todo{Peter: Seems a bit misleading by misrepresenting VZB's work; in reality they found fairly tight bounds in the case they worked in. Robin: How? This solely talks about their work for the log spin canonical rings which is clearly NOT tight. They give a larger bound than us for a subset of the cases we check...}
...
In fact, the proof of Theorem ~\ref{thm:main} holds not only for log spin canonical divisors, which must have distinct points, but also for arbitrary effective divisors
$\Delta$. \todo{Peter: $\Delta$ seems weird as the name of the divisor, since its typically reserved for log divisors.  Also, how is this true?  This only follows given sufficient base point freeness conditions.  It is true when $g=0$ if that's what you meant to say. Robin: Throughout VZB, for example, $\Delta$ is used to represent any effective divisor. Where does an assumption that $\Delta$ has coefficient $1$ come into play?} Furthermore, one may relax the assumption that $k$ is perfect. Instead, one only need assume that the stacky curve
is separably rooted, i.e. the residue field at each stacky point
is separable.
\end{rem}
\todo{Peter: This remark seems to say two very separate things.  Why are they in the same remark? Robin: This remark describes how the weaker conditions for which the theorem also holds.}

Theorem ~\ref{thm:main} is proven separately in the cases that the genus $g = 0, g = 1,$ and $g \geq 2$ in Theorems ~\ref{thm:g-0-main}, ~\ref{thm:g-1-main}, and ~\ref{thm:g-high-main}, respectively.  In each of these proofs, we follow a similar inductive process utilizing the lemmas of Section ~\ref{sec:induction}; however, in the first two we have to explicitly construct specific base cases and present a finite list of exceptional cases, while in the high genus case we deduce base cases from more general arguments.
\begin{rem}
...
\cite[Section III.4]{neves:halfcan}.  \todo{Peter: The statement about the high genus case seems a bit unclear/misleading Robin: how so}

Furthermore, explicit constructions also reveals that the bounds given in Theorem ~\ref{thm:main} are tight.
...
\end{rem}  \todo{Peter: Again, why are these two sections of the remark grouped together? Robin: Isn't this remark describing whether the theorem gives explicit constructions and tight bounds? Those seem pretty related...}

%The main theorem of this paper, Theorem ~\ref{thm:main}, combined
%with a lemma of Voight and Zureick-Brown \cite[Lemma 10.2.1]
%{vzb:stacky}, provides a statement on all rings of modular forms.
Together with a theorem from Voight and Zureick-Brown \cite
[Theorem 9.3.1]{vzb:stacky} 
%and \cite[Theorem 9.3.1]{vzb:stacky}, 
\todo{Peter: The paragraph used to be confusingly redundant.  I think I fixed it but read through the commented out stuff to make sure it says the correct thing. Robin: Why are you ignoring Lemma 10.2.1 as well as its connection to Kodaira--Spencer?}
...
generated in degrees at most $6 \cdot \max(3, e_1, \ldots, e_r)$.\todo{Peter: Are you sure it shouldn't be $6e$ and $12e$, because he showed the {\emph canonical ring} has the bounds $3e$ and $6e$, which would mean the {\emph half-canonical} ring should have bounds $6e$ and $12e$ right? Robin: I think you are correct, although don't use $e$ since it is max(3, ...) instead of max(5, ...)}
...
of the stacky points is separable.  \todo{Peter: We should clarify this last statement.  If we are assuming $k$ is algebraically closed, aren't the points automatically separably-rooted?  Really aren't we saying if perfect, then restrict to algebraically closed, and if not, everything works similarly as long as separably rooted? Robin: Yes I guess $k$ being algebraically closed implies perfectness... we can use a similar statement to Remark 5.5.7 from VZB}

With this stacks-theoretic description in mind, the remainder of this paper is primarily phrased using the language
of Weil divisors.
\end{rem}
...
Let $\sx$ be a stacky curve over $k$ with coarse space $X$ \todo{We haven't defined a course space yet.  Confusingly, this definition is labeled ``defn:course-space'' which made me think it was supposed to.} and stacky points $P_1, \ldots,
...
sum of distinct points each with trivial stabilizer. \todo{Peter: It isn't necessary in the proof that these log points be non-stacky, even though only such log divisors arise from modular forms.  Need we say that?  If so, then we should explicitly note that these are the only cases that arise.} A divisor
...
canonical divisor, or theta characteristic) \todo{Peter: I think we mismatched the parenthetical.  Also, why did we define the spin canonical divisor of $\halfcan_X$ especially.  Isn't it really just a special case of the definition before. Robin: How? the parenthetical encompasses all alternate names! Yes it's a special case, but we never give it a name otherwise. You could define it as the log spin canonical divisor with zero log divisor but that somehow doesn't seem to give the right picture.} associated to the
...
Now, we check the admissibility of $J= \{2, 3\}$ (corresponding to the $e_2,e_3=7$). \todo{Robin: I don't understand your edit, Peter. Admissibility is not a property of $J$.} By relabelling the
...
\end{align*}\todo{Peter: I think you need additional degree subscripts added to the polynomial ring or something, so this isn't infinite. Robin: Aren't there already existing degree subscripts?}
...
Let $(\sx', 0, L')$ be a log spin curve of genus 1 so that $L' = P -
 Q + \frac{2}{5} P_1.$ Let $u \in (R_L)_1$ be any nonzero element \todo{Peter: I'm very confused.  There are no elements in degree 1...  Oh, I see, I think you mean $L'=\frac{2}{5}P_1$ (without the $P-Q$) maybe?}.
...
\todo{Say something like a monomial is in $S$ if and only if it satisfies some condition with $c_h,$ and that we use this to check the three (*) conditions.}
...
%\end{lem}
%\begin{proof}
%To show \ref{custom:Ad-i} and \ref{custom:Ad-ii} look at some $\sx'$ with $r- 1$ stacky points $'s=\{Q_1, \ldots, Q_{j - 1}, Q_{j + 1}, \ldots Q_r\}$ including a chosen $Q_i$ (and excluding some $Q_j\ne Q_i$). Then select $y_{e_r'}$ from the choice that makes the set $J'=\{i\}$ an admissible set in $\sx'$; this naturally can be seen inside of $R_{\sx}$ and satisfies \ref{custom:Ad-i}. Furthermore, since $s$ is an admissible set of $L'$, in particular, we must have \ref{custom:Ad-ii} satisfied so $\frac{-\ord_{Q_l}
...
%\item When $g= 0$, since $L'$ has a nontrivial admissible subset within $\{e_1, \ldots, e_{r- 1}\}$ satisfying \ref{custom:Ad-i}, we know $\deg(\lfloor 3\lfloor)\ge 0$, so it is base-point free.
%\end{itemize}
%\end{proof}
...
%Then, since $X \not \cong \BP^1_k,$ and $\deg L = 1$, we must have $H^0(X, \halfcan) = 1$. Hence, we know $L$ is effective, so up to linear equivalence, we may assume $L = P$ where $P$ is hyperelliptic fixed. In this case, it is shown in \cite{neves:halfcan}[Theorem III.19] that $R_\halfcan$ is generated in degree $\leq 3$.
%
...
We first now show the theorem holds for $L_X$ by induction. Since $L_X$ is effective, we may induct on the degree of the divisor. The base case is easy: the theorem holds for $L_X' = 0$ by Example ~\ref{eg:base-1-0}. Assume it holds for $L_X' \in \di X$, with $L_X'$ effective. We will show it holds for $L_X' + P,$ verifying the inductive step. There are two cases, depending on whether $P$ is a basepoint of $L_X'$.
...
for $L' = P - Q + \sum_{i = 1}^{r}\subhalf {e_i}P_i$, as desired. \todo{Peter: Why is it immediate that $n$ threes satisfies the condition for admissibility?  We got rid of that proof from earlier.  In fact, didn't we find it to be false? Aaron: I don't think so, see Remark ~\ref{rem:three-cases}.}

\vskip.1in
\noindent
{\sf Case 2b: $\halfcan_X = 0$}

This case is analogous to 2a: If there is only one stacky point, we start at $L = \frac{3}{7}P_1,$ and inductively increment the stabilizer order. Note that by Table ~\ref{table:g-1-base}, $L = \frac{3}{7}P_1,$ will have a relation in degree $15.$ However, once $e_1 \geq 9$, we have $2 \cdot e_1 > 15$, so the theorem holds for such stacky curves. Once the divisor as at least two stacky points, the argument proceeds as in Case 2a.

\todo{Peter: Maybe you should mention something about how the exceptional cases differ, and how this affects the argument? Aaron: Ok, I did it imprecisely. Is this better?}
\end{proof}
...
\end{longtable}
\todo{Peter: Can we make a better description for the table above? Robin: Think of one then.}
...
\begin{proof}
\todo{Robin: I changed much of this back to the old wording. Also
please don't randomly change three of the $\halfcan$'s to $\halfcan_X$'s
when the latter is not used anywhere else in the section...}
...
{vzb:stacky}. When $n = -1$, then $R_\halfcan = k$.  When $n = 0$,
then $R_\halfcan = k[x]$.  When $n > 0$, inductively applying Lemma
...
trivial case where $R(\sx, \Delta, \halfcan) = k$.
...
\begin{enumerate}
	\item As noted in Remark ~\ref{rem:explicit-generators}, the 
		...
		algorithm for inductively determining the generators of $L$. 
		...
		\label{ques:spin-canonical-petri}
			...
			{vzb:stacky}?
		\end{question}
\todo{Aaron: Peter, you said you wanted to take out the question on generic ideals, but I think it might be interesting to think about. Why do you want to remove it?}	
...
\item One direction for further research is to 
		...
		^{e_3} = 0$.  \todo{Peter: I don't know if it is so worth while giving an explicit definition and precisely defining the work already done.  A general outline might suffice and be easier/shorter to read. Aaron: I have a feeling we should define it, but I'm ok with removing the last two sentences.}

		This motivates the following question:

\todo{Peter: Would this be better as a conjecture? Aaron: I don't have an actual conjecture. I'm not sure what the bounds would be.}
		\begin{question}
	...
	\item Voight and Zureick Brown define
		...
		\end{question}\todo{Peter: This whole question is kind of confusing.  I don't know what it is trying to say. Robin: Maybe preface the question with saying something like ``not just the initial ideal''}
	...
	\item In Subsection \ref{ssec:bounds-high-genus}, we reference 
		...
		\end{question}
		\todo{Peter: I don't like a finite list of exceptions.  The list contains infinitely many non-isomorphic stacky curves.  Finite family might also be imperfect.  Can we say except when $g=2$ (I believe there will be infinitely many counterexamples there) or it lies in a finite list of exception?  Do we believe that to be true? Aaron: Since we haven't really thought about this conjecture, I'm not comfortable making it a conjecture. I switched it back to a question. Is that ok? Robin: Perhaps give reference to Reid's characterizations?}
