...
--Roch, $\dim_k (P_L)_1 = 1.$ Let $y_3 \in (R_L)_3$ be any nonzero
...
let $y_5 \in (R_L)_5$ be an element with $y_5|_{P_1} = -P_1$. Then,
we claim there is some $\alpha \in k$ so that

$$R_L \cong k[u, y_3, y_5]/(y_3^4 - \alpha u y_5^2).$$
...
\span(s_1, s_2) \subseteq H^0(K)$ is basepoint-free. Then, the map
\begin{align*}
...
\end{align*}
is surjective. Indeed, since $V = \span(s_1, s_2) \subseteq
...
We wish to show $f$ is surjective. Note that $\dim_k \ker f = \dim_
k H^0((n-2)L) = (n-3)(g - 1)$ using Riemann--Roch and the
assumption $n \geq 4.$ Additionally, $\dim_k V \otimes H^0(nL) = 2
\cdot (n - 1)(g - 1)$, again using Riemann--Roch. By rank-nullity,
$$\dim_k \im f = 2 \cdot (n - 1)(g - 1) - (n - 3)(g - 1) = (n + 1)(g - 1) =
\dim_k H^0((n + 2)L)$$. Ergo, $f$ is surjective.
\end{proof}
...
, x_n$ of $R_L$ so that we obtain a surjection $\phi: k[x_1, \ldots
, x_n] \twoheadrightarrow R_L$ with kernel $I_L$. Let $I_{L, d}$ be
the $d$^\text{th} graded piece of $I_L$ and define
\[
...
Let $(X, \Delta, L)$ be a log spin curve of genus $g \geq 2,$ so
that $\Delta > 0.$ Choose generators $x_1, \ldots, x_n$ of $R_L$ so
that we obtain a surjection $\phi: k[x_1, \ldots, x_n]
\twoheadrightarrow R_L$ with kernel $I_L$. Let $s_1, s_2 \in k[x_1,
\ldots, x_n]_2$ be two elements so that $\phi(s_1) k$ $\oplus$
$\phi(s_2) k = V \subseteq H^0(X, 2L)$ is basepoint-free. For any $f \in
k[x_1, \ldots, x_n]$ such that $\deg f \leq 11,$ there exist $g, h \in
k[x_1, \ldots, x_n]_{d - 2}$ so that $s_1 g + s_2 h = f \bmod J_{L, d}.$
\end{lem}
\begin{proof}
...
Let $(X, \Delta, L)$ be a log spin curve of signature $(g; -; \delta)$
 with $g \geq 2$ and $\Delta > 0.$ Then $I_L$ is generated in
degree at most degree at most $10$.
\end{prop}
\begin{proof}
Suppose $f \in I_L$ with $\deg f \geq 11$. To complete the proof,
it suffices to show $f \in I_{L,d}'$. By Lemma
~\ref{lem:reducing-degree}, it suffices to show $s_1 g + s_2 h \in
I_{L, d}'$ where $\phi(s_ 1),\phi(s_2) \in H^0(X, 2L)$ are two sections
so that $\span(\phi(s_ 1), \phi(s_2)) = V \subseteq H^0(K)$ is
