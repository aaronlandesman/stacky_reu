...
algebraic category. \todo{mention that we will use the language
of Weil divisors?}
...
algebraically closed. We also assume that the stacky curves are
separably rooted, i.e. the residue field of each stacky points
is separable. The language of stacks will be omitted in
...
\cite[Chapter 5]{vzb:stacky}. \todo{again, not quite viewing as a
scheme}
...
For a homogeneous polynomial $f \in k[x]_{\vec{a}}$, we define the \textbf{initial term} $\initial(f)$ to be the monomial in the support of $f$ that is largest with respect to $\prec$, and we define $\initial(0)=0$.  Let $I \subseteq k[x]_{\vec{a}}$ be a homogeneous ideal.  We define the \textbf{initial ideal} $\initial(I)$ to be the ideal generated by $\{\initial(f) : f \in I\}$.  A \textbf{Gr\"obner basis} (also called a \textbf{standard basis}) for $I$ is a set $\{f_1,\dots,f_n\} \subset I$ such that 
+\[ \langle \initial(f_1),\dots,\initial(f_n) \rangle = \initial(I). \]
...
\subsection{Junk}
\todo{add this back into the intro}
...
By equation \label{eqn:deg1-sat-ind} for $d\in \mathbb{N}$,
\[
	h^0(X, \lfloor{d\halfcan}\rfloor) = h^0(X,\lfloor d\halfcan'
...
curves, with the same coarse space $X$ and stacky points $Q_1, \ldots, Q_r$. Let $J \subset
...
ideal of $I$ is
\[
	\initial(I) = \langle x_{3, 1} x_{7, 2}, x_{7, 2}^2 \rangle
\]
...
(k[x_{3, 1}, x_{5, 1}, x_{7, 1}, x_{7, 2}] / \initial(I))_d$ for 
all degrees $d \geq 0$. In particular, 

\begin{align*}
	\dim_k (k[x_{3, 1}, x_{5, 1}, x_{7, 1}, x_{7, 2}] / I)_d &=
<<<<<<< HEAD
	\dim_k (k[x_{3, 1}, x_{5, 1}, x_{7, 1}, x_{7, 2}]	/ \initial(I))_d \\
...
>>>>>>> 2abcb15b64c612b3ce3eacb6dff8768ce7c2527f
\end{align*}
