...
\section{High genus}
\label{sec:g_high}
We now consider the case when the genus is at least 2. In this case, we are able to bound the degrees and relations of the half canonical ring $R_L$. In this section, we do not obtain explicit presentations of $R_L.$ In contrast, in Sections ~\ref{sec:g_1} and ~\ref{sec:g_0_effective} we will not only obtain bounds on the generators and relations, but also obtain presentations inductively. The tradeoff is that in the cases of genus 0 and 1, we will have to explicitly deal with base cases, making the induction more involved. We begin with the case that $g \geq 2$.
...
Let $\sx$ be a tame, seperably rooted stacky curve of genus 1. Let $P,Q$ are distinct hyperelliptic fixed points, and $R_1,\ldots, R_n$ be points on $\sx$. In this section, will compute the canonical ring $R_{\halfcan'}$ in the cases that $\sigma = (1;-;0),\sigma = (1;e;0)$. That is, we will consider the cases.
\begin{enumerate}
	\item $\halfcan' = P-Q$
	\item $\halfcan' = P - Q + \frac{1}{3}R$
	\item $\halfcan' = P - Q + \frac{1}{5}R$
	\item $\halfcan' = P - Q + \frac{e-1}{2e}R, e > 5, e \equiv 1 \bmod 2$ 
\end{enumerate}
\todo{make table}
...
Let $L = P - Q + \frac{2}{5}R_1.$ Here, we 
\todo{say what the variables are}
$R_L \cong k[u,y_3, y_5]/(y_3^4 - \alpha uy_5^2),$ where we define $I_5 = y_3^4 - uy_5^2$ and $I_e$ is inductively defied via Theorem ~\ref{thm:ram_order_ind} so that $in(I_e) = in(I_{e-2})k[u, y_3,\ldots, y_e] + \langle y_e z \mid z \neq y_e, y_{e-2} \rangle.$
