...
\begin{enumerate}
	\item  $(\sx',J)$ is admissible with generators $x_j, y_{i, e_i'} \in R' = R_{L'}$ with $i \in J$ as in Definition ~\ref{defn:admissible}
...
0,
	...
	\item[(d)] For any $W \subset J,$ the set $(\sx,W)$ is admissible.
\end{enumerate}
\end{thm}

\begin{proof}
We will only prove this in the case (1), that $(\sx', L', J)$ is admissible, as the case (2) that $\sigma$ is as in Remark ~\ref{rem:three-cases} is completely analogous.
...
Finally, to check (d), we will first show that $(\sx, J)$ is
