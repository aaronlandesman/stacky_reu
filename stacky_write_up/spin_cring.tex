...
\cong \proj R$ \todo{Peter: I'm confused.  Why is this only true when $g\geq 2$? Robin: see statement of Petri's theorem (which actually usually assumes $g \geq 4$)... Aaron: You need genus at least 2 for $\Omega_X$ to be ample. There are mild generalizations of Petri's theorem, for example those in vzb, which also compute generators and relations for hyperelliptic curves, and curves of genus 3.}. When $g \geq 2$, Petri's theorem gives generators
...
\todo{Peter: Seems a bit misleading by misrepresenting VZB's work; in reality they found fairly tight bounds in the case they worked in. Robin: How? This solely talks about their work for the log spin canonical rings which is clearly NOT tight. They give a larger bound than us for a subset of the cases we check... Aaron: I think the point is that if they put in some more work, they could have reduced the bounds. However, I agree with Robin for two reasons: First, they didn't put in the work to get the tighter bounds. Second, even with a small amount of work, they would have only gotten bounds which are twice ours, as their grading is off from ours by a factor of two.}
...
\todo{Peter: It isn't necessary in the proof that these log points be non-stacky, even though only such log divisors arise from modular forms.  Need we say that?  If so, then we should explicitly note that these are the only cases that arise. Aaron: I don't think we need to say it, the application to modular forms still goes through whether or not we want to assume they are distinct. I like keeping log divisors like this so that we parallel \cite{vzb:stacky}} 
A divisor
