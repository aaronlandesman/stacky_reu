...
\todo{It's a bit strange how many tables are split across pages. Can we we make sure they each lie on a single page. Robin: do this at the end, not yet. Aaron: I changed our long tables to tables. Why did we use longtables at all? Our tables are all pretty short.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%% Introduction %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$M(\Gamma) = \bigoplus_{k = 0}^\infty M_k(\Gamma)$ \todo{Peter: Is $k$ bad to use here, since we will implicitly later use $k$ as the base field? Robin: Perhaps, although modular forms are traditionally referred to as having weight $k$. Not sure on this one. Aaron: I do not think this will confuse anyone.}. One of the best
...
rings, Voight and Zureick-Brown bounded generator degrees by $6 \cdot \max(e_1, \ldots, e_r)$
and relation degrees by $12 \cdot \max(e_1, \ldots, e_r)$ when $\halfcan$ is effective
\cite[Corollary 10.4.6]{vzb:stacky}. Note that the bounds we write here differ from those stated in Voight and Zureick-Brown \cite[Corollary 10.4.6]{vzb:stacky} by a factor of 2 because their grading convention differs from ours by a factor of 2.
...
$\Delta$. 
\todo{Peter: $\Delta$ seems weird as the name of the divisor, since its typically reserved for log divisors.  Also, how is this true?  This only follows given sufficient base point freeness conditions.  It is true when $g=0$ if that's what you meant to say. Robin: Throughout VZB, for example, $\Delta$ is used to represent any effective divisor. Where does an assumption that $\Delta$ has coefficient $1$ come into play?  Peter: All I meant with the first comment was that $D$ might make more sense then $\Delta$, because this divisor does not arise in a natural way purely from the log divisor.  On the other hand, it doesn't really matter.}
\todo{Peter: As for the second comment I made above, when $g>0$, we don't have the necessary base point freeness condition to apply this lemma on arbitrary divisors.} Furthermore, one may relax the assumption that $k$ is perfect. Instead, one only need assume that the stacky curve
...
relations. Furthermore, our proof of the genus $g \geq 2$ case also
...
rings. Many explicit systems of generators and relations for curves of genus $2 \leq g \leq 15$ are
detailed in interesting examples by Neves
...
\item[(b)] Choose an ordering $\prec$ on $k[u, x_1, \ldots, x_m]$ such that
	...
	\begin{enumerate}
		...
		\item[\customlabel{custom:sat-1-***}{(***)}] there does not
			exist $r<i$ such that $\sum_{j=1}^r y_j^{a_j}> c_{r + 1}$.
	\end{enumerate}
...
We now check that $\prod_{j=1}^h y_j^{\gamma_j}\in U_{\sum_{j=1}^h b_j c_j}$, by checking conditions \ref{custom:sat-1-*}, \ref{custom:sat-1-**}, and \ref{custom:sat-1-***}.  
Notice that if $\prod_{j=1}^r y_j^{\omega_j}\in S$, then our definition of $S$ implies $y_h\prod_{j=1}^r y_j^{\omega_j}\in S$ if and only if $c_r$ is maximal among the subset of $c_1, \ldots c_n$ of elements not greater than than $c_r + \sum_{j=1}^r c_j \omega_j$.
Therefore, Since $\prod_{j=1}^{h-1} y_j^{b_j}\in S$ but $\prod_{j=1}^h y_j^{b_j}\not\in S$, $c_h$ must not be maximal (among $c_1, \ldots, c_n$) such that $c_h \le \sum_{j=1}^h b_jc_j$, which means $c_{h+1}\le \sum_{j=1}^h b_jc_j$, meaning that $\sum_{j=1}^h {y_j}^{\gamma_j}$ satisfies \ref{custom:sat-1-*}.  
Suppose we choose $\omega_1, \ldots, \omega_h$ such that $\omega_j\le \gamma_j$ for all $j$ and $\omega_l \le \gamma_l$ for at least 1 $l$.  Then, since for all $r< h$, we have $\prod_{j=1}^r {y_j}^{\gamma_j}\in S$, we also have $\prod_{j=1}^r {y_j}^{\omega_j}\in S$.  Furthermore, since $(\gamma_1, \ldots, \gamma_h)$ was chosen to be minimal to satisfy the previous condition and that $\prod_{j=1}^h {y_j}^{\gamma_j}\not\in S$, we have $\prod_{j=1}^h {y_j}^{\omega_j}\in S$.  Therefore, in particular $c_h$ is minimal among $c_1, \ldots c_n$ that is not greater than $\sum_{j=1}^h c_j \omega_j$, so in particular $\sum_{j=1}^h c_j\omega_j < c_{h+1}$; therefore, $\prod_{j=1}^h {y_j}^{\gamma_j}$ satisfies condition \ref{custom:sat-1-**}.  
Since for each $r<h$, we have $\prod_{j=1}^r y_{j}^{\gamma_j}\in S$ as note above, condition \ref{custom:sat-1-***} holds for $\prod_{j=1}^h y_j^{\gamma_j}$.  
...
\begin{lem}
\label{lem:sat-2}
Let $(\sx, \Delta, \halfcan)$ and $(\sx', \Delta, \halfcan')$ \todo{The notation i was using for these throughout the paper was $\halfcan$ and $\halfcan'$ without the $\sx$ subscripts. We should make one of them consistent with the other.} be log spin curves with the same coarse space $X = X'$ having signatures $(g; e_1, \ldots, e_r; \delta)$ and $(g, e_1, \ldots, e_{r- 1}, \delta)$, where $e_r=3$.  Suppose $g > 0,$ and, if $g = 1,$ then $\deg 3\halfcan' \geq 2.$ Then, by Riemann-Roch $\sat(Eff(L'))\le 2$. \todo{Why is it automatic that the saturation is 2? Maybe you can explain this in the proof in a sentence.}
...
\begin{proof}
First, note that when genus is at least we shall show the assumptions on $g$ imply $H^0(\sx, 3 \halfcan), H^0(\sx, 4 \halfcan)$ are both basepoint-free: If $g \geq 2$ then $\deg 3 \halfcan > 2g - 1$ \todo{If $g = 2$ then we in fact have $\deg 3L = 3g - 3 = 2g - 1.$ Is this a big problem? It's almost basepoint-free, except if the point you take out yields the canonical divisor, after removing it.  Peter:  Note that this is $L$ not $L'$ whose base point freeness we are talking about.  In particular $\deg(\lfloor 3L \rfloor)=\deg(\lfloor 3L'\rfloor)+1\ge 2g> 2g-1$.} and $\deg 4 \halfcan > 2g - 1$, so $H^0(\sx, 3 \halfcan)$ and $H^0(\sx, 4 \halfcan)$ are base point free. If $g = 1$, we assume $\deg 3 \halfcan \geq 2 > 2g - 1$, so we also have $\deg 4 \halfcan \geq 2 > 2g - 1,$ so again $H^0(\sx, 3 \halfcan)$ and $H^0(\sx, 4 \halfcan)$ are base point free. 
...
\ssec{Bounds on Generators and Relations in Genus $\geq 2$}
...
\ssec{Main Theorem for Genus $\geq 2$}
...
\todo{Peter: Why is it immediate that $n$ threes satisfies the condition for admissibility?  We got rid of that proof from earlier.  In fact, didn't we find it to be false? Aaron: I don't think so, see Remark ~\ref{rem:three-cases}.}
...
\begin{tabular}
	...
	\end{tabular}
	\caption{Generators and Relations for Genus 0 Non-effective Base Cases}
...
\end{tabular}	
	\caption{Checking \ref{custom:Ad-iii} for Genus 0 Non-Effective Base Cases}
...
\item One direction for further research is to 
		...
		^{e_3} = 0$.  
\todo{Peter: I don't know if it is so worth while giving an explicit definition and precisely defining the work already done.  A general outline might suffice and be easier/shorter to read. Aaron: I have a feeling we should define it, but I'm ok with removing the last two sentences.}
