...
relations generated in degrees up to $\max(10, 2e_1, \ldots, 2e_r)$
...
\begin{enumerate}
	...
	\item In Subsection \ref{ssec:bounds-high-genus}, we reference 
		...
		construction of a uniform bound on the generation of the half-
		canonical ring and its ideal of relations at 5 and 10 in the non-
		log, non-stacky case when genus is at least 2.  We extend this 
		bound of 5 and 10 to the log case, and then apply our inductive 
		lemmas to add stacky points and get bounds of $\max(5,e)$ and $\max(
		10, 2e)$.  Reid in fact proves something slightly stronger: that in 
		most cases his bound is actually 3 and 6 with well-characterized 
		exceptions.
		\begin{question}\label{ques:reduce-5,10-to-4,8}
			Can we reduce the bounds in Theorem \ref{thm:main} on 
			generation from $\max(5,e)$ and $\max(10, 2e)$ to $\max(4,e)$ and $
			\max(8, 2e)$. 
		\end{question}
		\begin{rem}\label{rem:reduce-3,6}
			It is unlikely that this can be reduced further, because Lemma 
