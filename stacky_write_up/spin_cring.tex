...
\todo{Write all block orders as tensors so we don't need to say the $y_i$ preceede the $x_i$.}
\todo{It's a bit strange how many tables are split across pages. Can we we make sure they each lie on a single page.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%% Introduction %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$M(\Gamma) = \bigoplus_{k = 0}^\infty M_k(\Gamma)$ \todo{Peter: Is $k$ bad to use here, since we will implicitly later use $k$ as the base field?}. One of the best
...
and Zureick-Brown give tight bounds \cite[Chapters 7-9]{vzb:stacky}.
 This paper \todo{Peter: I changed ``work to paper.'' Either way, it is a little awkward, because this could refer to VZB, or the one we are writing (of course I know it is the latter, but the reader won't)} extends their work to all Fuchsian groups $\Gamma$.

We can now consider the orbifold $\Gamma \backslash \BH$ over $\BC$.
For example, in the case $\Gamma$ acts freely on $\BH$, $\Gamma
\backslash \BH$ is a Riemann Surface over $\BC$.  $\Gamma \backslash \BH$ may be non-compact, but in such cases we can form a
compact Riemann Surface, $\Gamma \backslash \BH^*,$ by adding in
cusps, with associated divisor of cusps $\Delta$.\todo{Peter: It is weird to have our first discussion of divisors be about the divisors of cusps.  Shouldn't canonical or half canonical divisors be introduced first?}
...
generalization of the GAGA principle: there is an equivalence of categories between orbifold
curves and log stacky curves over $\BC$ \cite[Proposition 6.1.5]
{vzb:stacky}. Hence, for the remainder
...
\cong \proj R$ \todo{Peter: I'm confused.  Why is this only true when $g\geq 2$?}. When $g \geq 2$, Petri's theorem gives generators
...
\cite[Corollary 10.4.6]{vzb:stacky}. \todo{Peter: Seems a bit misleading by misrepresenting VZB's work; in reality they found fairly tight bounds in the case they worked in.}
...
is separable.
\end{rem}
\todo{Peter: This remark seems to say two very separate things.  Why are they in the same remark?}
...
\cite[Section III.4]{neves:halfcan}.  \todo{Peter: The statement about the high genus case seems a bit unclear/misleading}
...
\end{rem}  \todo{Peter: Again, why are these two sections of the remark grouped together?}
...
\todo{Peter: The paragraph used to be confusingly redundant.  I think I fixed it but read through the commented out stuff to make sure it says the correct thing.}
...
generated in degrees at most $6 \cdot \max(3, e_1, \ldots, e_r)$.\todo{Peter: Are you sure it shouldn't be $6e$ and $12e$, because he showed the {\emph canonical ring} has the bounds $3e$ and $6e$, which would mean the {\emph half-canonical} ring should have bounds $6e$ and $12e$ right?}
...
canonical divisor, or theta characteristic) \todo{Peter: I think we mismatched the parenthetical.  Also, why did we define the spin canonical rdivisor of $\halfcan_X$ especially.  Isn't it really just a special case of the definition before.} associated to the
...
Now, we check the admissibility of $J= \{2, 3\}$ (corresponding to the $e_2,e_3=7$). By relabelling the
...
\end{align*}\todo{Peter: I think you need additional degree subscripts added to the polynomial ring or something, so this isn't infinite.}
...
\todo{Peter: Maybe you should mention something about how the exceptional cases differ, and how this affects the argument? Ok, I did it imprecisely. Is this better?}
\end{proof}
...
n \cdot \infty$ with $n \in \BZ$.  When $n=-1$, then $R_\halfcan=k$.  When $n=0$, then $R_\halfcan = k[x]$.  When $n>0$, inductively applying Lemma \ref{lem:sat-1} tells us that $R_\halfcan$ is generated in degree 1 with relations generated in degree 2.  All three of these cases meet the conclusion of Theorem \ref{thm:g-0-main}.
...
\todo{Peter: Would this be better as a conjecture? Aaron: I don't have an actual conjecture. I'm not sure what the bounds would be.}
	...
	\item Voight and Zureick Brown define
		...
		\end{question}\todo{Peter: This whole question is kind of confusing.  I don't know what it is trying to say.}
	...
	\item In Subsection \ref{ssec:bounds-high-genus}, we reference 
		...
		\end{question}
		\todo{Peter: I don't like a finite list of exceptions.  The list contains infinitely many non-isomorphic stacky curves.  Finite family might also be imperfect.  Can we say except when $g=2$ (I believe there will be infinitely many counterexamples there) or it lies in a finite list of exception?  Do we believe that to be true? Aaron: Since we haven't really thought about this conjecture, I'm not comfortable making it a conjecture. I switched it back to a question. Is that ok?}
