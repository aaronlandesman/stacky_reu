...
Fix a genus $g$, a log divisor $\Delta$, and a coarse space $X$.  Suppose $3\le r\in \mathbf{N}$ so that all stacky curves $\sx'$ with $r-1$ ramified points of degree 3 over the coarse space $X$ have all subsets $J'$ admissible in $L'$.  Then if $\sx$ has coarse space $X$, any subset of $J$ of $\{e_1, \ldots, e_r\}$ (with all $e_i=3$) is admissible viewed in $L$.
\end{lem}
...
\end{enumerate}
\todo{Is it ok that two genus 1 base cases are also exceptional, in terms of their relation degree?}
...
Let $L' = \frac{2}{5}R_1$ be a half canonical divisor on a stacky curve $\sx$. Let $u \in (R_L)_1$ be any nonzero element. We obtain $\di u|_{R_1} = 0,$ since $2P -2Q \sim 0$ and by Riemann--Roch $\dim_k (R_L)_1 = 1.$ Let $y_3 \in (R_L)_3$ be any nonzero element. We obtain $\di y_3|_{R_1} = - R_1,$ by Riemann Roch, since if $y_3$ did not have a pole at $R$, we would obtain $y_3 \in H^0(\sx,3P-3Q) \cong H^0(\sx,P-Q) \cong 0$ as $P \neq Q$. Finally, let $y_5 \in (R_L)_5$ be an element with $y_5|_{R_1} = -R_1$. Then, we claim there is some $\alpha \in k$ so that
$R_L \cong k[u,y_3, y_5]/(y_3^4 - \alpha uy_5^2).$
The proof of this can be gotten by first using Riemann--Roch and counting the pole order at $R_1$ considerations to check the above relation exists, and that the generators and relation determine a ring whose $k$th graded piece has the same dimension of $R_L$. It is analogous to Example ~\ref{ex:base-377}, and is omittied.

Note that $L'$ is admissible, as can be checked in a fashion analogous to the verification in Example ~\ref{eg:base-1-33}.

Finally, note that this particular case is also exceptional, because there is a relation in degree $12 > 2 \cdot 5$, but once $e' = 5$ is incremented to $e = e'+2 = 7,$ using Theorem ~\ref{thm:ram_order_ind}, this relation in degree 12 caries over, and so the case $L' = P - Q + \frac{3}{7}R_1$ is not exceptional because $12 < 2 \cdot 7$.
\end{example}


\iffalse
The base case of $L' = \frac{3}{7}R_1$ has $R_L \cong k[u,y_5,y_7]/(y_5)^3 - uy_7^2.$ 
\fi

The base case of $L' = \frac{3}{7}R_1$ can be similarly computed. Note this is yet another exceptional case.
