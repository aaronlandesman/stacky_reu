...
\todo{Peter: I think we need something about odd weight modular 
forms before going right into stacks.}
Following Voight and Zureick-Brown \cite{vzb:stacky}, we generalize 
Petri's theorem in many \todo{Peter: not clear to me that ``many'' 
is a good word to use here.  We mostly generalize in one direction}
directions. 
...
\geq 2}$ if 
\[
	\di \sx = \di X \oplus \left(\bigoplus_{i = 1}^r \left\langle 
	\frac{1}{e_i}P_i \right\rangle \right) \subset \BQ \otimes \di X.
\]
\todo{Peter: Would it be better to say $Div(\sx)=(Div(X))[\frac{1}{e_i}P_i]_{1\le i\le r}$ or something like that?}
As is done in Voight and Zureick-Brown \cite{vzb:stacky} stacky curves may be formalized as stacks, but for the purposes of this paper, it suffices to view $\sx$ as a scheme whose associated divisor group allows a finite number of stacky points. \todo{I don't know if this is a good statement to make.  We aren't really viewing $\sx$ as a scheme.  Rather we're are determining information about its canonical rings through knowledge of scheme data.}  A {\bf log spin curve} is a triple $(\sx, \Delta, \halfcan)$ where $\Delta \in \di X$ is an effective divisor and $2\halfcan = K_X + \Delta + \sum_{i=1}^{r}\subhalf{e_i}P_i.$
...
graded ring and its relations are generated.  O'Dorney
shows that the canonical ring of any $\BQ$-divisor $D =
\frac{\alpha_1}{\beta_1}P_1 + \frac{\alpha_2}{\beta_2}P_2$ of
$\BP_k^1$ is generated in degrees up
to $e := \max(\beta_1, \beta_2)$ with relations generated in
degrees up to $2e$ \cite[Theorem 6]{dorney:canonical}. Furthermore,
O'Dorney gives a larger bound in terms of least common multiples of the denominators when the support of $D$ is allowed to
have arbitrarily many points.
...
\todo{do this part}
Part on Kodaira--Spencer \cite{kodaira-spencer:deformations}.
...
\begin{equation}
	\deg \alpha := \sum_{i = 1}^{n} d f_i  > \sum_{i = 1}^{n} d f_i' =: \deg \beta
\end{equation}
...
usually have an inclusion $R \supset R'$ of log spin canonical
rings such that $R'$ is generated by elements $x_{d_i, i}$ and $R$
is generated over $R'$ by elements $y_j$. In these cases, it is
...
Let $k[x_{d_1, 1}, \ldots, x_{d_n, n}]$ and $k[y_{c_1, 1}, \ldots,
y_{c_m, m}]$ be weighted polynomial rings with existing term
orders $\prec_x$ and $\prec_y$, respectively. Let $\alpha :=
\prod_{i = 1}^{n} x_{d_i, i}^{f_i}$ $\prod_{j = 1}^{m} y_{c_j, j}
^{h_i}$ and $\beta := \prod_{i = 1}^{n} x_{d_i, i}^{f_i'}
\prod_{j = 1}^{m} y_{c_j, j}^{h_i'}$ be monomials in $k[x_{d_1, 1},
\ldots, x_{d_n, n}] \otimes$ $k[y_{d_1', 1}, \ldots, y_{d_m', m}]$.
Let $\alpha_x := \prod_{i = 1}^{n} x_{d_i, i}^{f_i}$ be the
part of $\alpha$ in $k[x_{d_1, 1}, \ldots, x_{d_n, n}]$ and
likewise with $\alpha_y$, $\beta_x$, and $\beta_y$.

In the \textbf{(graded) block} (or \textbf{elimination}) term
ordering on $k[x_{d_1, 1}, \ldots, x_{d_n, n}] \otimes k[y_{d_1', 1},
\ldots, y_{d_m', m}]$, we define $\alpha \succ \beta$ if
  \begin{enumerate}
	\item[(i)] $\deg \alpha  > \deg \beta$ or
  \item[(ii)] $\deg \alpha = \deg \beta$ and 
    \begin{enumerate}
    \item $\alpha_y \succ_y \beta_y$ or
    \item $\alpha_y = \beta_y$ and $\alpha_x \succ_x \beta_x$
    \end{enumerate}
  \end{enumerate}
\end{defn}

\todo{write below part}
For a homogeneous polynomial $f \in k[x]_{\vec{a}}$, we define the \textbf{initial term} $\initial_{\prec}(f)$ to be the monomial in the support of $f$ that is largest with respect to $\prec$, and we define $\initial_{\prec}(0)=0$.  Let $I \subseteq k[x]_{\vec{a}}$ be a homogeneous ideal.  We define the \textbf{initial ideal} $\initial_{\prec}(I)$ to be the ideal generated by $\{\initial_{\prec}(f) : f \in I\}$.  A \textbf{Gr\"obner basis} (also called a \textbf{standard basis}) for $I$ is a set $\{f_1,\dots,f_n\} \subset I$ such that 
+\[ \langle \initial_{\prec}(f_1),\dots,\initial_{\prec}(f_n) \rangle = \initial_{\prec}(I). \]
 
\begin{rem}
A Gr\"obner basis for $I$ \emph{a priori} depends on a choice of basis for the ambient graded polynomial ring.
\end{rem}
...
\todo{Pervasive change: Make sure all $\initial(I)$ are written as $\initial_\succ(I)$. We can even build this in to the macro possibly? }
...
By (3), every monomial of $k[x_1, \ldots, x_m, y_1, \ldots, y_n]$ is an element of either $S$, $\langle T\rangle$, or $\initial_\prec(I') k[x_1, \ldots, x_m, y_1, \ldots, y_n].$ Therefore,
\begin{align*}
	\initial(I) & \subseteq \initial(I') k[x_1, \ldots, x_m, y_1, \ldots, y_n] + \langle T \rangle.
\end{align*}
\end{proof}

To apply Lemma \ref{lem:relations_from_generators_induction}, we will need an appropriate monomial ordering.  \todo{Reference the lemma, instead of using previous, since things might get moved around. Aaron: Great, thanks!}
...
\todo{Perhaps note that this ordering is independent of whether we take the order relative to a line bundle}
\noindent One can easily verify graded $P$-lex is a monomial ordering in the sense defined in Cox--Little--Shea \cite[Chapter 2, $\mathsection$ 2, Definition 1]{cls:ideals-varieties-algorithms}.

We area almost ready to state Lemma ~\ref{lem:sat-1}, which will yield an inductive procedure for determining the generators and relations of $R_D$, where $D \in \di \BP^1$ is an effective divisor. While in O'Dorney \cite[Theorem 8]{dorney:canonical} gives a bound for the degree of generation of arbitrary $D \in \di \BP^1$, we obtain a much tighter bound in the case $D$ is effective. Furthermore, Lemma ~\ref{lem:sat-1} also applies to curves of genus $g > 0$. Before stating Lemma ~\ref{lem:sat-1}, we recall some notation from O'Dorney \cite{dorney:canonical}.

\todo{Peter: This sentence is very ugly.  On the other hand, I feel we need to say something about O'Dorney before the following definition, or else it is completely unmotivated.  How should we state it better?  Also note that in its current form this sentence neglects that we also extend the result to higher genus; it was too difficult to have this in though. Aaron: Do you like it any better now?}
...
\item[(c)] In particular, $R_\halfcan$ is generated over $R_\halfcan'$ in degrees at most $\beta$ with $I$ generated over $I'$ in degrees at most $\max(2\beta, \beta + \tau)$ (where $\tau = \max(1, \max_{1\le i, m}(\deg(x_i)))$).  \todo{Peter: Part (c) is immediate from (a) and (b), so maybe it shouldn't be its own section.  On the other hand, the other parts already have a lot of information written down (especial part (b)), so its not clear where it should be put.  Maybe in a remark after the lemma? Aaron: I like including it in the lemma. I think it's really the most important point of this, so it's fine to include it here. It's also ok to just include this as a statement after parts a and b, if we don't particularly care about referencing part c.}
...
\alpha}{\beta}$. Note that the elements $t_1,\ldots, t_{\lfloor d_i \frac{\alpha}{\beta}\rfloor}$ in $(R_{\halfcan})_{d_i}$ with $\ord_P^{\halfcan'}(t_i) = i$ are linearly independent over $(R_{\halfcan'})_d$. Hence, by a dimension count, these sections form a $k$-basis for
$(R_{\halfcan})_d$ over $(R_{\halfcan'})_d$.\todo{take out two sentences}

%For any $1\le i < j \le n$, we have $\frac{c_i}{d_i} < \frac{c_j}{d_j}$.  
Choose a positive best lower approximation $\frac{c_i}{d_i}$.  Choose $z_1, \ldots, z_r \in R_\halfcan$ so that $\sum_{j=1}^r \deg(z_j) = d_i$ and $\deg(z_j)<d_i$ for all $j\in \{1, \ldots r\}$. Observe that $\frac{c_i}{d_i}$ is a best lower approximation for $\frac{\alpha}{\beta}$ and $\frac{-\ord_P^{\halfcan'}(z_j)}{\deg(z_j)}< \frac{\alpha}{\beta}$, for each $j$.  Hence, for each $j$, 
\[
...
\halfcan})_{d_i}$ with $-\ord_{P}^{L'}(y_i)=c_i$.  Note that this choice is generic if $d_i>1$ or $d_i=1$ and $c_i=\lfloor \frac{\alpha}{\beta}\rfloor$. \todo{This sentence on genericness seems like it does not belong in the proof.}

Choose $c,d\in \mathbb{N}$ so that $d\le \beta,$  $\frac{c}{d} < \frac{\alpha}{
\beta},$ and $\frac{c}{d}$ is not a best lower approximation of $\frac{\alpha}{\beta}$. Choose a
best lower approximation $\frac{c_i}{d_i}$ with $c_i$ maximal such that $d_i< d$.  This implies $\frac{c_i}{d_i}\ge \frac{c}{d},$ so \todo{Why does the next equation follow from this inequality?}\todo{Aaron: Is it true that $c_1 \leq \cdots \leq c_n$? I think it is true and that it is implicitly being used here and later. Perhaps this needs to be said, or perhaps it doesn't, I'm not sure.}
...
\[
	S_i = y_j S_{i-j}.
\]
Observe that this recursive definition ensures that $S_i$ contains only elements $z\in R_\halfcan$ satisfying $-\ord^{\halfcan'}_P(z)=i$. \todo{Perhaps it's worth reminding the reader here that $y_i$ are as in the statement of (a) and have order $-c_i$.}
Then define 
\[
	S = \cup_{i=1}^{\infty} S_i.  
\]\todo{make equation and reference}
Note that $S_0$ is not part of this union and in fact $S\cap S_0 = \emptyset$ by pole order considerations.

Applying Equation \ref{eqn:c-from-lower-terms}, we see that $S$ contains elements in degree $d\in \mathbb{N}$ with each pole order in $\{1 \ldots, \lfloor d\frac{\alpha}{\beta}\rfloor\}$. \todo{Maybe we should make reference these generating the desired lattice, as described in Evan's paper? I think this is basically a rehashing of Evan's 1-point case?} Thus, by dimension counting, $S$ forms a $k$-basis for $R_\halfcan$ over $R_{\halfcan'}$, and we have proven part (a) \todo{why follows from 3.1}.
...
We check that $S$, $T$, and $\prec$ meet the hypothesis of Lemma \ref{lem:relations_from_generators_induction}.  In part (a), we have shown $S$ \todo{What is $S$. I presume it is the complement, but can you spell it out?} forms a $k$-basis for $R_\halfcan$ over $R_{\halfcan'}$ implying condition (1) of Lemma ~\ref{lem:relations_from_generators_induction} holds.  Our choice of order for monomials in $k[y_1, \ldots, y_n]$ and block order for those in $k[u, x_1, \ldots, x_m, y_1, \ldots, y_n]$ implies that $T\succ S\succ k[u, x_1, \ldots, x_m]$, implying that condition (2) of Lemma \ref{lem:relations_from_generators_induction} holds.  
...
\]
Thus, we see that if $f\in k[u, y_1, \ldots, y_n]$, then $f=u^b \prod_{j=1}^l {y_j}^{a_j}\in S$ for some $1\le l\le n$, $b\in \mathbb{Z}_{\ge 0}$, so that $a_l > 0$ and $a_j\in \mathbb{Z}_{\ge 0}$ for $1 \leq j \leq l$. For $j\in \mathbb{N}$ with $l < j\le n$, assign $a_j=0$. \todo{Is there a reason why you don't just take the product up to $n$ and let $l$ be the highest number so that $a_l \neq 0$? This seems simpler to me}
If $i\in \{1, \ldots, n\}$, then $y_i f$ is either in $S$ or there is some $h\in \mathbb{N}$ such that $i\le h\le \max(i,l)$ satisfying $\prod_{j=1}^h y_j^{a_j}\not\in S_{\sum_{j=1}^h c_j a_j}$ \todo{Peter: terrible subscripts... Aaron: Indeed, why not just omit the subscript?}, 
and for all $r<h$, we have $\prod_{j=1}^r {y_j}^{a_j}\in S_{\sum_{j=1}^r c_ja_j}$.  We see that this latter case when $y_i f \notin S$ is equivalent to $\prod_{j=1}^h {y_j}^{a_j}\in U_h$. \todo{Aaron: Why to we see this? Presumably this is using conditions 1,2,3 but it would be helpful for you to spell out which. Also, what are the latter and former cases? I don't see the casework anywhere}
Thus, since $S$ generates $\cup_{i=1}^n y_i \cdot k[u, y_1, \ldots, y_n]$ and $\bigcup_{i=1}^{n-1} U_i$ generates every monomial in $\{y_1, \ldots, y_n\} k[y_1, \ldots, y_n]-S$, we have shown that all monomials of $k[u, x_1, \ldots, x_m, y_1, \ldots, y_n]$ are contained in
\begin{align*}
...
3}P$, where $P\in X$ is a base point of $\halfcan'$ (which includes the case when $H^0(X, \lfloor \halfcan'\rfloor) = 0$). \todo{Ask David, is it useful to include this parenthetical?}
...
\begin{enumerate}
	...
	\item[(b)] Equip $k[y_3, y_4]$ with $grevlex$ so that $y_3 \prec 
		...
		order with $y_3, y_4$ preceeding $x_2, \ldots, x_m$. Then,
		\begin{align*}
...
Therefore, general elements $y_3$ and $y_4$ satisfy $-\ord_P^{\halfcan'}(y_i) = 1$ by Riemann-Roch, proving part (a). \todo{Ask David, Understand why/whether we need to cite Lemma 5.4.7.}

A quick computation checks that
the following set $S$ is a $k$ basis for $R$ over $R'$:
...
\begin{enumerate}
	...
	\item[(b)] Equip $k[y_3, y_4, y_5]$ with $grevlex$ so that $y_3 \prec 
		...
		\begin{align*}
			\initial_\prec(I) &= \initial_\prec(I) k[y_3, y_4, y_5, x_3, \ldots, x_m] \\
...
we cannot assume that $\{P_i\} \cap \Supp(L_X) = \emptyset$, as $H^0(X, L_X)$ may have no global sections.  We circumvent this issue by working with the orders of zeros and poles relative to the divisor we are inducting from, rather than relative to the structure sheaf.\todo{Peter: This last sentence is kind of awkward.  I changed ``instead of relative to the structure sheaf'' to ``rather than relative to the structure sheaf,'' but it can still be made to sound cleaner... I hope...}

\begin{defn}
\label{defn:admissible}
Let $(\sx', \Delta, L'),(\sx, \Delta , L)$ be log spin
curves, with the same course space $X$ and stacky points $Q_1, \ldots, Q_r$. Let $J \subset
...
The following Lemma will slightly strengthen condition (Ad-ii) from Definition ~\ref{defn:admissible} and this improvement is crucially used in the proof of part (c) of Lemma ~\ref{lem:raise-stacky-order}. Lemma ~\ref{lem:admissible_inequality} can be thought of as an analog of \cite[Lemma 8.5.3]{vzb:stacky}.  \todo{Peter: In general, I'm not sure how important it is for us to draw all the analogs between your definition and proofs and those of VZB.  As long as we make not in the section's introduction that they are similar, we might leave it out of the remaining transitions and remarks.}

\begin{lem}
\label{lem:admissible_inequality}
Condition (Ad-ii) of Definition ~\ref{defn:admissible} implies the
stronger condition \todo{Peter: How is it a stronger condition if it implied by that of (Ad-ii).  If $a\implies b$ then $b$ is by definition a weaker condition right?  I agree that the latter condition is somehow better, but it seems inaccurate to call it stronger.  I may be being over picky here...} that
...
(k[x_{3, 1}, x_{5, 1}, x_{7, 1}, x_{7, 2}] / in_\prec(I))_d$ for 
