...
Suppose $(\sx,\Delta,L)$ is a log spin curve. Note that $\deg \lfloor L \rfloor$ is even. because $2 \cdot \deg L = \deg \Delta + \deg \lfloor K \rfloor = \deg \Delta + 2(g-1).$ Therefore, $\deg \Delta$ is even. In particular, we shall often use $\deg \Delta \neq 1$.
\end{rem}
