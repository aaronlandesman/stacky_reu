...
There exists a log half-canonical divisor $\halfcan$ on $\sx$ such
that we have a graded isomorphism of $\BC$-algebras
\[
	\bigoplus_{k = 0}^{\infty} M_k(\Gamma) \cong R(\sx (\Gamma), 
\Delta, \halfcan)
\]

\noindent
Let $\sigma = (g; e_1, \ldots, e_r; \delta)$ be the signature
of $\sx$. Then $\bigoplus_{k = 0}^{\infty} M_k(\Gamma)$ is
