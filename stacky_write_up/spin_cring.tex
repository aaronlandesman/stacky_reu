...
\label{sec:induction}
First we present several lemmata which provide the
inductive steps in the proof of the main theorem, Theorem
...
Before proving three inductive lemmas, we give a criteria to determine if a set of monomials generates the initial ideal of relations of $k[x_1, \ldots, x_m]\to R_D$.  \todo{Perhaps say which parts of the proofs of which future lemmas it will be used in?}
...
Now, let $f\in T$.  Since $S$ forms a $k$-basis of $R_{\halfcan}$ over $R_{\halfcan'}$ by (1), we can write $f - (\sum_{g\in S'} C_g g)-r=0$ for some finite subset $S'\subseteq S_{\deg(f)}$, $C_g\in k$ for all $g\in S'$, and $r\in R_{\halfcan'}$.  This demonstrates that 
\begin{align*}
...
To apply the previous lemma, we will need an appropriate monomial ordering.  \todo{Reference the lemma, instead of using previous, since things might get moved around}
The following definition provides the necessary ordering for the subsequent lemma (Lemma \ref{lem:sat-1}).
\begin{defn}
Suppose $\halfcan$ is a divisor of $X$ such that $R_\halfcan$ is generated $x_1, \ldots x_m$.  Then we have a map $\phi: k[z_1, \ldots z_m]\to R_\halfcan$ given by $z_i\mapsto x_i$.  If $P$ is a point in $X$, then $\phi$ defines a graded $P$ reverse lexicographic order {\bf{graded P-revlex}} on $k[z_1, \ldots, z_m]$ as follows: 
If $f=\prod_{i=1}^m {x_i}^{q_i}\ne g=\prod_{i=1}^m {x_i}^{r_i}$, then $f\prec g$ if one of the following holds:
\begin{enumerate}
\item $\deg(f) < \deg(g)$
...
\end{enumerate}
\end{defn}
\noindent One can easily verify that these satisfy the conditions of an ordering.  

Before stating the subsequent lemma, which improves O'Dorney's bounds on generators and relations over $\mathbb{P}^1$ \cite[Theorem 8]{dorney:canonical} in the case of effective divisors, we first introduce some of his notation \cite{dorney:canonical}. 

\begin{defn}\label{den:lower-approximation}
If $\frac{\alpha}{\beta}\in \BQ$ (written in reduced form with $\alpha \in \mathbb{Z}, \beta> \mathbb{N}$), then $\frac{c}{d}$ (written in reduced form) is a {\bf{best lower approximation}} to $\frac{\alpha}{\beta}$ if there is no $\frac{c'}{d'}\in \mathbf{Q}$ with $0<d'<d$ and $\frac{c}{d}\le \frac{c'}{d'}$. 
\end{defn}
...
$u, x_1, \ldots, x_m$ in degree at most $\sigma$, with $\deg u = 1$, $\ord_P^{\halfcan'}(x_i)=0$ for all $1 \leq i \leq m$, and $\ord_P^{\halfcan'}(u) = 0$.  Suppose $\halfcan = \halfcan' + \frac{\alpha}{\beta} P$
...
\]
Equip $k[y_1, \ldots, y_n]$ with graded P-revlex and $k[u, x_1, \ldots, x_m, y_1, \ldots, y_n]$ with block order.  
...
\item[(c)] In particular, $R_\halfcan$ is generated over $R_\halfcan'$ in degrees at most $\beta$ with $I$ generated over $I'$ in degrees at most $\max(2\beta, \beta + \sigma)$.
...
\[
	h^0 (X, d \halfcan ) = h^0(X, d\halfcan') + d\frac{\alpha}{\beta}
\]
...
c_i $ at $P$.  Moreover, a set of $\lfloor d_i \frac{\alpha}{\beta}\rfloor$ elements in $(R_{D})_{d_i}$ with pole orders $1, \ldots, \lfloor
d_i \frac{\alpha}{\beta} \rfloor$ are linearly independent over $(R_{\halfcan'})_d$, and thus form a $k$-basis for
...
We now restrict our attention to log canonical rings of stacky curves.  The previous lemma accounts for the relevant induction cases when the half-canonical ring is already saturated in 1.  We complement it with the following two lemmas that allow us to inductively add points, under certain conditions when the half-canonical ring is saturated at degrees two and three.
