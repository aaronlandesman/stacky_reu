...
\begin{proof}
First, note that since we are in genus 0, we can choose the elements $y_3,y_4,$ and $y_5$ are general and have $-\ord
...
\begin{longtable}	{| c || c | c | c |}
	...
	\hline
	\hline
	$(1;-; 0)$ & $\{2\}$ & $\emptyset$ \\	\hline

	$(1; 3; 0)$ & $\{2,3,7\}$ & $\{14\}$ \\	\hline

	$(1; 5; 0)$ & $\{2,3,5\}$ & $\{12\}$ \\	\hline
...
Table ~\ref{table:g-1-exceptional}. In the following example we will explain the case $\sigma = (1;5;0)$ as the other cases are even easier. 
...
~\ref{ssec:g_0_ram_orders} \todo{fix} we prove inductive theorems, and in
...
$\halfcan$ is an effective divisor. \todo{need to do this case}
...
~\ref{lem:sat_three_induction} to add ramified points with
...
~\ref{lem:sat_three_induction} (recall from Subsection ~\ref
{ssec:g_0_saturation} that saturation is 3 here), and the
immediate consequence of parts (a) and (c) of Lemma
~\ref{lem:sat_three_induction} is that any $R(\sx', \Delta,
...
the $r > 5$ case with ramification orders all $3$, with $\sigma' =
...
%By Lemma ~\ref{lem:g_0_admissible_cases}, we know that for
%any tame log spin curves $(\sx', \Delta, \halfcan')$ over a perfect
%field $k$ with signature $\sigma'$ and set $J$ such that $(\sigma',
%J)$ is one of the listed base cases, $(\sigma', J)$ is admissible.
%Furthermore, $R' := R(\sx', \Delta, \halfcan')$ is generated as a $k
%$- algebra by elements of degree at most $e = \max(5, e_1, \ldots, e
%_r)$ with relations in degree at most $2e$. It also admits a
%presentation
%
%\begin{align*}
	%R' \cong \left( k[x_1, \ldots, x_m] \otimes k[y_{i,e_i'}]_{i \in J} \right)/I'
...
%
%\noindent
%given by admissibility satisfying the statements that for all $i \in 
%J$, $e_i > \deg z$ for any generator $z$ of $R'$ and that $in_\prec
%(I')$ is minimally generated by products of two monomials under
%some ordering.
%
%Then we can apply two processes of induction, by adding points via
%Lemma ~\ref{lem:sat_two_induction} and by raising
%ramification orders via Theorem ~\ref{thm:ram_order_ind},
%to obtain the desired result for any tame log spin curve $(\sx,
%\Delta, L)$ over a perfect field $k$ with signature $\sigma$
%covered by this induction process.
%
%
%
...
%number of ramified points via Lemma
%~\ref{lem:sat_three_induction} on Lemma ~\ref{lem:g_0_admissible_cases}
