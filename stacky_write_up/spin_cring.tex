...
Then 
\begin{itemize}
\item $R_{D'}$ is generated over $R_D$ by elements $y_1, \ldots, y_n$ such that $y_i$ lies in  
...
\item A Grobner basis with the initial terms $y_i x_j$, $y_i y_j$,
...
Suppose $(\sx', 0, \halfcan')$ is a log spin curve with coarse
...
$(\sx, 0, \halfcan)$ be another log spin curve
...
\todo{Aaron: It becomes a bit tedious for this proof to alternatingly cite ~\ref{lem:admissible_subset} and ~\ref{thm:ram_order_ind} is there a reason these were separated/can we merge the back together? Better yet, this should probably be a corollary somewhere, since we have to use this implicitly in all three genus cases.}
