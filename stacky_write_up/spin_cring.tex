...
Before proving three inductive lemmas, we introduce some notation from O'Dorney \cite{dorney:canonical}. 
\begin{defn}\label{den:lower-approximation}
If $\frac{\alpha}{\beta}\in \BQ$ (for $\alpha, \beta \in \mathbb{Z}, \beta> 0$ written in reduced form), then a \begin{bf}{best lower approximation}\end{bf} to $\frac{\alpha}{\beta}$ is a a fraction $\frac{c}{d}$ (written in reduced form with $d>0$) such that there is no $\frac{c'}{d'}$ with $0<d'<d$ and $\frac{c}{d}\le \frac{c'}{d'}$. 
\end{defn}
Moreover, the non-negative best lower approximations of $\frac{\alpha}{\beta}$ form a sequence
\[
...
\caption{This figure shows the positive best lower approximations shown as $*$ and other lattice points shown in $o$ of $\frac{14}{5}$.  Note that they minimally generate lattice in $\mathbb{N}\times \mathbb{N}$ not above $5y=14x$.}
\end{figure}

Previously discussed O'Dorney's result and our improvement when restricting to effective. Moved to Intro.
\todo{new transition}

Before proving three inductive lemmas, we give a criteria to determine if a set of monomials generates the initial ideal of relations.  \todo{Perhaps say which parts of the proofs of which future lemmas it will be used in?}
...
Now, suppose a monomial $f\in T$ has a pole order $a$ at $P$. Then, since $S$ forms a $k$-basis of $R_{\halfcan}$ over $R_{\halfcan'}$ by (1), we can write $f - (\sum_{g\in S'} C_g g)-r=0$ for some finite subset $S'\subseteq S_{\deg(f)}$, $C_g\in k$ for all $g\in S'$, and $r\in R_{\halfcan'}$.  This demonstrates that 
\begin{align*}
	\initial(I) & \supseteq \langle T \rangle.
\end{align*}

Finally, suppose $f\in S$.  Note that by (2) any polynomial $G$ in $x_1,\ldots, x_m, y_1, \ldots, y_n$ with initial term in $S$ cannot have a term in $T$.  Furthermore, since $S$ forms a $k$-basis for $R_\halfcan$ over $R_{\halfcan'}$ by (1), any such polynomial does not map to an element of $R_{\halfcan'}$ (since it has a non-zero constant factor with $f$).  Thus, $G=0$ is not a relation, so $f\not\in \initial(I)$.  Therefore, 
\[
	\initial(I)\subseteq R_\halfcan-S.
\]

Since every monomial of $k[x_1, \ldots, x_m, y_1, \ldots, y_n]$ is an element of either $S$, $\langle T\rangle$, or $\initial_\prec(I') k[x_1, \ldots, x_m, y_1, \ldots, y_n]$ by (3),
\begin{align*}
	\initial(I) & = \initial(I') k[x_1, \ldots, x_m, y_1, \ldots, y_n] + \langle T \rangle.
\end{align*}
\end{proof}

As suggested by the the previous lemma, \todo{Reference the lemma, instead of using previous, since things might get moved around} we will need an ordering on monomials, and the following definition will prove useful in the ensuing lemma. \todo{Again, reference this}
\begin{defn}
We equip the graded ring $k[x_1, \ldots x_m]$, with weight $a_i$ and pole order $-\ord_P(x_i)$ attached to $x_i$, \todo{attached to $x_i$ doesn't really sound like the correct terminology to me} the graded $P$ reverse lexicographic order {\bf{g-P-revlex}} \todo{This name seems a bit funny looking to me} if for $f=\prod_{i=1}^m {x_i}^{q_i}\ne g=\prod_{i=1}^m {x_i}^{r_i}$, $f\prec g$ if one of the following holds
\begin{enumerate}
\item $\deg(f) < \deg(g)$
\item $\deg(f) = \deg(g)$ and $-\ord_P(f) > -\ord_O(g)$
\item $\deg(f) = \deg(g)$, $-\ord_P(f) = -\ord_P(g)$, and for $i$ maximal such that $q_i-r_i$ is non-zero, $q_i-r_i$ is negative. \todo{I do not understand what this means, maximal under what ordering?}
\end{enumerate}
\end{defn}
One can easily verify that these satisfy the conditions of an ordering.  
We now state a lemma that allows us to inductively add $\mathbb{Q}$-points \todo{$\BQ$ points usually refers to maps to $\spec \BQ$. Maybe we can just say a rational multiple of a poitn?} to a divisor and describe the new generators and relations.
...
satisfying $h^0(X, \lfloor{\halfcan'}\rfloor)\ge 1$. Suppose $P$ is not a base-point of $d\halfcan'$ for all $d\in \mathbb{N}^+$ meaning that $R_{\halfcan'}$ has generators
...
\begin{enumerate}
\item[(a)] $R_{\halfcan}$ is generated over $R_{\halfcan'}$ by elements $y_1, \ldots, y_n$ such thaty $\deg(y_i)=d_i$ and $-\ord_P(y_i)=c_i$ when viewed as sections of $\so_{\halfcan'}$.

\item[(b)] Choose an ordering $\prec$ on $k[u, x_1, \ldots, x_m]$ such that
\[
	\ord_u(f) < \ord_u(g) \implies f\prec g
\]
and equip $k[y_1, \ldots, y_n]$ with g-P-revlex and $k[u, x_1, \ldots, x_m, y_1, \ldots, y_n]$ with block order \todo{it should be called block order, instead of ``a block order''}.
...
$V=\{x_i y_j: 1\le i\le m, 1\le j\le n\}$
 and $U_i$ is the set of monomials of the form $\prod_{j=1}^{i} y_j^{a_j}$ with $a_j\in \mathbb{Z}^{\ge 0}, a_i\ne 0$ such that
\begin{enumerate}
	\item $\sum_{j=1}^i a_j c_j \ge c_{i+1}$\\
	\item there does not exist $((b_1, \ldots b_i)\ne (a_1, \ldots a_i)$ such that with all $b_j\le a_j$ and $\sum_{j=1}^i b_jc_j\ge c_{i+1})$ \\
	\item there does not exist $h<i$ such that $\sum_{j=1}^h y_j^{a_j}\ge c_{h+1}$
\end{enumerate}
...
\beta} \rfloor \}$. We will use positive best lower approximations to
construct the generators $\{y_i\}$ as described in the lemma's statement.
