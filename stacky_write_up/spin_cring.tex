...
rings, Voight and Zureick-Brown bounded generator degrees by $3 \cdot \max(e_1, \ldots, e_r)$
and relation degrees by $6 \cdot \max(e_1, \ldots, e_r)$ when $\halfcan$ is effective
...
\todo{Peter: The paragraph used to be confusingly redundant.  I think I fixed it but read through the commented out stuff to make sure it says the correct thing. Robin: Why are you ignoring Lemma 10.2.1 as well as its connection to Kodaira--Spencer? These are necessary.}
...
\begin{proof}
First, we shall show the assumptions on $g$ imply $H^0(\sx, 3 \halfcan), H^0(\sx, 4 \halfcan)$ are both basepoint-free: If $g \geq 2$ then $\deg 3 \halfcan > 2g - 1$ \todo{If $g = 2$ then we in fact have $\deg 3L = 3g - 3 = 2g - 1.$ Is this a big problem? It's almost basepoint-free, except if the point you take out yields the canonical divisor, after removing it.} and $\deg 4 \halfcan > 2g - 1$, so $H^0(\sx, 3 \halfcan)$ and $H^0(\sx, 4 \halfcan)$ are base point free. If $g = 1$, we assume $\deg 3 \halfcan \geq 2 > 2g - 1$, so we also have $\deg 4 \halfcan \geq 2 > 2g - 1,$ so again $H^0(\sx, 3 \halfcan)$ and $H^0(\sx, 4 \halfcan)$ are base point free. 
...
In the case of a genus 1 curve, $X$, we know $K_X \sim 0$, and therefore the only possibilities for \todo{``log''?} spin canonical divisors are $\halfcan' \sim 0$ or $\halfcan' \sim P - Q$ where $P, Q$ are distinct points, fixed  under the hyperelliptic involution. We inductively construct presentations by adding points through Lemmas ~\ref{lem:sat-1} and ~\ref{lem:sat-2} and incrementing the values of the $e_i$'s using Lemma ~\ref{lem:raise-stacky-order}.
