...
\todo{Peter: I understand why this should be true, although its not clear to me that there it easily follows.  Maybe we should explain it a little as to not confuse the reader. Aaron: Can we ask David, this was clear to me in his paper, and he also didn't include it. Say by the definition}
...
This paper leaves open many directions for further research.\todo{Peter: This sentence sounds weird.  It's also kind of not true; as David said, we are kind of closing the topic}
...
\begin{enumerate}
	\item One direction for further research is to 
		...
		^{e_3} = 0$.  \todo{Peter: I don't know if it is so worth while giving an explicit definition and precisely defining the work already done.  A general outline might suffice and be easier/shorter to read.}

		This motivates the following question:

\todo{Peter: Would this be better as a conjecture?}
		\begin{question}
	...
	\item As noted in Remark ~\ref{rem:explicit-generators}, the 
		...
		\halfcan$ when the genus of $\sx$ \todo{Peter: Minor note: should we say genus of $\sx$ or genus of $X$; did we actually define that a genus of a stacky curve equals that of its coarse space?} is $0$ or $1$.  \todo{Peter: This question also seems like it might be the most important (as far as modular forms applications go.  Maybe we should put it first?}
		\begin{question}
...
\todo{Peter @ Aaron: I think the last sentence of the question can be removed; generic isn't relevant in this part of further questions (it was relevant in part (2)).  Also the sentence after the question can be removed.}
	\item In Subsection \ref{ssec:bounds-high-genus}, we reference 
		...
		\begin{conjecture}\label{ques:reduce-5,10-to-4,8}
			When $g \geq 2,$ the bounds in Theorem \ref{thm:main} on 
			...
			apart from a finite list of exceptions?
		\end{conjecture}
		\todo{Peter: I don't like a finite list of exceptions.  The list contains infinitely many non-isomorphic stacky curves.  Finite family might also be imperfect.  Can we say except when $g=2$ (I believe there will be infinitely many counterexamples there) or it lies in a finite list of exception?  Do we believe that to be true?}
...
%			apart from a well characterized list of exceptions.
	...
	\item While Theorem ~\ref{thm:main} gives a minimal set of 
		generators \todo{Peter: I believe this is actually not true.  If $R_{\halfcan'}$ is minimally generated by $x_1, \ldots x_m$ and $R_\halfcan$ is generated by $y_1, \ldots, y_n$ over $R_{\halfcan'}$, then $R_{\halfcan}$ is {\emph{generated}} by $x_1, \ldots, x_m, y_1, \ldots y_n$ but not necessarily minimally.  It's possible to have something like $y_1y_3-y_2^2=x_i$.}
		for the log spin canonical ring $R_\halfcan,$ the set of 
...
We are grateful to David Zureick-Brown for introducing us to the study of stacky canonical rings, providing invaluable guidance, and being \todo{Peter: using passive voice (i.e. ``being'') makes this sound kind of generic} an
...
collaboration. \todo{Peter: Does it make sense to thank the REU? Ken essentially is the REU.} We thank Jorge Sentieiro Neves for pointing out the
