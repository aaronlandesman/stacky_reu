...
\]
giving us a non-trivial relation with leading term $x_iy_j$. \todo{Explicitly state ordering at the beginning}
...
Finally, we examine the case (for $\alpha \ge \beta$) of $(y_i)^{i + 1}$ when $i=\lfloor \frac{\alpha}{\beta}\rfloor$.  If $\beta=1$, then there are no relations with initial term a power of $y_i$ and we are immediately done.  Otherwise, assuming $\beta\ne 1$, we note that $c_id_{i+1}=c_{i+1}d_i+1=c_{i+1}+1$. \todo{reference to Evan's paper where he proves this}  Therefore, if $i>1$ then $c_i^{d_{i+1}+1}$ and $c_{i-1} c_{i+1}$ both lie in the $d_{i+1}+1$ degree with pole order $c_{i+1}+1$.  Following a similar technique to the previous paragraphs we find
\[
...
\todo{lots of corollaries; peter can do this tomorrow}
...
\begin{align*}
	\ord_{x_3}(f) < \ord_{x_3}(g) \implies f \prec g.
\end{align*}
...
\begin{enumerate}
	...
	\item[(b)] $\sigma_2 = (0; 3, 5, 9; 0), J_2 = \{2, 3\}$
	\item[(c)] $\sigma_3 = (0; 5, 5, 7; 0), J_3 = \{1, 2, 3\}$
	\item[(d)] $\sigma_4 = (0; 3, 3, 3, 5; 0), J_4 = \{1, 2, 3, 4\}$
	\item[(e)] $\sigma_5 = (0; 3, 3, 3, 3, 3; 0), J_5 = \{1, 2, 3, 4, 5\}$
\end{enumerate}
\end{lem}
...
\begin{enumerate}
	\item[(a)] $(\deg \lfloor e_3 \rfloor, \max_{d \geq 0} \# S_{(
		\sigma_1, J_1)}(3)) = (1, 0)$

	\item[(b)] $(\deg \lfloor e_2 \rfloor, \max_{d \geq 0} \#S_{(
		\sigma_2, J_2)}(2)) = (1, 0)$ \\
		$(\deg \lfloor e_3 \rfloor, \max_{d \geq 0} \#S_{(\sigma_2, J_2)}(3))
		= (1, 0)$

	\item[(c)] $(\deg \lfloor e_1 \rfloor, \max_{d \geq 0}
		\#S_{(\sigma_3, J_3)}(1)) = (2, 1)$ \\
		$(\deg \lfloor e_2 \rfloor, \max_{d \geq 0}
		\#S_{(\sigma_3, J_3)}(2)) = (2, 1)$ \\
		$(\deg \lfloor e_3 \rfloor, \max_{d \geq 0}
		\#S_{(\sigma_3, J_3)}(3)) = (1, 0)$

	\item[(d)] $(\deg \lfloor e_i \rfloor, \max_{d \geq 0}
		\#S_{(\sigma_4, J_4)}(i)) = (3, 1)$	for $i \neq 4$ \\
		$(\deg \lfloor e_4 \rfloor, \max_{d \geq 0} \#S_{(\sigma_4, J_4)}(4))
		= (2, 0)$.

	\item[(e)] $(\deg \lfloor e_i \rfloor, \max_{d \geq 0}
		\#S_{(\sigma_5, J_5)}(1)) = (5, 0)$	for all $i \in J$.
\end{enumerate}
